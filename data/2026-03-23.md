...
TL;DR: EgoForge: Egocentric goal-directed world simulator generating coherent first-person video from minimal static inputs using trajectory-level reward-guided diffusion refinement.


<details>
  <summary>Details</summary>
Motivation: Generative world models struggle with egocentric video due to rapid viewpoint changes, frequent hand-object interactions, and goal-directed procedures dependent on latent human intent. Existing approaches have limitations: hand-centric instructional synthesis with limited scene evolution, static view translation without action dynamics, or reliance on dense supervision like camera trajectories and long video prefixes.

Method: EgoForge uses minimal static inputs: single egocentric image, high-level instruction, and optional auxiliary exocentric view. Introduces VideoDiffusionNFT, a trajectory-level reward-guided refinement that optimizes goal completion, temporal causality, scene consistency, and perceptual fidelity during diffusion sampling.
...
Conclusion: EgoForge represents an effective egocentric goal-directed world simulator that generates coherent first-person video from minimal inputs, addressing key challenges in egocentric video generation through trajectory-level reward-guided refinement.
...
TL;DR: SemanticFL is a novel federated learning framework that uses pre-trained diffusion model semantics to address non-IID data challenges in multimodal perception tasks, achieving significant accuracy improvements over baselines.


<details>
  <summary>Details</summary>
Motivation: Federated learning suffers from performance degradation due to non-IID client data, especially in multimodal perception settings. Conventional methods fail to address semantic discrepancies between clients, leading to suboptimal performance for multimedia systems requiring robust perception.

Method: Leverages multi-layer semantic representations from pre-trained Stable Diffusion (VAE-encoded latents and U-Net hierarchical features) to create a shared latent space aligning heterogeneous clients. Uses an efficient client-server architecture offloading heavy computation to the server, with a unified consistency mechanism employing cross-modal contrastive learning to stabilize convergence.

Result: Extensive experiments on CIFAR-10, CIFAR-100, and TinyImageNet under diverse heterogeneity scenarios show SemanticFL surpasses existing federated learning approaches, achieving accuracy gains of up to 5.49% over FedAvg.

Conclusion: SemanticFL effectively addresses non-IID challenges in federated learning by leveraging diffusion model semantics, validating its effectiveness in learning robust representations for heterogeneous and multimodal data in perception tasks.
...
TL;DR: A unified multi-task transformer framework for ultrasound image analysis that handles segmentation, detection, classification, and landmark regression across diverse datasets, improving performance from 67% to 85% on validation and achieving 81.84% average score on test set.


<details>
  <summary>Details</summary>
Motivation: Ultrasound images exhibit significant variability across scanners, operators, and anatomical targets, causing poor generalization of models trained in one setting to new hospitals and clinical conditions. The Foundation Model Challenge for Ultrasound Image Analysis (FMC-UIA) requires a single model to handle multiple tasks across diverse organs and datasets.

Method: A unified multi-task framework based on a transformer visual encoder from Qwen3-VL family. Intermediate token features are projected into spatial feature maps and fused using a lightweight multi-scale feature pyramid, enabling both pixel-level predictions and global reasoning. Each task uses small task-specific prediction heads, with training employing task-aware sampling and selective loss balancing to manage heterogeneous supervision and reduce task imbalance.

Result: Performance improved from 67% to 85% on the validation set and achieved an average score of 81.84% on the official test set across all tasks (segmentation, detection, classification, and landmark regression).

Conclusion: The proposed unified multi-task framework is simple to optimize and adaptable across a wide range of ultrasound analysis tasks, effectively addressing the generalization challenges posed by ultrasound image variability across different clinical settings.
...
<details>
  <summary>Details</summary>
Motivation: Current FireANTs uses Adam optimizer which requires storing momentum and squared-momentum estimates, consuming significant memory and prohibiting use for large images. There's a need for more memory-efficient optimization methods for diffeomorphic image registration.

Method: Proposes a modified Levenberg-Marquardt optimizer with only a single scalar damping parameter as optimizer state, adaptively tuned using a trust region approach. Also investigates Metropolis-Hastings style rejection steps to prevent updates that worsen the loss function.

Result: The proposed optimizer reduces memory by up to 24.6% for large volumes while retaining performance across all four datasets. A single hyperparameter configuration transfers without modification across different modalities (brain MRI, lung CT, abdominal registration) and matches or outperforms Adam on three of four benchmarks.

Conclusion: The modified Levenberg-Marquardt optimizer provides a memory-efficient alternative to Adam for diffeomorphic image registration, enabling registration of significantly larger images while maintaining or improving performance across diverse medical imaging modalities.
...
TL;DR: LoFi introduces location-aware fine-grained representation learning for chest X-ray retrieval and phrase grounding, using joint optimization of sigmoid, captioning, and location-aware captioning losses with a lightweight LLM.


<details>
  <summary>Details</summary>
Motivation: Current contrastive models lack region-level supervision, and large vision language models fail to capture fine-grained representations in external validation, leading to suboptimal performance on retrieval and phrase grounding tasks in chest X-rays where clinically relevant findings are spatially confined.

Method: Proposes Location-aware Fine-grained representation learning (LoFi) that jointly optimizes sigmoid, captioning, and location-aware captioning losses using a lightweight large language model. The location-aware captioning loss provides region-level supervision through grounding and dense captioning objectives. Integrates a fine-grained encoder into retrieval-based in-context learning for enhanced chest X-ray grounding.

Result: Extensive experiments demonstrate superior retrieval and phrase grounding performance on MIMIC-CXR and PadChest-GR datasets.

Conclusion: LoFi effectively addresses limitations in fine-grained representation learning for chest X-rays by providing region-level supervision through location-aware captioning, enabling improved performance on clinically important retrieval and phrase grounding tasks.
...
TL;DR: A framework for vehicle camouflage attacks formulated as conditional image-editing, using ControlNet to synthesize camouflaged vehicles on real images with joint optimization for structural fidelity, style consistency, and adversarial effectiveness.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks are vulnerable to adversarial attacks, particularly camouflage attacks that manipulate object appearance to deceive detectors while remaining stealthy to humans. Existing approaches need improvement in attack effectiveness, structural preservation, and human-perceived stealthiness.
...
Result: Achieves significantly stronger attack effectiveness with >38% AP50 decrease on COCO and LINZ datasets. Better preserves vehicle structure and improves human-perceived stealthiness compared to existing approaches. Generalizes effectively to unseen black-box detectors and exhibits promising transferability to physical world.

Conclusion: The proposed framework successfully addresses vehicle camouflage attacks through conditional image-editing, demonstrating superior attack effectiveness, structural preservation, and stealthiness while showing strong generalization to black-box detectors and physical world transferability.
...
TL;DR: ProactiveBench benchmark reveals MLLMs lack proactive help-seeking behavior; proactiveness doesn't correlate with model capacity, and fine-tuning via RL can teach this skill.


<details>
  <summary>Details</summary>
Motivation: To investigate whether multimodal large language models (MLLMs) can exhibit proactive help-seeking behavior similar to humans, such as requesting user interventions when facing limitations like occluded objects, poor image quality, or ambiguous sketches.

Method: Introduce ProactiveBench, a benchmark built from seven repurposed datasets testing proactiveness across tasks like recognizing occluded objects, enhancing image quality, and interpreting coarse sketches. Evaluate 22 MLLMs, analyze correlations with model capacity, test "hinting" strategies, examine conversation history and in-context learning effects, and explore reinforcement learning fine-tuning.
...
Conclusion: ProactiveBench serves as a foundational benchmark for building proactive multimodal models, revealing current MLLM limitations in help-seeking behavior while demonstrating that proactiveness can be learned through targeted fine-tuning approaches.
...
TL;DR: NA-VQA is a benchmark for evaluating deep temporal and narrative reasoning in long-form videos using 88 movies and 4.4K open-ended QA pairs with varying evidence distances, showing current MLLMs struggle with far-range dependencies and proposing Video-NaRA framework for narrative modeling.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal large language model benchmarks for long-video reasoning focus on localized cues and fail to capture narrative reasoning - the ability to track intentions, connect distant events, and reconstruct causal chains across entire movies.

Method: Introduces NA-VQA benchmark with 88 full-length movies and 4.4K open-ended question-answer pairs grounded in multiple evidence spans (Short, Medium, Far distances). Proposes Video-NaRA framework that builds event-level chains and stores them in structured memory for retrieval during reasoning.

Result: State-of-the-art MLLMs perform poorly on questions requiring far-range evidence, highlighting the need for explicit narrative modeling. Video-NaRA improves long-range reasoning performance by up to 3% compared to existing approaches.

Conclusion: NA-VQA addresses the gap in evaluating deep narrative reasoning in long videos, demonstrating current models' limitations with long-range dependencies and showing that explicit narrative modeling (Video-NaRA) can improve performance on complex narrative structures.
...
TL;DR: Instruction-free tuning for medical LVLMs using momentum proxy instructions and response shuffling to reduce reliance on curated instruction datasets while maintaining instruction-following capabilities.


<details>
  <summary>Details</summary>
Motivation: Medical domain faces challenges in constructing large-scale, high-quality instruction datasets due to specialized expert knowledge requirements, creating a need for approaches that reduce dependency on handcrafted instructions.

Method: Proposes instruction-free tuning using momentum proxy instructions as replacements for curated text instructions, preserving pre-trained LVLM instruction-following capabilities while enabling parameter updates valid during inference. Incorporates response shuffling strategy to mitigate over-reliance on previous words.

Result: Achieves state-of-the-art accuracy on multiple-choice visual question answering tasks across SKINCON, WBCAtt, CBIS, and MIMIC-CXR datasets, significantly enhancing fine-tuning efficiency of LVLMs in medical domains.

Conclusion: Instruction-free tuning with momentum proxy instructions and response shuffling enables effective medical LVLM fine-tuning without explicit instruction datasets, addressing domain-specific data scarcity challenges while maintaining model capabilities.
...
TL;DR: VeloxNet is a lightweight CNN that replaces SqueezeNet's fire modules with gMLP blocks featuring spatial gating units, achieving better accuracy with 46.1% fewer parameters for embedded aerial image classification.


<details>
  <summary>Details</summary>
Motivation: Deep learning deployment on embedded devices for aerial disaster monitoring and infrastructure inspection requires balancing accuracy with strict constraints on model size, memory, and latency.

Method: Introduces VeloxNet, a lightweight CNN architecture that replaces SqueezeNet's fire modules with gated multi-layer perceptron (gMLP) blocks. Each gMLP block uses a spatial gating unit (SGU) that applies learned spatial projections and multiplicative gating to capture spatial dependencies across full feature maps in single layers.

Result: VeloxNet reduces parameter count by 46.1% relative to SqueezeNet (from 740,970 to 399,366) while improving weighted F1 scores: 6.32% on AIDER, 30.83% on CDD, and 2.51% on LDD. Outperforms eleven baselines including MobileNet variants, ShuffleNet, EfficientNet, and recent vision transformers.

Conclusion: Substituting local convolutional modules with spatial gating blocks improves both classification accuracy and parameter efficiency for resource-constrained deployment, demonstrating the effectiveness of global spatial modeling over local receptive fields.
...
TL;DR: ViTRM is a parameter-efficient vision architecture that replaces multi-layer ViT encoders with a single tiny 3-layer block applied recursively, achieving competitive performance with 6-84× fewer parameters than CNN/ViT models.


<details>
  <summary>Details</summary>
Motivation: Current deep vision models (CNNs, ViTs) are parameter-intensive and computationally demanding, limiting deployment in resource-constrained environments. There's a need for parameter-efficient alternatives that maintain performance.
...
Result: ViTRM maintains competitive performance on CIFAR-10 and CIFAR-100 despite using up to 6× fewer parameters than CNN-based models and 84× fewer parameters than ViT models.
...
TL;DR: FedAgain is a trust-based federated learning strategy that enhances robustness for kidney stone identification by using dual trust mechanisms to weight client contributions, outperforming standard FL baselines on non-IID and corrupted data.


<details>
  <summary>Details</summary>
Motivation: AI reliability in medical imaging depends on robustness to heterogeneous images from diverse devices across hospitals, which is challenging due to data privacy concerns and non-IID distributions.

Method: FedAgain integrates a dual trust mechanism combining benchmark reliability and model divergence to dynamically weight client contributions, mitigating noisy/adversarial updates during aggregation while preserving data privacy.

Result: Extensive experiments across five datasets (MNIST, CIFAR-10, two private kidney stone datasets, MyStone) show FedAgain consistently outperforms standard FL baselines under non-IID and corrupted-client scenarios.

Conclusion: FedAgain represents a practical advance toward reliable, privacy-preserving, and clinically deployable federated AI for medical imaging by maintaining diagnostic accuracy and performance stability under varying conditions.
...
TL;DR: Gastric-X is a large-scale multimodal benchmark for gastric cancer analysis with 1.7K cases containing CT scans, endoscopic images, biochemical indicators, diagnostic notes, and tumor annotations, used to evaluate VLMs on five clinical tasks.
...
Method: Introduce Gastric-X benchmark with 1.7K cases containing paired resting/dynamic CT scans, endoscopic images, structured biochemical indicators, expert diagnostic notes, and tumor bounding box annotations. Systematically evaluate recent VLMs on five core tasks: Visual Question Answering, report generation, cross-modal retrieval, disease classification, and lesion localization.

Result: The benchmark enables systematic examination of VLM capabilities on clinical tasks simulating critical stages of clinical workflow, from visual understanding to multimodal decision support, and probes whether VLMs can meaningfully correlate biochemical signals with spatial tumor features and textual reports.

Conclusion: Gastric-X represents a step toward aligning machine intelligence with physicians' cognitive and evidential reasoning processes, serving as a resource to inspire development of next-generation medical VLMs for gastric cancer diagnosis.
...
TL;DR: ReXInTheWild is a benchmark of 955 clinician-verified multiple-choice questions across 7 clinical topics using 484 real-world medical photographs, evaluating vision-language models' ability to interpret everyday medical images.


<details>
  <summary>Details</summary>
Motivation: Everyday medical photographs are widely used in telemedicine but lack comprehensive benchmarks to evaluate vision-language models' ability to interpret their medical content, which requires both fine-grained natural image understanding and domain-specific medical reasoning.

Method: Created ReXInTheWild benchmark with 955 clinician-verified multiple-choice questions spanning 7 clinical topics across 484 photographs sourced from biomedical literature, then evaluated leading multimodal large language models on this benchmark.

Result: Substantial performance variation among models: Gemini-3 achieved 78% accuracy, Claude Opus 4.5 (72%), GPT-5 (68%), while medical specialist model MedGemma reached only 37%. Error analysis revealed four categories of common errors requiring different mitigation strategies.

Conclusion: ReXInTheWild provides a challenging, clinically grounded benchmark at the intersection of natural image understanding and medical reasoning, highlighting current limitations of vision-language models in interpreting real-world medical photographs.
...
TL;DR: The paper introduces FS23K, a large-scale BSL fingerspelling dataset created via iterative annotation, and proposes a recognition model incorporating bi-manual interactions and mouthing cues, achieving 50% CER reduction over prior state-of-the-art.


<details>
  <summary>Details</summary>
Motivation: Fingerspelling is essential in BSL for proper names, technical terms, and words without lexical signs, but recognition is challenging due to rapid signing and letter omissions. Existing BSL fingerspelling datasets are either small or inaccurate in temporal and letter-wise annotations.

Method: 1) Introduces FS23K, a new large-scale BSL fingerspelling dataset constructed using an iterative annotation framework; 2) Proposes a fingerspelling recognition model that explicitly accounts for bi-manual interactions and mouthing cues.

Result: With refined annotations, the approach halves the character error rate (CER) compared to the prior state of the art on fingerspelling recognition.

Conclusion: The findings demonstrate the effectiveness of the method and highlight its potential to support future research in sign language understanding and scalable, automated annotation pipelines.
...
TL;DR: SurfaceXR combines headset hand tracking with smartwatch IMU data to enable robust surface-based interactions in XR, addressing fatigue and imprecision issues of mid-air gestures.


<details>
  <summary>Details</summary>
Motivation: Mid-air gestures in XR cause fatigue and imprecision, while surface-based interactions offer better accuracy and comfort. Current egocentric vision methods fail due to hand tracking challenges and unreliable surface plane estimation.
...
Conclusion: SurfaceXR successfully addresses the limitations of mid-air gestures and current vision-based methods by leveraging complementary sensor modalities to enable accurate, comfortable surface-based interactions in XR environments.
...
TL;DR: dinov3.seg extends dinov3.txt into a dedicated OVSS framework with task-specific architecture, joint text embeddings, early/late feature refinement, and high-resolution inference, outperforming SOTA on five benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current VLMs have suboptimal representations for dense prediction due to global contrastive objectives, leading to limited spatial precision and robustness in cluttered scenes for OVSS. Existing methods rely too heavily on post-hoc similarity refinement.

Method: Four key contributions: 1) Task-specific architecture adapting prior OVSS design principles; 2) Joint text embeddings aligned with both global [CLS] token and local patch-level features; 3) Early refinement of visual representations before image-text interaction plus late refinement of correlation features; 4) High-resolution local-global inference via sliding-window aggregation.
...
Conclusion: dinov3.seg provides a comprehensive framework for OVSS that addresses limitations of current VLMs through systematic architectural design, refined feature processing, and advanced inference strategies, achieving superior performance on challenging benchmarks.
...
TL;DR: Lightweight, socially informed architecture for pedestrian intention prediction using four behavioral streams with uncertainty quantification, achieving state-of-the-art performance on PSI benchmarks while maintaining efficiency.


<details>
  <summary>Details</summary>
Motivation: Accurate pedestrian intention prediction is crucial for autonomous vehicle safety in urban environments, requiring models that are both precise and efficient while providing calibrated uncertainty estimates for risk-aware decision making.

Method: Proposes a lightweight architecture that fuses four behavioral streams (attention, position, situation, interaction) using highway encoders, a compact 4-token Transformer, and global self-attention pooling. Includes two uncertainty quantification heads: variational bottleneck for epistemic uncertainty and Mahalanobis distance detector for distributional shift detection.

Result: Outperforms recent vision language models on PSI 1.0 benchmark (0.9 F1, 0.94 AUC-ROC, 0.78 MCC). Establishes first baseline on PSI 2.0 dataset (0.78 F1, 0.79 AUC-ROC). Selective prediction improves test accuracy by 0.4 percentage points at 80% coverage. Model is modality-agnostic and suitable for resource-constrained platforms.

Conclusion: The proposed approach provides accurate, uncertainty-calibrated pedestrian intention prediction using interpretable features, offering both high performance and efficiency for integration into autonomous vehicle systems with risk-aware capabilities.
...
TL;DR: MoCA3D is a monocular, class-agnostic 3D model that predicts projected 3D bounding box corners and per-corner depths without requiring camera intrinsics at inference, enabling image-plane geometry understanding for downstream applications.


<details>
  <summary>Details</summary>
Motivation: Existing monocular 3D object understanding methods require known camera intrinsics to obtain image-plane geometry (projected 3D box corners), which limits applications in the wild where intrinsics are unknown. Downstream applications need image-plane geometry that cannot be easily obtained without intrinsics.

Method: MoCA3D formulates pixel-space localization and depth assignment as dense prediction via corner heatmaps and depth maps. It predicts projected 3D bounding box corners and per-corner depths without requiring camera intrinsics at inference time, using a class-agnostic approach.

Result: MoCA3D achieves state-of-the-art performance, improving image-plane corner PAG (Pixel-Aligned Geometry) by 22.8% while remaining comparable on 3D IoU. It uses up to 57 times fewer trainable parameters than existing methods.

Conclusion: MoCA3D enables practical downstream applications that were previously impractical under unknown camera intrinsics, demonstrating utility beyond standard baseline models for monocular 3D object understanding in the wild.
...
TL;DR: SeeClear improves monocular depth estimation for transparent objects by converting them into opaque shapes using diffusion-based generative opacification, enabling use of standard depth estimators without retraining.


<details>
  <summary>Details</summary>
Motivation: Transparent objects present unique challenges for monocular depth estimation due to refraction and transmission effects that violate appearance assumptions used by conventional depth networks, leading to unstable or incorrect predictions.

Method: The framework first localizes transparent regions, then uses a diffusion-based generative opacification module to transform refractive appearances into geometrically consistent opaque shapes, allowing processed images to be fed into off-the-shelf monocular depth estimators without architectural changes.

Result: Experiments on synthetic and real-world datasets demonstrate significant improvements in depth estimation for transparent objects compared to state-of-the-art methods.

Conclusion: SeeClear provides an effective solution for transparent object depth estimation by converting challenging transparent appearances into opaque representations compatible with existing depth estimation frameworks.
...
TL;DR: StreetForward is a pose-free, tracker-free feedforward framework for dynamic street scene reconstruction using 3D Gaussian Splatting with temporal mask attention for motion-aware representations, enabling novel view synthesis at new poses and times without per-scene optimization.


<details>
  <summary>Details</summary>
Motivation: Enable rapid scene reconstruction for autonomous driving applications to efficiently utilize large-scale driving datasets in closed-loop simulation and downstream tasks, eliminating time-consuming per-scene optimization while handling dynamic street scenes.

Method: Pose-free and tracker-free feedforward framework using temporal mask attention module (building on VGGT's alternating attention) to capture dynamic motion from image sequences, representing static and dynamic content uniformly with 3D Gaussian Splatting, optimized via cross-frame rendering with spatio-temporal consistency.

Result: Superior performance on novel view synthesis and depth estimation on Waymo Open Dataset, with zero-shot generalization to CARLA and other datasets, enabling inference of per-pixel velocities and high-fidelity novel views at new poses and times.

Conclusion: StreetForward demonstrates effective dynamic street reconstruction without pose/tracker requirements, enabling efficient utilization of driving datasets for simulation and downstream tasks through motion-aware representations and feedforward inference.
...
TL;DR: EvoNAS is an efficient distributed framework for multi-objective evolutionary neural architecture search that addresses expensive candidate evaluation and ranking inconsistency through a hybrid VSS-ViT supernet with cross-architecture knowledge distillation and distributed parallel evaluation.


<details>
  <summary>Details</summary>
Motivation: Large vision models have high inference costs that limit deployment on resource-constrained edge devices, while existing evolutionary neural architecture search suffers from expensive candidate evaluation and inconsistent ranking of subnetworks.

Method: Proposes EvoNAS with: 1) Hybrid VSS-ViT supernet combining Vision State Space and Vision Transformer modules, 2) Cross-Architecture Dual-Domain Knowledge Distillation (CA-DDKD) for improved representation and ranking consistency, 3) Distributed Multi-Model Parallel Evaluation (DMMPE) framework using GPU resource pooling and asynchronous scheduling for efficient validation.

Result: EvoNAS achieves over 70% efficiency improvement compared to conventional data-parallel evaluation. The searched EvoNet architectures achieve Pareto-optimal trade-offs between accuracy and efficiency on COCO, ADE20K, KITTI, and NYU-Depth v2, outperforming CNN-, ViT-, and Mamba-based models in inference latency and throughput while maintaining strong generalization on downstream tasks.

Conclusion: EvoNAS provides an effective solution for balancing predictive accuracy with real-time efficiency in computer vision, enabling practical deployment of optimized vision architectures on resource-constrained edge devices through efficient evolutionary search with improved ranking consistency and parallel evaluation.
...
TL;DR: Proposes Event Prompter for RGB-Event pedestrian attribute recognition, using lightweight DCT/IDCT for event feature extraction, associative memory with Hopfield networks, and cross-attention fusion to reduce computational overhead while leveraging contextual guidance.


<details>
  <summary>Details</summary>
Motivation: Existing two-stream multimodal fusion methods for event-based PAR introduce significant computational overhead and neglect valuable guidance from contextual samples, limiting practical deployment and performance.

Method: Event Prompter replaces auxiliary backbone with lightweight DCT/IDCT operations on event data for frequency-domain feature extraction; uses external memory bank with Hopfield networks for associative memory-augmented representation learning; employs cross-attention for RGB-event fusion and feed-forward networks for attribute prediction.

Result: Extensive experiments on multiple benchmark datasets fully validate the effectiveness of the proposed RGB-Event PAR framework, demonstrating improved performance with minimal computational overhead.

Conclusion: The proposed framework addresses computational inefficiency and contextual guidance limitations of existing methods, providing an effective solution for event-based pedestrian attribute recognition in challenging conditions.
...
TL;DR: ConvNeur is a two-branch vision architecture that decouples global reasoning (via lightweight neural memory) from local representation (via locality-preserving branch), achieving efficient subquadratic scaling while maintaining performance on vision tasks.


<details>
  <summary>Details</summary>
Motivation: Modern vision models need to capture both global image-level context and local details while remaining computationally efficient. Current approaches often struggle with the tradeoff between global reasoning and local representation, leading to either computational inefficiency or loss of fine-grained details.

Method: Introduces ConvNeur, a two-branch architecture: 1) Lightweight neural memory branch aggregates global context on a compact set of tokens, 2) Locality-preserving branch extracts fine structure. A learned gate allows global cues to modulate local features without entangling their objectives, achieving subquadratic scaling with image size.

Result: ConvNeur matches or surpasses comparable alternatives on standard classification, detection, and segmentation benchmarks at similar or lower computational cost. It offers favorable accuracy versus latency trade-offs at similar budgets and demonstrates that efficiency follows from global-local decoupling.

Conclusion: Decoupling global reasoning from local representation is an effective principle for efficient vision models. ConvNeur demonstrates that separating these roles yields computational efficiency (subquadratic scaling) while maintaining performance, supporting the view that efficiency follows from global-local decoupling.
...
TL;DR: Two-stage acceleration framework for diffusion-based image tokenization decoders: multi-scale sampling strategy plus distillation into single-step denoising models, achieving order-of-magnitude speedup with minimal quality degradation.
...
Method: Two-stage acceleration: 1) Multi-scale sampling strategy starting at coarse resolution and progressively doubling resolution at each stage (theoretical O(log n) speedup), 2) Distillation of diffusion decoder at each scale into single-step denoising models for fast single forward pass per scale.
...
Conclusion: The approach offers a foundation for future work in efficient visual tokenization and downstream generation by making diffusion-based decoders practical for real-time and large-scale applications.
...
TL;DR: CurveStream is a training-free, curvature-aware hierarchical visual memory management framework for streaming video understanding that addresses token explosion and catastrophic forgetting by identifying critical semantic transitions through geometric feature analysis.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs struggle with streaming videos due to linear explosion of visual tokens causing OOM errors and catastrophic forgetting. Existing methods use uniform sampling or low-level metrics lacking semantic awareness, which disrupts contextual coherence and misses critical semantic transitions.

Method: CurveStream uses curvature analysis of continuous feature trajectories to identify high-curvature regions aligned with critical semantic transitions. It computes real-time Curvature Scores, employs online K-Sigma dynamic thresholding, and adaptively routes frames into clear and fuzzy memory states under strict token budgets.

Result: CurveStream achieves over 10% absolute performance gains (10.69% on StreamingBench and 13.58% on OVOBench) over respective baselines, establishing new state-of-the-art results for streaming video perception across diverse temporal scales.

Conclusion: The curvature-aware hierarchical memory management framework effectively addresses streaming video challenges by preserving semantic coherence and critical transitions, demonstrating significant performance improvements without requiring training.
...
TL;DR: MagicSeg introduces a diffusion model-driven pipeline for automatically generating datasets for open-world semantic segmentation, eliminating the need for expensive pixel-level annotations by using text-to-image generation with counterfactual samples and self-supervised mask extraction.


<details>
  <summary>Details</summary>
Motivation: Open-world semantic segmentation requires extensive image-text pair datasets with fine-grained pixel annotations, which are economically prohibitive due to high human labor and time costs. Current approaches lack sufficient annotated data for diverse categories.

Method: MagicSeg uses diffusion models to generate datasets automatically: 1) Starts from class labels to generate textual descriptions, 2) Uses diffusion models to generate images from text, 3) Generates both positive and negative (counterfactual) image samples, 4) Integrates open-vocabulary detection and interactive segmentation models to extract object masks as pseudo labels, 5) Applies contrastive language-image pretraining with pseudo mask supervision and counterfactual contrastive training.

Result: Achieves state-of-the-art performance on PASCAL VOC (62.9%), PASCAL Context (26.7%), and COCO (40.2%) datasets, demonstrating the effectiveness of the automatically generated dataset for enhancing open-world semantic segmentation capabilities.

Conclusion: MagicSeg provides an effective pipeline for generating high-quality datasets for open-world semantic segmentation without expensive manual annotation, leveraging diffusion models for image generation and self-supervised techniques for mask extraction, resulting in strong downstream segmentation performance.
...
TL;DR: FlowScene is a tri-branch scene generative model that uses multimodal graphs to collaboratively generate scene layouts, object shapes, and object textures through a tight-coupled rectified flow model, achieving superior realism, style coherence, and controllability compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Current scene generation methods have limitations: language-driven retrieval methods lack object-level control and scene-level style coherence, while graph-based methods offer better controllability but struggle to produce high-fidelity textured results, limiting practical utility.

Method: FlowScene uses a tri-branch generative model conditioned on multimodal graphs with a tight-coupled rectified flow model that exchanges object information during generation. It collaboratively generates scene layouts, object shapes, and object textures through cross-branch information exchange.
...
Conclusion: FlowScene enables fine-grained control of objects' shapes, textures, and relations while enforcing scene-level style coherence across structure and appearance, addressing limitations of existing scene generation approaches.
...
TL;DR: K-GMRF is an online, training-free framework for covariance tracking that uses second-order forced rigid-body motion on Lie groups to achieve zero steady-state error under constant rotation, outperforming first-order methods.


<details>
  <summary>Details</summary>
Motivation: Existing covariance tracking estimators either ignore manifold constraints or rely on first-order updates that cause inevitable phase lag during rapid evolution, limiting their ability to track non-stationary covariance matrices accurately in vision applications.

Method: Reformulates covariance tracking as forced rigid-body motion on Lie groups using Euler-Poincaré equations, interpreting observations as torques driving latent angular velocity, and propagating via structure-preserving symplectic integrator.

Result: Theoretically proven zero steady-state error under constant rotation; experimentally shows 30x angular error reduction on synthetic ellipses, geodesic error reduction from 29.4° to 9.9° on SO(3) stabilization with dropout, and IOU improvement from 0.55 to 0.74 on OTB motion-blur sequences.

Conclusion: K-GMRF provides a robust, training-free covariance tracking framework with superior performance to first-order baselines, serving as both a plug-and-play geometric prior for data-constrained scenarios and an interpretable layer within deep architectures.
...
TL;DR: ChangeRWKV introduces a novel architecture for remote sensing change detection that combines Transformer-like parallel training with RNN-like linear-time inference, achieving state-of-the-art performance with significantly reduced computational cost.


<details>
  <summary>Details</summary>
Motivation: Existing methods face a trade-off: CNNs are efficient but lack global context, while Transformers capture long-range dependencies but at prohibitive computational cost. There's a need for an architecture that reconciles this conflict for operational-scale change detection.

Method: ChangeRWKV builds upon the Receptance Weighted Key Value (RWKV) framework, featuring: 1) a hierarchical RWKV encoder for multi-resolution feature representation, and 2) a Spatial-Temporal Fusion Module (STFM) to resolve spatial misalignments across scales while distilling fine-grained temporal discrepancies.
...
Conclusion: ChangeRWKV demonstrates a new, efficient, and powerful paradigm for operational-scale change detection that combines the best of both CNNs and Transformers without their respective limitations.
...
TL;DR: Physion-Eval is a benchmark for evaluating physical realism in video generation models using expert human reasoning across 22 physical categories, revealing that 83.3-93.5% of generated videos contain physical glitches.


<details>
  <summary>Details</summary>
Motivation: Current video generation models are increasingly used as world simulators, but existing evaluation methods (automated metrics or coarse human judgments) provide limited insight into when and why generated dynamics violate real-world physical constraints.

Method: Created a large-scale benchmark with 10,990 expert reasoning traces across 22 fine-grained physical categories. Each generated video is derived from a real-world reference video depicting clear physical processes, annotated with temporally localized glitches, structured failure categories, and natural-language explanations of violated physical behavior.

Result: Revealed striking limitation: in physics-critical scenarios, 83.3% of exocentric and 93.5% of egocentric generated videos exhibit at least one human-identifiable physical glitch. The benchmark covers five state-of-the-art models across both egocentric and exocentric views.

Conclusion: Physion-Eval sets a new standard for physical realism evaluation and aims to guide the development of physics-grounded video generation. The benchmark is publicly available for community use.
...
TL;DR: FB-CLIP enhances zero-shot fine-grained anomaly detection by addressing CLIP's limitations through multi-strategy textual representations and foreground-background separation with semantic consistency regularization.


<details>
  <summary>Details</summary>
Motivation: Fine-grained anomaly detection is critical for industrial and medical applications, but labeled anomalies are scarce. Existing vision-language models like CLIP struggle with foreground-background feature entanglement and coarse textual semantics, limiting their effectiveness in zero-shot anomaly detection scenarios.

Method: FB-CLIP combines multi-strategy textual representations (End-of-Text features, global-pooled representations, attention-weighted token features) with multi-view soft separation in visual modality (identity, semantic, spatial dimensions) and background suppression. It introduces Semantic Consistency Regularization (SCR) to align image features with normal/abnormal textual prototypes while suppressing uncertain matches and enlarging semantic gaps.

Result: FB-CLIP effectively distinguishes anomalies from complex backgrounds and achieves accurate fine-grained anomaly detection and localization under zero-shot settings, outperforming existing approaches.

Conclusion: The proposed FB-CLIP framework successfully addresses CLIP's limitations for zero-shot fine-grained anomaly detection through enhanced textual representations, foreground-background separation, and semantic consistency regularization, demonstrating strong performance in challenging industrial and medical applications.
...
TL;DR: LoD-Loc v3 improves aerial visual localization in dense urban areas using instance segmentation and synthetic data, achieving better cross-scene generalization and dense scene performance than previous methods.


<details>
  <summary>Details</summary>
Motivation: Previous LoD-Loc v2 method has poor cross-scene generalization and frequent failures in dense building scenes due to semantic building silhouette alignment with low-detail city models.

Method: Two key innovations: 1) Synthetic data generation pipeline creating InsLoD-Loc - largest instance segmentation dataset for aerial imagery (100k images with precise building annotations); 2) Reformulating localization paradigm from semantic to instance silhouette alignment to reduce pose estimation ambiguity.

Result: Extensive experiments show LoD-Loc v3 outperforms existing SOTA baselines with large margins in both cross-scene and dense urban scenarios, demonstrating remarkable zero-shot generalization capability.

Conclusion: LoD-Loc v3 successfully addresses limitations of prior work through instance segmentation and synthetic data, achieving superior aerial visual localization performance in challenging dense urban environments.
...
TL;DR: ParallelVLM is a training-free speculative decoding framework that accelerates Video-LLMs by using parallel draft-then-verify stages with unbiased pruning, achieving 2.4-3.4× speedup over autoregressive decoding.


<details>
  <summary>Details</summary>
Motivation: Current Video-LLMs suffer from inefficient autoregressive decoding due to massive video tokens, and existing visual token pruning approaches cause information loss with only modest acceleration.

Method: Proposes ParallelVLM with two parallelized stages: 1) training-free draft-then-verify speculative decoding framework, and 2) Unbiased Verifier-Guided Pruning strategy to eliminate positional bias in attention-guided pruning for better draft-target model alignment.

Result: Expands draft window by 1.6-1.8× with high accepted lengths, accelerates video understanding benchmarks by 3.36× on LLaVA-Onevision-72B and 2.42× on Qwen2.5-VL-32B compared to vanilla autoregressive decoding.

Conclusion: ParallelVLM effectively overcomes mutual waiting and limited speedup problems in long-video settings, maximizing hardware utilization while maintaining high decoding quality through unbiased pruning.
...
<details>
  <summary>Details</summary>
Motivation: Existing NVS methods struggle with synthesizing plausible views for unobserved regions, especially under single-view input, and face challenges in maintaining geometry- and appearance-consistency across generated views.

Method: Reformulates NVS as orbit video generation, adapting a pre-trained video generation model with: 1) camera adapters for accurate camera control, 2) normal map generation branch with attention guidance for geometric consistency, and 3) pixel-space supervision to alleviate blurry appearance from spatial compression.
...
Conclusion: OrbitNVS effectively addresses key challenges in NVS by leveraging video generation priors and incorporating geometric guidance mechanisms, achieving state-of-the-art performance particularly in single-view scenarios.
...
TL;DR: UniPR is an end-to-end object-level real-to-sim perception and reconstruction framework that processes single stereo image pairs to simultaneously reconstruct all objects in a scene with proper scale and pose, eliminating modular pipeline inefficiencies.


<details>
  <summary>Details</summary>
Motivation: Existing real-to-sim transfer methods use modular pipelines (detection, segmentation, shape reconstruction, pose estimation) that suffer from inefficiency and cumulative errors due to operating on partial information and discarding global context.

Method: UniPR operates directly on single stereo image pairs, leverages geometric constraints to resolve scale ambiguity, introduces Pose-Aware Shape Representation to eliminate per-category canonical definitions and bridge reconstruction-pose estimation gap, and is trained on LVS6D dataset with over 6,300 objects.

Result: UniPR reconstructs all objects in a scene in parallel within a single forward pass, achieving significant efficiency gains while preserving true physical proportions across diverse object types.

Conclusion: UniPR demonstrates potential for practical robotic applications by providing an efficient, end-to-end solution for real-to-sim perception and reconstruction that overcomes limitations of modular pipelines.
...
TL;DR: HRNet: Hybrid Registration Network for multimodal image registration that jointly learns stable shared features and unified hybrid transformations to handle both global rigid and local non-rigid deformations.


<details>
  <summary>Details</summary>
Motivation: Address two key limitations in multimodal registration: (1) modality-private cues leaking into shared feature space despite disentanglement methods, and (2) most multi-scale frameworks supporting only single transformation types, limiting applicability when both global misalignment and local deformation coexist.

Method: Proposes HRNet with shared backbone using Modality-Specific Batch Normalization (MSBN) for multi-scale feature extraction, Cross-scale Disentanglement and Adaptive Projection (CDAP) module to suppress modality-private cues and project shared features into stable subspace, and Hybrid Parameter Prediction Module (HPPM) for non-iterative coarse-to-fine estimation of global rigid parameters and deformation fields fused into coherent deformation field.
...
Conclusion: HRNet effectively addresses hybrid multimodal registration by jointly learning stable shared feature space and unified hybrid transformation, outperforming existing methods and providing a comprehensive solution for multimodal registration challenges.
...
<details>
  <summary>Details</summary>
Motivation: Existing relative pose estimation methods face a fundamental trade-off: feature-matching pipelines with RANSAC achieve high accuracy but lack end-to-end differentiability, while ViT-based regressors are differentiable but too computationally expensive for real-time deployment. The core bottlenecks are the coupling between rotation and translation estimation and insufficient cross-view feature alignment.

Method: IUP-Pose uses a lightweight Multi-Head Bi-Cross Attention (MHBC) module for implicit dense alignment of cross-view features without explicit matching supervision. The framework employs a decoupled pipeline: two shared-parameter rotation stages iteratively refine rotation with uncertainty estimation, then feature maps are realigned via rotational homography H_inf before translation prediction, enabling end-to-end differentiability.

Result: The method achieves 73.3% AUC@20deg on MegaDepth1500 benchmark while maintaining full end-to-end differentiability. It achieves 70 FPS throughput with only 37M parameters, demonstrating excellent accuracy-efficiency trade-off suitable for real-time edge deployment.

Conclusion: IUP-Pose successfully addresses the accuracy-efficiency trade-off in relative pose estimation by combining geometry-driven decoupled iterative refinement with implicit dense alignment, enabling both high accuracy and real-time performance with end-to-end differentiability for practical deployment scenarios.
...
TL;DR: DPTracker introduces a dual prompt-driven feature encoding method for nighttime UAV tracking, using pyramid illumination prompts and dynamic viewpoint prompts to address illumination and viewpoint challenges.


<details>
  <summary>Details</summary>
Motivation: Existing UAV tracking feature encoding methods overlook critical illumination and viewpoint cues essential for robust perception under challenging nighttime conditions, leading to degraded tracking performance.

Method: Proposes a dual prompt-driven feature encoding method with two components: 1) Pyramid illumination prompter extracts multi-scale frequency-aware illumination prompts, 2) Dynamic viewpoint prompter modulates deformable convolution offsets to accommodate viewpoint variations for view-invariant features.

Result: Extensive experiments validate DPTracker's effectiveness in nighttime UAV tracking. Ablation studies confirm each component's contribution, and real-world tests demonstrate robustness across diverse nighttime scenarios.

Conclusion: The dual prompt-driven approach enables domain-invariant feature encoding for reliable nighttime UAV tracking by addressing illumination and viewpoint challenges through prompt-conditioned adaptation and context-aware evolution.
...
TL;DR: UniBioTransfer is a unified framework for multiple deepface generation tasks including face/hair transfer and reenactment, addressing data scarcity and cross-task conflicts through a swapping-based data construction and BioMoE architecture.
...
Method: Proposes UniBioTransfer with unified data construction strategy including swapping-based corruption for dynamic attributes, and BioMoE (mixture-of-experts) architecture with two-stage training to disentangle task-specific knowledge.

Result: Extensive experiments show UniBioTransfer outperforms both existing unified models and task-specific methods across various deepface generation tasks, demonstrating effectiveness, generalization, and scalability.

Conclusion: UniBioTransfer successfully addresses multi-task deepface generation challenges, enabling handling of both conventional and shape-varying transformations while naturally generalizing to unseen tasks with minimal fine-tuning.
...
TL;DR: OmniDiT is a unified diffusion transformer framework for virtual try-on and try-off that addresses detail preservation, generalization, and efficiency challenges through self-evolving data curation, adaptive position encoding, shifted window attention, and alignment loss.
...
Method: 1) Self-evolving data curation pipeline creating Omni-TryOn dataset (380k+ garment-model-tryon pairs with text prompts); 2) Token concatenation with adaptive position encoding for multiple reference conditions; 3) First introduction of Shifted Window Attention into diffusion models for linear complexity; 4) Multiple timestep prediction and alignment loss to improve generation fidelity.
...
Conclusion: OmniDiT successfully addresses key challenges in virtual try-on through a unified diffusion transformer framework with efficient attention mechanisms, comprehensive dataset curation, and improved generation fidelity techniques.
...
TL;DR: GravCal is a feedforward model that calibrates noisy gravity priors from single RGB images using a fusion of residual correction and image-based estimation with learned gating.


<details>
  <summary>Details</summary>
Motivation: Gravity estimation is crucial for visual-inertial systems, but IMU-based gravity priors are unreliable under linear acceleration, vibration, and transient motion. Existing methods either estimate gravity directly from images or assume accurate inertial input, leaving the practical problem of correcting noisy gravity priors from single images largely unaddressed.

Method: GravCal is a feedforward model that takes one RGB image and a noisy gravity prior as input. It combines two complementary predictions: (1) a residual correction of the input prior, and (2) a prior-independent image estimate. The model uses a learned gate to adaptively fuse these predictions based on prior quality.

Result: Extensive experiments show GravCal reduces mean angular error from 22.02° (raw IMU prior) to 14.24°, with larger improvements when the prior is severely corrupted. The model also introduces a novel dataset of over 148K frames with paired VIO-derived ground-truth gravity and Mahony-filter IMU priors across diverse scenes and arbitrary camera orientations.

Conclusion: GravCal effectively calibrates noisy gravity priors from single images, outperforming raw inertial priors. The learned gate correlates with prior quality, providing a useful confidence signal for downstream systems. The approach addresses a practical gap in gravity estimation for visual-inertial applications.
...
TL;DR: SAVN-CE introduces continuous audio-visual navigation with intermittent sound targets, addressed by MAGNet transformer model with memory-augmented goal reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing audio-visual navigation approaches rely on precomputed RIRs and discrete grid positions, creating unrealistic spatially discontinuous observations. Real-world scenarios involve continuous movement and intermittent sound targets where agents may lose goal information when sounds stop.
...
Conclusion: SAVN-CE establishes a more realistic continuous audio-visual navigation setting, and MAGNet effectively addresses the challenge of intermittent sound targets through multimodal encoding and memory-augmented reasoning, advancing embodied AI capabilities.
...
TL;DR: JMVR is a novel EEG-based visual reconstruction framework that treats EEG and text as independent modalities for joint learning, preserving EEG-specific spatial and chromatic details through multi-scale encoding and image augmentation, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Current EEG-based visual reconstruction approaches are deeply coupled with alignment frameworks that force EEG features to align with text or image semantic representations, which may condense the rich spatial and chromatic details in EEG signals, resulting in mere conditioned image generation rather than high-fidelity visual reconstruction.

Method: Proposes a Joint-Modal Visual Reconstruction (JMVR) framework that treats EEG and text as independent modalities for joint learning to preserve EEG-specific information. It employs a multi-scale EEG encoding strategy to capture both fine- and coarse-grained features, alongside image augmentation to enhance the recovery of perceptual details.

Result: Extensive experiments on the THINGS-EEG dataset demonstrate that JMVR achieves state-of-the-art performance against six baseline methods, specifically exhibiting superior capabilities in modeling spatial structure and chromatic fidelity.

Conclusion: The JMVR framework effectively addresses the limitations of current alignment-based approaches by preserving EEG-specific spatial and chromatic information through joint-modal learning, multi-scale encoding, and image augmentation, enabling high-fidelity visual reconstruction from EEG signals.
...
TL;DR: The paper proposes optimizing user-provided bounding boxes to better align with video diffusion model attention maps, improving generation quality and control adherence through differentiable bounding box adjustments.


<details>
  <summary>Details</summary>
Motivation: Current text-to-video diffusion models struggle with enforcing adherence to bounding box or layout control inputs, limiting their practical utility for controlled video generation.

Method: Proposes optimizing bounding boxes by aligning them with the model's internal attention maps using a differentiable smooth mask and attention-maximization objective, carefully balancing foreground and background focus.

Result: Small modifications to bounding boxes significantly improve generation quality and control adherence, validated through thorough experiments and user studies.

Conclusion: Optimizing bounding boxes to match model attention patterns is an effective approach for improving controlled video generation, with code released to foster community research.
...
TL;DR: DynFlowDrive is a latent world model for autonomous driving that uses flow-based dynamics to predict scene evolution under different driving actions, with a stability-aware trajectory selection strategy.


<details>
  <summary>Details</summary>
Motivation: Existing world models in autonomous driving use appearance generation or deterministic regression, which fail to capture trajectory-conditioned scene evolution and lead to unreliable action planning.

Method: Proposes DynFlowDrive, a latent world model using rectified flow formulation to learn a velocity field describing scene state changes under different driving actions, enabling progressive future latent state prediction. Includes stability-aware multi-mode trajectory selection that evaluates candidate trajectories based on induced scene transition stability.

Result: Extensive experiments on nuScenes and NavSim benchmarks show consistent improvements across diverse driving frameworks without additional inference overhead.

Conclusion: DynFlowDrive effectively models trajectory-conditioned scene evolution and improves planning reliability in autonomous driving systems through flow-based dynamics and stability-aware trajectory selection.
...
<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models achieve high visual quality but systematically fail at numerical control when prompts specify explicit object counts, limiting their practical utility for precise image generation tasks.

Method: ATHENA leverages intermediate representations during sampling to estimate object counts and applies count-aware noise corrections early in denoising. It offers three variants with increasing complexity: static prompt-based steering, dynamic count-aware control, and more advanced adaptive mechanisms.

Result: Experiments show ATHENA consistently improves count fidelity, especially at higher target counts, while maintaining favorable accuracy-runtime trade-offs across multiple diffusion backbones on established benchmarks and a new complex dataset.

Conclusion: ATHENA provides an effective model-agnostic solution for improving numerical control in diffusion models through test-time adaptive steering, addressing a fundamental limitation without requiring architectural modifications or retraining.
...
TL;DR: VLADR introduces a Vision-Language Attribute Disentanglement and Reinforcement approach for Lifelong Person Re-ID, leveraging VLM's generalizable knowledge through explicit modeling of universally shared human attributes to improve inter-domain knowledge transfer and mitigate forgetting.


<details>
  <summary>Details</summary>
Motivation: Existing LReID methods focus on learning from scratch or visual classification-pretrained models, while Vision-Language Models (VLMs) offer generalizable knowledge but are underutilized for fine-grained attribute learning, limiting acquisition and anti-forgetting capacity.

Method: VLADR includes: 1) Multi-grain Text Attribute Disentanglement mechanism to mine global and diverse local text attributes; 2) Inter-domain Cross-modal Attribute Reinforcement scheme with cross-modal attribute alignment for visual attribute extraction and inter-domain attribute alignment for fine-grained knowledge transfer.
...
Conclusion: Explicit modeling of universally shared human attributes through VLM-driven approach effectively improves inter-domain knowledge transfer, enhancing both new knowledge acquisition and forgetting mitigation in lifelong person re-identification.
...
TL;DR: UDML is an Unbiased Dynamic Multimodal Learning framework that addresses limitations in existing dynamic fusion methods by introducing noise-aware uncertainty estimation and modality reliance bias compensation.


<details>
  <summary>Details</summary>
Motivation: Traditional multimodal methods assume static modality quality, while existing dynamic methods rely on empirical metrics that fail at extreme noise levels and ignore intrinsic modality dependency bias, causing hard-to-learn modalities to be doubly penalized.

Method: Proposes UDML with two key components: 1) Noise-aware uncertainty estimator that adds controlled noise to modality data and predicts noise intensity from features to establish clear corruption-noise correspondence; 2) Quantification of inherent modality reliance bias via modality dropout and incorporation into weighting mechanism to eliminate dual suppression effect.

Result: Extensive experiments across diverse multimodal benchmark tasks validate the effectiveness, versatility, and generalizability of UDML, with code publicly available.

Conclusion: UDML successfully addresses key limitations in dynamic multimodal fusion by enabling accurate uncertainty measurement across noise conditions and eliminating bias against hard-to-learn modalities, outperforming static fusion approaches.
...
TL;DR: The paper proposes a self-constrained prior method to improve 3D Gaussian Splatting (3DGS) for more accurate depth rendering and surface reconstruction by using TSDF grids derived from rendered depth maps to constrain Gaussian placement and properties.


<details>
  <summary>Details</summary>
Motivation: While 3DGS has advantages over NeRF in rendering quality and speed, there is still room for improvement in recovering high-fidelity surfaces. The authors aim to address the limitation of 3DGS in producing accurate depth rendering and surface reconstruction.

Method: Proposes a self-constrained prior derived from a TSDF grid obtained by fusing depth maps rendered with current 3D Gaussians. The prior creates a distance field band around estimated surfaces to impose specific constraints: removing Gaussians outside the band, moving Gaussians closer to surfaces, and adjusting opacity in geometry-aware ways. The prior is regularly updated with more accurate recent depth images and can progressively narrow the band to tighten constraints.

Result: The method demonstrates superiority over state-of-the-art methods in evaluations on widely used benchmarks, justifying the proposed approach for improved surface reconstruction and depth rendering.

Conclusion: The self-constrained prior effectively improves 3DGS for surface reconstruction by providing geometry-aware constraints that can be progressively refined, leading to more accurate depth rendering and higher fidelity surfaces compared to existing methods.
...
TL;DR: TSegAgent: A zero-shot geometric reasoning approach for tooth segmentation and identification from 3D dental scans, eliminating need for task-specific training and dense annotations.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on task-specific 3D neural networks requiring densely annotated datasets, resulting in high annotation costs and limited generalization to unseen scan sources.

Method: Reformulates dental analysis as zero-shot geometric reasoning problem by combining general-purpose foundation models with explicit geometric inductive biases from dental anatomy. Uses multi-view visual abstraction and geometry-grounded reasoning with structural constraints like dental arch organization and volumetric relationships.

Result: Enables accurate and reliable tooth segmentation and identification with low computational and annotation cost, while exhibiting strong generalization across diverse and previously unseen dental scans.

Conclusion: Reasoning-oriented formulation with geometric inductive biases provides effective alternative to purely data-driven approaches, addressing annotation cost and generalization limitations in digital dentistry.
...
TL;DR: AI framework combining self-supervised anomaly detection with demographic-aware learning improves rare cardiac anomaly detection from ECGs while reducing performance disparities across populations.


<details>
  <summary>Details</summary>
Motivation: Rare cardiac anomalies are difficult to detect from ECGs due to long-tailed distribution with limited case counts and demographic disparities in diagnostic performance, leading to delayed recognition and uneven quality of care.

Method: Two-stage framework: 1) Self-supervised anomaly detection pretraining reconstructing masked global/local ECG signals, modeling trends, and predicting patient attributes; 2) Fine-tuning for multi-label ECG classification using asymmetric loss for long-tail abnormalities, with anomaly score maps for localization and CPU-based optimization.

Result: Achieves 94.7% AUROC for rare anomalies, reduces common-rare performance gap by 73%, maintains consistent diagnostic accuracy across age and sex groups on longitudinal cohort of over 1 million clinical ECGs.

Conclusion: Equity-aware AI framework demonstrates strong clinical utility, interpretable anomaly localization, and scalable performance across multiple cohorts, with potential to mitigate diagnostic disparities in biomedical signals and digital health.
...
<details>
  <summary>Details</summary>
Motivation: To investigate whether 2D foundation image models inherently possess 3D world model capabilities, despite being trained primarily on 2D data, and to explore their potential for 3D world synthesis.

Method: Proposes an agentic multi-architecture approach with three components: 1) VLM-based director that formulates prompts for image synthesis, 2) generator that synthesizes new image views, and 3) VLM-backed two-step verifier that evaluates and curates generated frames from both 2D image and 3D reconstruction perspectives.

Result: Demonstrates that 2D foundation models do encapsulate 3D world understanding, and the agentic approach successfully produces coherent, robust 3D reconstructions with expansive, realistic, and 3D-consistent worlds that can be explored through novel view rendering.

Conclusion: 2D foundation image models inherently possess 3D world model capabilities, and these can be effectively harnessed through an agentic framework to generate high-quality 3D-consistent worlds, bridging the gap between 2D image generation and 3D world synthesis.
...
### [54] [BALM: A Model-Agnostic Framework for Balanced Multimodal Learning under Imbalanced Missing Rates](https://arxiv.org/abs/2603.19718)
*Phuong-Anh Nguyen,Tien Anh Pham,Duc-Trong Le,Cam-Van Thi Nguyen*

Main category: cs.CV

TL;DR: BALM is a plug-in framework for balanced multimodal learning under imbalanced missing rates, using feature calibration and gradient rebalancing modules to improve robustness in multimodal emotion recognition.


<details>
  <summary>Details</summary>
Motivation: Multimodal learning suffers from imbalance where information-rich modalities dominate optimization, especially in realistic settings with imbalanced missing rates (IMR) where each modality is absent with different probabilities, distorting representation learning and gradient dynamics.

Method: BALM comprises two complementary modules: 1) Feature Calibration Module (FCM) recalibrates unimodal features using global context to establish shared representation basis across heterogeneous missing patterns; 2) Gradient Rebalancing Module (GRM) balances learning dynamics across modalities by modulating gradient magnitudes and directions from both distributional and spatial perspectives.
...
Conclusion: BALM is a model-agnostic plug-in framework that can be seamlessly integrated into diverse backbones without altering their architectures, effectively addressing multimodal imbalance under imbalanced missing rates.
...
TL;DR: PerformRecast: A method for expression-only portrait video editing that disentangles facial expression from head pose using 3DMM parameter consistency and facial/non-facial region decoupling for better control and quality.


<details>
  <summary>Details</summary>
Motivation: Existing portrait animation methods struggle to disentangle facial expression from head pose rotation, limiting independent expression editing capabilities needed for film and animation production.

Method: Improves keypoints transformation formula to align with 3DMM parameter separation (identity, expression, pose), and decouples facial/non-facial regions with separate teacher model supervision to avoid boundary misalignment.

Result: Produces high-quality expression-only edited videos that are more faithful to driving videos, outperforming existing methods in both controllability and efficiency.

Conclusion: PerformRecast enables versatile expression-only video editing with fine-grained control, addressing limitations of previous portrait animation approaches through 3DMM consistency and region-aware supervision.
...
### [56] [PhysNeXt: Next-Generation Dual-Branch Structured Attention Fusion Network for Remote Photoplethysmography Measurement](https://arxiv.org/abs/2603.19752)
*Junzhe Cao,Bo Zhao,Zhiyi Niu,Dan Guo,Yue Sun,Haochen Liang,Yong Xu,Zitong YU*

Main category: cs.CV

TL;DR: PhysNeXt is a dual-input deep learning framework that combines raw video frames and spatial-temporal map representations for robust remote photoplethysmography, achieving more stable heart rate signal recovery under challenging conditions.


<details>
  <summary>Details</summary>
Motivation: Current rPPG methods have limitations: end-to-end video modeling preserves complete spatiotemporal information but introduces substantial noise from motion artifacts and illumination variations, while STMap representations reduce data volume and computational complexity but may lose high-frequency details. There's a need to integrate the strengths of both approaches.

Method: PhysNeXt is a dual-input deep learning framework that jointly exploits video frames and STMap representations. It incorporates a spatio-temporal difference modeling unit, a cross-modal interaction module, and a structured attention-based decoder to collaboratively enhance pulse signal extraction robustness.

Result: Experimental results demonstrate that PhysNeXt achieves more stable and fine-grained rPPG signal recovery under challenging conditions, validating the effectiveness of joint modeling of video and STMap representations.

Conclusion: The proposed PhysNeXt framework effectively integrates the strengths of both raw video and STMap representations for remote photoplethysmography, providing more robust heart rate signal extraction. The codes will be released for community use.
...
### [57] [ReLi3D: Relightable Multi-view 3D Reconstruction with Disentangled Illumination](https://arxiv.org/abs/2603.19753)
*Jan-Niklas Dihlmann,Mark Boss,Simon Donne,Andreas Engelhardt,Hendrik P. A. Lensch,Varun Jampani*

Main category: cs.CV

TL;DR: ReLi3D is a unified end-to-end pipeline that simultaneously reconstructs 3D geometry, spatially-varying physically-based materials, and environment illumination from sparse multi-view images in under one second.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D asset reconstruction requires separate pipelines for geometry, materials, and illumination, each with distinct limitations and computational overhead. There's a need for a unified approach that can handle all these components simultaneously from sparse multi-view images.

Method: Uses transformer cross-conditioning architecture to fuse multi-view input, followed by a unified two-path prediction strategy: one path predicts object structure and appearance, the other predicts environment illumination. Combines with differentiable Monte Carlo multiple importance sampling renderer and mixed domain training protocol using both synthetic PBR datasets and real-world RGB captures.

Result: Achieves simultaneous reconstruction of complete 3D geometry, spatially-varying physically-based materials, and environment illumination from sparse multi-view images in under one second. Demonstrates generalizable results in geometry, material accuracy, and illumination quality.

Conclusion: ReLi3D unifies previously separate reconstruction tasks into a single feed-forward pass, enabling near-instantaneous generation of complete, relightable 3D assets by leveraging multi-view constraints for improved material and illumination disentanglement.
...
### [58] [Uncertainty-aware Prototype Learning with Variational Inference for Few-shot Point Cloud Segmentation](https://arxiv.org/abs/2603.19757)
...
<details>
  <summary>Details</summary>
Motivation: Existing prototype-based methods for few-shot 3D segmentation use deterministic prototypes that fail to capture uncertainty from scarce supervision, leading to degraded robustness and limited generalization.

Method: UPL introduces: 1) dual-stream prototype refinement module leveraging both support and query samples, and 2) probabilistic formulation treating prototypes as latent variables solved via variational inference for explicit uncertainty modeling.
...
Conclusion: UPL demonstrates that incorporating uncertainty modeling into prototype learning significantly improves few-shot 3D segmentation performance and provides interpretable predictions.
...
TL;DR: GNAP is a neural network training approach that dynamically adjusts network size through alternating growth and pruning phases to achieve high accuracy with minimal parameters.


<details>
  <summary>Details</summary>
Motivation: Traditional CNNs have fixed architectures and parameter counts, which may be inefficient. The goal is to create networks that can adapt their size during training to optimally fit data while minimizing parameter usage.

Method: GNAP uses two complementary mechanisms: growth phases where network size expands when performance saturates, and autonomous pruning phases where parameters are simultaneously trained and pruned via gradient descent. Networks start small and periodically grow to add expressive power.

Result: Achieved 99.44% accuracy on MNIST with only 6.2k parameters and 92.2% accuracy on CIFAR10 with 157.8k parameters, demonstrating the ability to train extremely sparse networks with high accuracy.

Conclusion: GNAP successfully creates adaptive neural networks that can dynamically adjust their size during training, achieving state-of-the-art accuracy with significantly fewer parameters than traditional approaches.
...
### [60] [PCSTracker: Long-Term Scene Flow Estimation for Point Cloud Sequences](https://arxiv.org/abs/2603.19762)
*Min Lin,Gangwei Xu,Xianqi Wang,Yuyi Peng,Xin Yang*

Main category: cs.CV

TL;DR: PCSTracker is the first end-to-end framework for consistent scene flow estimation in point cloud sequences, addressing temporal consistency issues in long sequences through iterative geometry-motion joint optimization, spatio-temporal trajectory updates, and sliding-window inference.
...
Method: Proposes PCSTracker with three key components: 1) Iterative Geometry Motion Joint Optimization (IGMO) module that models temporal evolution of point features to handle dynamic geometric changes; 2) Spatio-Temporal Point Trajectory Update (STTU) module that leverages broad temporal context to infer positions for occluded points; 3) Overlapping sliding-window inference strategy with cross-window propagation and in-window refinement for long sequences.

Result: Achieves best accuracy in long-term scene flow estimation on synthetic PointOdyssey3D and real-world ADT3D datasets, maintains real-time performance at 32.5 FPS, and demonstrates superior 3D motion understanding compared to RGB-D-based approaches.

Conclusion: PCSTracker effectively addresses temporal consistency challenges in point cloud scene flow estimation for long sequences, providing accurate, real-time performance while handling dynamic geometry, occlusions, and error accumulation.
...
<details>
  <summary>Details</summary>
Motivation: Existing hallucination evaluation benchmarks for MLLMs are limited by oversimplified tasks with saturated metrics and insufficient diversity, failing to adequately assess hallucination extent in state-of-the-art multimodal models.

Method: Proposes FREAK benchmark using high-quality photorealistic images featuring fine-grained counter-commonsense edits to evaluate hallucination in detailed visual perception. Includes a controlled subset for indirect evaluation and systematic assessment of Chain-of-Thought prompting techniques.

Result: Extensive experiments reveal severe hallucination issues in SOTA models regarding detailed visual perception. Systematic evaluation of CoT techniques provides critical insights into hallucination patterns and model reasoning processes.

Conclusion: FREAK addresses the gap in comprehensive multimodal hallucination assessment and enables deeper investigation of hallucination phenomena in MLLMs' detailed visual perception capabilities.
...
### [62] [Adapting a Pre-trained Single-Cell Foundation Model to Spatial Gene Expression Generation from Histology Images](https://arxiv.org/abs/2603.19766)
*Donghai Fang,Yongheng Li,Zhen Wang,Yuansong Zeng,Wenwen Min*

Main category: cs.CV

TL;DR: HINGE adapts pre-trained single-cell foundation models for histology-conditioned spatial expression generation by retrofitting them with visual context injection while preserving learned gene relationships.


<details>
  <summary>Details</summary>
Motivation: Spatial transcriptomics is expensive and low-throughput, motivating prediction from histology. Existing generative approaches lack explicit modeling of gene-gene dependencies, while single-cell foundation models capture these relationships but can't directly handle histology conditioning due to missing visual pathways and objective mismatches.

Method: HINGE retrofits pre-trained sc-FMs with SoftAdaLN (lightweight, identity-initialized modulation for layer-wise visual context injection), expression-space masked diffusion objective, and warm-start curriculum for training stability and objective alignment.
...
Conclusion: HINGE establishes a practical approach to adapt pre-trained single-cell foundation models for histology-conditioned spatial expression generation while preserving critical gene relationships learned during pre-training.
...
### [63] [FlashCap: Millisecond-Accurate Human Motion Capture via Flashing LEDs and Event-Based Vision](https://arxiv.org/abs/2603.19770)
...
TL;DR: FlashCap introduces a flashing LED-based motion capture system for precise motion timing, creating FlashMotion dataset with millisecond resolution across multiple modalities, and proposes ResPose for improved pose estimation and timing accuracy.


<details>
  <summary>Details</summary>
Motivation: Precise motion timing (PMT) is critical for sports analysis but overlooked in human pose estimation due to lack of high-temporal-resolution datasets. Current high-speed RGB camera solutions are expensive, light-sensitive, and computationally complex, limiting daily use.

Method: Developed FlashCap, a flashing LED-based motion capture system for PMT. Collected FlashMotion dataset with millisecond resolution across event, RGB, LiDAR, and IMU modalities. Proposed ResPose, a baseline model that learns residual poses from events and RGB data.

Result: FlashMotion dataset validated for high quality. ResPose reduces pose estimation errors by ~40% and achieves millisecond-level timing accuracy, enabling new research opportunities in precise motion timing and high-temporal-resolution HPE.

Conclusion: FlashCap provides a practical solution for precise motion timing, FlashMotion dataset fills the gap in high-temporal-resolution motion data, and ResPose demonstrates effectiveness for both PMT and improved pose estimation, opening new research directions.
...
TL;DR: A training-free object detection method using segmentation foundation models with feature-based classification, designed for GUI testing in continuous integration where data variation is low but training data generation is undesirable.


<details>
  <summary>Details</summary>
Motivation: Address use cases with low data variation but requirements for no training data generation or model training, particularly in automated GUI testing for continuous integration in software development where retraining costs are prohibitive.

Method: Combine segments from segmentation foundation models with simple feature-based classification, eliminating need for training datasets or model retraining when objects or designs change.

Result: Achieves performance almost comparable to learning-based methods like YOLO on icon detection in navigation maps for automotive UI testing, without requiring any training.

Conclusion: Proposed training-free approach using segmentation foundation models with feature-based classification provides viable alternative to learning-based methods for low-variation object detection tasks, particularly beneficial in automated testing scenarios where training data generation and retraining are impractical.
...
TL;DR: TIEdit benchmark with 5,120 edited images and 15,360 MOS ratings across 3 dimensions, plus EditProbe LLM-based evaluator using intermediate-layer probing for better correlation with human judgments.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation benchmarks for text-guided image editing (TIE) are limited in scale and show weak correlation with human perceptual judgments, creating a need for more reliable assessment methods.

Method: 1) TIEdit benchmark: 512 source images with editing prompts across 8 tasks, 5,120 edited images from 10 SOTA models, 20 expert annotators producing 307,200 ratings aggregated into 15,360 MOS across 3 dimensions. 2) EditProbe: LLM-based evaluator that extracts informative representations from intermediate layers of multimodal large language models to capture semantic and perceptual relationships between source images, editing instructions, and edited results.
...
Conclusion: TIEdit and EditProbe together provide a foundation for more reliable and perceptually aligned evaluation of text-guided image editing methods, addressing current limitations in TIE assessment.
...
### [66] [ReManNet: A Riemannian Manifold Network for Monocular 3D Lane Detection](https://arxiv.org/abs/2603.19776)
*Chengzhi Hong,Bijun Li*

Main category: cs.CV

TL;DR: ReManNet introduces a Road-Manifold Assumption for monocular 3D lane detection, treating roads as smooth 2D manifolds and lanes as embedded 1D submanifolds, with Riemannian Gaussian descriptors on SPD manifolds and a 3D Tunnel Lane IoU loss for improved geometric reasoning.


<details>
  <summary>Details</summary>
Motivation: Monocular 3D lane detection suffers from depth ambiguity and weak geometric constraints. Existing methods rely on simplified physical assumptions and weakly encode road geometry, leading to ill-posed 2D-to-3D lifting that often produces artifacts like concavities, bulges, and twists.

Method: Proposes the Road-Manifold Assumption: roads as smooth 2D manifolds in R³, lanes as embedded 1D submanifolds, and sampled lane points as dense observations. ReManNet uses an image backbone with detection heads for initial predictions, encodes geometry as Riemannian Gaussian descriptors on the SPD manifold, fuses these with visual features via a lightweight gate, and employs a 3D Tunnel Lane IoU loss for joint point-curve optimization.

Result: Achieves state-of-the-art or competitive results on standard benchmarks. On OpenLane, improves F1 by +8.2% over baseline and +1.8% over previous best, with scenario-level gains up to +6.6%.

Conclusion: The Road-Manifold Assumption provides a principled geometric-topological coupling for 3D lane detection, and ReManNet demonstrates its effectiveness through Riemannian geometry encoding and the novel 3D-TLIoU loss, significantly improving performance over existing methods.
...
### [67] [One Model, Two Minds: Task-Conditioned Reasoning for Unified Image Quality and Aesthetic Assessment](https://arxiv.org/abs/2603.19779)
*Wen Yin,Cencen Liu,Dingrui Liu,Bing Su,Yuan-Fang Li,Tao He*

Main category: cs.CV

TL;DR: TATAR is a unified framework that addresses fundamental mismatches between Image Quality Assessment (IQA) and Image Aesthetic Assessment (IAA) through task-aware reasoning and asymmetric rewards, outperforming prior unified baselines while remaining competitive with task-specific models.


<details>
  <summary>Details</summary>
Motivation: Existing unified IQA/IAA methods use task-agnostic approaches that apply the same reasoning strategy and reward to both tasks, which is fundamentally misaligned because IQA relies on low-level perceptual cues and benefits from concise distortion-focused reasoning, while IAA requires deliberative semantic judgment and is poorly served by point-wise score regression.

Method: TATAR combines three components: 1) fast-slow task-specific reasoning construction (concise perceptual rationales for IQA, deliberative aesthetic narratives for IAA), 2) two-stage SFT+GRPO learning establishing task-aware behavioral priors before reward-driven refinement, and 3) asymmetric rewards (Gaussian score shaping for IQA, Thurstone-style completion ranking for IAA).
...
Conclusion: The results establish task-conditioned post-training as a principled paradigm for unified perceptual scoring, demonstrating that addressing the fundamental reasoning and optimization mismatches between IQA and IAA through task-aware design leads to superior performance.
...
TL;DR: DeSC proposes a decoupled sensitivity-consistency framework for weakly supervised video anomaly detection that separates temporal sensitivity and semantic consistency streams to overcome the sensitivity-stability trade-off in unified frameworks.


<details>
  <summary>Details</summary>
Motivation: Existing weakly supervised video anomaly detection methods using unified frameworks suffer from a fundamental sensitivity-stability trade-off, where conflicting objectives for detecting transient (sensitive) and sustained (stable) anomalies lead to either fragmented predictions or over-smoothed responses.

Method: DeSC trains two specialized streams with distinct optimization strategies: 1) temporal sensitivity stream with aggressive optimization to capture high-frequency abrupt changes, and 2) semantic consistency stream with robust constraints to maintain long-term coherence and reduce noise. Their complementary strengths are fused through a collaborative inference mechanism.
...
Conclusion: The decoupled approach effectively addresses the sensitivity-stability trade-off in weakly supervised video anomaly detection by separating temporal sensitivity and semantic consistency objectives, leading to superior performance through complementary stream fusion.
...
### [69] [Learning Hierarchical Orthogonal Prototypes for Generalized Few-Shot 3D Point Cloud Segmentation](https://arxiv.org/abs/2603.19788)
*Yifei Zhao,Fanyu Zhao,Zhongyuan Zhang,Shengtang Wu,Yixuan Lin,Yinsheng Li*

Main category: cs.CV

TL;DR: HOP3D is a framework for generalized few-shot 3D point cloud segmentation that uses hierarchical orthogonal prototypes and entropy regularization to balance adaptation to novel classes while preserving base-class performance.


<details>
  <summary>Details</summary>
Motivation: Generalized few-shot 3D point cloud segmentation faces the stability-plasticity trade-off: adapting to novel classes from few annotations often interferes with shared representations and causes base-class forgetting.

Method: HOP3D introduces hierarchical orthogonalization that decouples base and novel learning at both gradient and representation levels, along with an entropy-based few-shot regularizer that leverages predictive uncertainty to refine prototype learning.
...
Conclusion: HOP3D effectively mitigates base-novel interference through hierarchical orthogonal prototypes and entropy regularization, enabling robust novel-class adaptation without degrading base-class performance in few-shot 3D point cloud segmentation.
...
TL;DR: A plug-and-play framework for text-to-motion generation that enables structured, localized body-part editing via compact scalar-based phase control, allowing fine-grained manipulation of motion magnitude, speed, and timing while maintaining overall coherence.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-motion generation methods struggle with modifying specific body parts while maintaining overall motion coherence. Current approaches rely on cumbersome, high-dimensional joint constraints (like trajectories) that hinder user-friendly, iterative refinement.

Method: Proposes Modular Body-Part Phase Control, a plug-and-play framework that models body-part latent motion channels as sinusoidal phase signals characterized by amplitude, frequency, phase shift, and offset. Uses a modular Phase ControlNet branch to inject these signals via residual feature modulation, decoupling control from the generative backbone.

Result: The approach provides predictable and fine-grained control over motion magnitude, speed, and timing while preserving global motion coherence. It works with both diffusion- and flow-based models and offers a practical paradigm for controllable text-to-motion generation.

Conclusion: Modular Body-Part Phase Control enables structured, localized editing through a compact scalar-based phase interface, addressing limitations of existing methods and providing a user-friendly solution for iterative refinement in text-to-motion generation.
...
### [72] [Evaluating Vision Foundation Models for Pixel and Object Classification in Microscopy](https://arxiv.org/abs/2603.19802)
*Carolin Teuber,Anwai Archit,Tobias Boothe,Peter Ditte,Jochen Rink,Constantin Pape*

Main category: cs.CV

TL;DR: Vision foundation models (VFMs) like SAM, SAM2, DINOv3, and domain-specific variants improve pixel and object classification in microscopy compared to traditional hand-crafted features, establishing a benchmark for VFM applications in biomedical imaging.


<details>
  <summary>Details</summary>
Motivation: Despite deep learning's dominance in computer vision, interactive semantic segmentation (pixel classification) and object-level classification in biomedical imaging still rely on feature-based shallow learning due to data diversity, lack of large pretraining datasets, and need for computational/label efficiency. The paper investigates whether vision foundation models (VFMs) can improve these tasks compared to current approaches.
...
Result: Results demonstrate consistent improvements over hand-crafted features, providing a clear pathway toward practical improvements in pixel and object classification for microscopy applications.

Conclusion: The study establishes a benchmark for VFMs in microscopy and informs future developments in this area, showing that vision foundation models can effectively enhance traditional shallow learning approaches for biomedical image analysis tasks.
...
TL;DR: SeGroS is a fine-tuning framework that addresses granularity mismatch and supervisory redundancy in Unified Multimodal Models by using visual grounding maps to create semantic visual hints and corrupted inputs for improved generation fidelity.


<details>
  <summary>Details</summary>
Motivation: Current generative training paradigms for Unified Multimodal Models suffer from inherent limitations, particularly granularity mismatch and supervisory redundancy, which hinder their performance in multimodal understanding and generation tasks.
...
Conclusion: SeGroS effectively resolves the granularity mismatch and supervisory redundancy issues in Unified Multimodal Models, leading to substantial improvements in multimodal generation performance and alignment.
...
### [74] [HUGE-Bench: A Benchmark for High-Level UAV Vision-Language-Action Tasks](https://arxiv.org/abs/2603.19822)
*Jingyu Guo,Ziye Chen,Ziwen Li,Zhengqing Gao,Jiaxin Huang,Hanlue Zhang,Fengming Huang,Yu Yao,Tongliang Liu,Mingming Gong*

Main category: cs.CV

TL;DR: HUGE-Bench is a new benchmark for high-level UAV vision-language-action tasks that tests agents' ability to interpret concise commands and execute complex, safe trajectories using aligned 3D Gaussian Splatting-Mesh representations with process-oriented and collision-aware evaluation metrics.


<details>
  <summary>Details</summary>
Motivation: Existing UAV VLN benchmarks focus on long, step-wise route descriptions with goal-centric evaluation, which doesn't adequately test real-world operations where brief, high-level commands must be grounded into safe multi-stage behaviors. There's a need for diagnostic benchmarks that evaluate high-level semantic understanding and safety awareness.

Method: The authors created HUGE-Bench with 4 real-world digital twin scenes, 8 high-level tasks, and 2.56M meters of trajectories. They use an aligned 3D Gaussian Splatting (3DGS)-Mesh representation that combines photorealistic rendering with collision-capable geometry for scalable generation and collision-aware evaluation. They introduce process-oriented and collision-aware metrics to assess process fidelity, terminal accuracy, and safety.

Result: Experiments on representative state-of-the-art VLA models reveal significant gaps in high-level semantic completion and safe execution, demonstrating that current models struggle with interpreting concise language and executing complex, safe trajectories.

Conclusion: HUGE-Bench serves as a diagnostic testbed for high-level UAV autonomy, highlighting the need for improved models that can better interpret high-level commands and execute safe, process-oriented behaviors in real-world scenarios.
...
### [75] [Fourier Splatting: Generalized Fourier encoded primitives for scalable radiance fields](https://arxiv.org/abs/2603.19834)
*Mihnea-Bogdan Jurca,Bert Van hauwermeiren,Adrian Munteanu*

Main category: cs.CV

TL;DR: Fourier Splatting introduces scalable planar primitives with Fourier encoding for novel view synthesis, enabling quality scaling without pruning and achieving SOTA results comparable to volumetric methods.


<details>
  <summary>Details</summary>
Motivation: Current 3D Gaussian Splatting methods tie visual fidelity strictly to primitive count, requiring quality downscaling through primitive pruning. There's a need for inherently scalable primitives that can adapt to varying bandwidth constraints without retraining.

Method: Proposes Fourier Splatting with scalable planar primitives parameterized by Fourier descriptors. Uses straight-through estimator for gradient extension beyond primitive boundaries and HYDRA densification strategy (MCMC-based decomposition of complex primitives into simpler ones).

Result: Achieves state-of-the-art rendering quality among planar-primitive frameworks and comparable perceptual metrics to leading volumetric representations on standard benchmarks. Enables single trained model to render at varying levels of detail via Fourier coefficient truncation.

Conclusion: Fourier Splatting provides a versatile solution for bandwidth-constrained high-fidelity rendering by introducing the first inherently scalable primitive for radiance field rendering, overcoming the quality-primitive count dependency of existing methods.
...
### [76] [Hyper-Connections for Adaptive Multi-Modal MRI Brain Tumor Segmentation](https://arxiv.org/abs/2603.19844)
*Lokendra Kumar,Shubham Aggarwal*

Main category: cs.CV

TL;DR: Hyper-Connections (HC) improve multi-modal brain tumor segmentation across five 3D architectures with up to +1.03% mean Dice gain, especially enhancing fine-grained boundary delineation and modality sensitivity.


<details>
  <summary>Details</summary>
Motivation: To enhance multi-modal brain tumor segmentation by replacing fixed residual connections with dynamic Hyper-Connections that can better adaptively aggregate features from different imaging modalities.

Method: Hyper-Connections are integrated as drop-in replacements for fixed residual connections across five architectures (nnU-Net, SwinUNETR, VT-UNet, U-Net, U-Netpp) and evaluated on BraTS 2021 dataset for volumetric multi-modal brain tumor segmentation.

Result: Dynamic HC consistently improves all 3D models with up to +1.03% mean Dice gain, most pronounced in Enhancing Tumor sub-region. HC-equipped models develop sharper sensitivity to clinically dominant sequences (T1ce for Tumor Core/Enhancing Tumor, FLAIR for Whole Tumor). 2D improvements are smaller and configuration-sensitive.

Conclusion: Hyper-Connections establish a simple, efficient, and broadly applicable mechanism for multi-modal feature fusion in medical image segmentation, with volumetric spatial context amplifying the benefit of adaptive aggregation.
...
TL;DR: The paper proposes a framework to diagnose failure modes in deep learning-based online mapping by disentangling memorization of input features from overfitting to known map geometries, introducing novel metrics and dataset analysis tools to improve generalization assessment and training set design.


<details>
  <summary>Details</summary>
Motivation: Deep learning-based online mapping models for autonomous driving frequently fail to generalize beyond familiar environments, suffering from memorization of input features and overfitting to known map geometries, which necessitates better diagnostic tools for understanding these failure modes.

Method: Proposes a framework with evaluation subsets controlling for geographical proximity and geometric similarity, introduces Fréchet distance-based reconstruction statistics for shape fidelity, defines localization and map geometry overfitting scores, develops MST diversity and symmetric coverage measures for dataset analysis, and formulates an MST-based sparsification strategy for training set optimization.

Result: Experiments on nuScenes and Argoverse 2 across multiple state-of-the-art models demonstrate more trustworthy generalization assessment, show that map geometry-diverse and balanced training sets improve performance, and reveal that MST-based sparsification reduces redundancy while maintaining or enhancing performance with smaller training sets.

Conclusion: The framework enables failure-mode-aware evaluation protocols and map geometry-centric dataset design for deployable online mapping, providing tools to diagnose generalization issues and optimize training data composition for better real-world performance.
...
### [78] [IsoCLIP: Decomposing CLIP Projectors for Efficient Intra-modal Alignment](https://arxiv.org/abs/2603.19862)
*Simone Magistri,Dipam Goswami,Marco Mistretta,Bartłomiej Twardowski,Joost van de Weijer,Andrew D. Bagdanov*

Main category: cs.CV

TL;DR: IsoCLIP addresses intra-modal misalignment in CLIP by identifying and removing anisotropic directions from the shared embedding space, improving performance on intra-modal tasks like image-to-image retrieval without additional training.


<details>
  <summary>Details</summary>
Motivation: CLIP's vision-language models perform well on inter-modal tasks but suffer from intra-modal misalignment when applied to intra-modal tasks like image-to-image retrieval, due to how the contrastive loss shapes the shared embedding space.

Method: Analyzes CLIP's projector weights to identify an isotropic subspace where modalities are well-aligned and anisotropic directions specific to each modality. Proposes removing anisotropic directions to improve intra-modal alignment, using spectral analysis of the inter-modal operator derived from projector weights.
...
Conclusion: Intra-modal misalignment in CLIP stems from anisotropic directions in the shared embedding space; removing these directions improves intra-modal task performance without retraining, offering a computationally efficient solution.
...
TL;DR: A greedy frame selection method for long-video QA that jointly optimizes question relevance and semantic coverage under fixed frame budgets, using complementary embeddings and question-adaptive strategies.


<details>
  <summary>Details</summary>
Motivation: Current VLMs for long-video QA face inference bottlenecks from excessive visual tokens. Sparse sampling misses key moments, while purely relevance-based selection collapses to near-duplicates and lacks temporal coverage.

Method: Question-adaptive greedy frame selection with 1 FPS candidate pool (max 1000 frames), dual embeddings (SigLIP for relevance, DINOv2 for semantics), maximizing weighted sum of modular relevance and facility-location coverage terms. Uses four preset strategies and lightweight question-type classifier.

Result: Consistent accuracy gains over uniform sampling and strong baselines on MLVU benchmark across frame budgets, with largest improvements under tight budgets.

Conclusion: The proposed method effectively balances relevance and coverage for long-video QA, providing theoretical approximation guarantees and practical performance improvements, especially with limited frame budgets.
...
<details>
  <summary>Details</summary>
Motivation: Current multimodal large language models (MLLMs) perform substantially worse than human experts in medical image quality assessment, especially when providing descriptive assessments with clinical reasoning. Two key challenges hinder improvement: 1) high cost of acquiring descriptive annotations, and 2) inability of one-time data collection to adapt to evolving model weaknesses.

Method: MedQ-Engine is a closed-loop data engine with three core components: 1) Failure prototype discovery via data-driven clustering of model evaluation results, 2) Progressive human-in-the-loop annotation using these prototypes as retrieval anchors to explore a million-scale image pool, and 3) Quality-assured fine-tuning with an entropy-guided routing mechanism that triages annotations to minimize labeling cost. The system operates in iterative cycles of evaluation, annotation, and fine-tuning.

Result: Experiments across five medical imaging modalities show MedQ-Engine elevates an 8B-parameter model to surpass GPT-4o by over 13% and narrows the gap with human experts to only 4.34%. The system achieves this using only 10K annotations with more than 4x sample efficiency over random sampling.

Conclusion: MedQ-Engine provides an effective framework for improving MLLMs in medical image quality assessment through iterative, targeted data collection and model refinement. The closed-loop approach addresses both annotation cost and adaptation to evolving model weaknesses, enabling near-human performance with high efficiency.
...
TL;DR: VideoSeek is a long-horizon video agent that uses video logic flow to actively seek answer-critical evidence instead of exhaustively parsing full videos, achieving strong accuracy with far fewer frames than prior methods.


<details>
  <summary>Details</summary>
Motivation: Current video agentic models rely on greedy parsing over densely sampled video frames, resulting in high computational cost. There's a need for more efficient video understanding that can identify and focus on answer-critical evidence rather than processing all frames.

Method: VideoSeek operates in a think-act-observe loop with a toolkit for collecting multi-granular video observations. It leverages video logic flow to actively seek answer-critical evidence, enabling query-aware exploration over accumulated observations.

Result: On four challenging video understanding and reasoning benchmarks, VideoSeek achieves strong accuracy while using far fewer frames than prior video agents and standalone LMMs. Notably, it achieves 10.2 absolute points improvement on LVBench over GPT-5 while using 93% fewer frames.

Conclusion: VideoSeek demonstrates that leveraging video logic flow for active evidence-seeking enables efficient video understanding with reduced computational cost while maintaining or improving performance. The approach highlights the importance of toolkit design and strong reasoning capabilities for practical video understanding.
...
TL;DR: SIMPLER is a pre-fine-tuning architecture selection method that identifies redundant layers in vision transformers before adaptation, reducing both training and inference costs without requiring gradients or hyperparameter tuning.


<details>
  <summary>Details</summary>
Motivation: Fine-tuning Earth Observation foundation models is computationally expensive, with high training time and memory demands. Existing parameter-efficient methods reduce training costs but retain full inference complexity, while post-hoc compression only optimizes inference after costly full fine-tuning.
...
Result: On Prithvi-EO-2, SIMPLER prunes up to 79% of parameters while retaining 94% of baseline performance, yielding 2.1x training speedup and 2.6x inference speedup. The method generalizes to TerraMind (multimodal EO foundation model) and ImageNet-pretrained ViT-MAE across tasks, architectures, and spectral modalities.
...
TL;DR: ATCG introduces analogical textual concept generation to improve Generalized Category Discovery by transferring knowledge from labeled to unlabeled data through visual-textual reasoning, enhancing fine-grained category separation.


<details>
  <summary>Details</summary>
Motivation: Current GCD methods rely on visual-only pipelines with loose coupling between supervised learning and discovery, resulting in brittle boundaries especially for fine-grained, look-alike categories.

Method: ATCG is a plug-and-play module that analogizes from labeled knowledge to new observations, generating textual concepts for unlabeled samples and fusing them with visual features to create a visual-textual reasoning process.

Result: ATCG consistently improves overall, known-class, and novel-class performance across six benchmarks, with largest gains on fine-grained data, and works with both parametric and clustering GCD pipelines.

Conclusion: ATCG effectively transfers prior knowledge to novel data through analogical textual concept generation, sharpening category separation and improving GCD performance without requiring changes to existing pipeline designs.
...
TL;DR: PanORama introduces the first multiview-consistent panoptic segmentation method for operating rooms that achieves cross-view consistency through feature-level modeling in a single forward pass, without requiring camera calibration.


<details>
  <summary>Details</summary>
Motivation: Operating rooms are complex, cluttered environments with high occlusion where reliable spatial understanding is crucial for surgical situational awareness. Current multiview segmentation methods suffer from mispredictions due to limited visibility in some views and lack of cross-view consistency.

Method: PanORama models cross-view interactions at the feature level within the backbone network in a single forward pass, enabling view consistency to emerge directly rather than through post-hoc refinement. The approach is calibration-free, requiring no camera parameters, and generalizes to unseen viewpoints.

Result: Achieves >70% Panoptic Quality (PQ) on MM-OR and 4D-OR datasets, outperforming previous state-of-the-art methods. The method demonstrates effective multiview consistency and generalization to unseen camera viewpoints.

Conclusion: PanORama substantially enhances multiview segmentation and spatial understanding in operating rooms, opening new opportunities for surgical perception and assistance systems. The calibration-free approach with inherent cross-view consistency represents a significant advancement for OR scene understanding.
...
Method: SegVGGT introduces object queries that interact with multi-level geometric features to integrate instance identification into a visual geometry grounded transformer. To address attention dispersion from massive global image tokens, the Frame-level Attention Distribution Alignment (FADA) strategy explicitly guides object queries to attend to instance-relevant frames during training without extra inference overhead.

Result: SegVGGT achieves state-of-the-art performance on ScanNetv2 and ScanNet200 benchmarks, outperforming both recent joint models and RGB-D-based approaches. It also demonstrates strong generalization capabilities on ScanNet++.

Conclusion: The proposed unified end-to-end framework successfully bridges the gap between 3D reconstruction and instance segmentation, enabling simultaneous feed-forward processing directly from multi-view RGB images with superior performance and generalization.
...
TL;DR: RAM introduces a motion-aware semantic tracker with adaptive Kalman filtering for robust identity association, enhanced by memory-augmented temporal HMR for consistent motion reconstruction, and a lightweight predictor with gated fusion for continuity and robustness in 3D human motion capture.


<details>
  <summary>Details</summary>
Motivation: The paper addresses challenges in markerless 3D human motion capture in-the-wild, particularly dealing with severe occlusions, dynamic interactions, and maintaining tracking stability and motion continuity in complex multi-person scenarios.

Method: RAM incorporates: 1) motion-aware semantic tracker with adaptive Kalman filtering for robust identity association; 2) memory-augmented Temporal HMR module injecting spatio-temporal priors for consistent motion reconstruction; 3) lightweight Predictor module forecasting future poses; 4) gated combiner adaptively fusing reconstructed and predicted features.

Result: Experiments on PoseTrack and 3DPW benchmarks demonstrate RAM substantially outperforms previous state-of-the-art methods in both Zero-shot tracking stability and 3D accuracy.

Conclusion: RAM offers a generalizable paradigm for markerless 3D human motion capture in-the-wild, achieving superior performance in tracking stability and motion reconstruction accuracy through its integrated approach to identity association, temporal consistency, and predictive continuity.
...
### [87] [LIORNet: Self-Supervised LiDAR Snow Removal Framework for Autonomous Driving under Adverse Weather Conditions](https://arxiv.org/abs/2603.19936)
*Ji-il Park,Inwook Shim*

Main category: cs.CV

TL;DR: LIORNet: A self-supervised LiDAR denoising network that combines distance, intensity, and learning-based approaches to remove adverse weather noise without manual annotations, outperforming SOTA methods in accuracy and speed.


<details>
  <summary>Details</summary>
Motivation: LiDAR performance degrades significantly in adverse weather (snow, rain, fog) due to spurious noise points causing false perception. Existing approaches have limitations: distance-based filters struggle to distinguish valid points, intensity-based methods lack adaptability with fixed thresholds, and learning-based methods suffer from annotation costs, poor generalization, and computational overhead.

Method: LIORNet uses a U-Net++ backbone with self-supervised learning guided by pseudo-labels generated from multiple physical/statistical cues: range-dependent intensity thresholds, snow reflectivity, point sparsity, and sensing range constraints. This eliminates need for manual annotations and integrates strengths of distance, intensity, and learning-based paradigms.
...
Conclusion: LIORNet provides a practical, robust solution for LiDAR perception in extreme weather with strong potential for real-time deployment in autonomous driving systems, overcoming annotation difficulties and limitations of single-principle approaches.
...
TL;DR: A novel framework for optimizing diffusion model inference by learning timestep-specific masks to dynamically bypass or reuse features, achieving faster sampling with minimal quality loss.


<details>
  <summary>Details</summary>
Motivation: Diffusion Probabilistic Models suffer from high inference latency due to iterative denoising, and existing optimization methods have prohibitive memory costs from full-chain backpropagation.

Method: Proposes learning timestep-specific masks to determine which blocks to execute or bypass via feature reuse at each inference stage, with timestep-aware loss scaling and knowledge-guided mask rectification for pruning redundant dependencies.

Result: Achieves significant efficiency gains across DDPM, LDM, DiT, and PixArt models while maintaining generative quality, demonstrating superior balance between sampling speed and quality.

Conclusion: By treating denoising as a sequence of optimized computational paths, the method provides an architecture-agnostic solution for accelerating diffusion model inference with memory-efficient training.
...
TL;DR: HiPath is a lightweight vision-language model framework for structured pathology report generation that uses hierarchical patch aggregation, contrastive learning, and slot-based prediction to outperform existing methods on real-world Chinese pathology data.


<details>
  <summary>Details</summary>
Motivation: Existing pathology vision-language models reduce structured, multi-granular pathology reports to flat labels or free-form text, failing to capture the hierarchical structure and clinical nuances of real pathology documentation.

Method: HiPath uses three trainable modules (15M parameters total) built on frozen UNI2 and Qwen3 backbones: Hierarchical Patch Aggregator (HiPA) for multi-image visual encoding, Hierarchical Contrastive Learning (HiCL) for cross-modal alignment via optimal transport, and Slot-based Masked Diagnosis Prediction (Slot-MDP) for structured diagnosis generation.

Result: Trained on 749K real-world Chinese pathology cases from three hospitals, HiPath achieves 68.9% strict accuracy and 74.7% clinically acceptable accuracy with 97.3% safety rate, outperforming all baselines under the same frozen backbone. Cross-hospital evaluation shows only 3.4 percentage point drop in strict accuracy while maintaining 97.1% safety.

Conclusion: HiPath demonstrates that treating structured report prediction as the primary training objective with lightweight hierarchical modules enables accurate, safe, and generalizable pathology report generation that preserves clinical structure and nuance.
...
Method: Uses covariance pooling to encode convolutional features as SPD matrices, proposes continuous pose encoding via Cholesky decomposition of SPD matrices, and employs manifold-aware network head that respects Riemannian geometry of SPD spaces.

Result: Experiments demonstrate relevance of second-order pooling and continuous representations for direct pose regression, including improved performance under partial occlusion.

Conclusion: The proposed approach effectively addresses limitations of direct pose regression by incorporating spatial second-order statistics and continuous pose representations through SPD matrix encoding and Riemannian geometry-aware learning.
...
Conclusion: 2K Retrofit provides a practical solution for efficient high-resolution geometric prediction, enabling the deployment of foundation models in real-world applications like autonomous driving, robotics, and AR/MR without prohibitive computational costs.
...
TL;DR: X-World is an action-conditioned multi-camera generative world model that simulates realistic future video observations for autonomous driving evaluation, enabling controllable, reproducible simulation of driving scenarios.


<details>
  <summary>Details</summary>
Motivation: Current autonomous driving evaluation relies heavily on costly, biased, and non-reproducible real-world road testing. There's a need for a real-world simulator that can generate realistic future observations under proposed actions while remaining controllable and stable over long horizons.
...
Conclusion: X-World provides a practical foundation for scalable and reproducible evaluation of autonomous driving systems by enabling realistic, controllable simulation of driving scenarios directly in video space.
...
TL;DR: MedSPOT introduces a workflow-aware sequential grounding benchmark for clinical GUI environments, evaluating multimodal models' ability to perform reliable visual grounding across interdependent steps in medical software workflows.


<details>
  <summary>Details</summary>
Motivation: Existing GUI benchmarks focus on isolated, single-step grounding queries, overlooking the sequential, workflow-driven reasoning required in real-world medical interfaces where tasks evolve across independent steps and dynamic interface states.

Method: Created MedSPOT benchmark with 216 task-driven videos and 597 annotated keyframes, modeling procedural interaction as a sequence of structured spatial decisions. Introduced strict sequential evaluation protocol that terminates task assessment upon first incorrect grounding prediction, and developed comprehensive failure taxonomy for systematic diagnosis.

Result: The benchmark captures interface hierarchies, contextual dependencies, and fine-grained spatial precision under evolving conditions, enabling measurement of error propagation in multi-step workflows through the proposed evaluation protocol.

Conclusion: By shifting evaluation from isolated grounding to workflow-aware sequential reasoning, MedSPOT establishes a realistic and safety-critical benchmark for assessing multimodal models in medical software environments.
...
TL;DR: First evaluation of Test-Time Adaptation (TTA) methods for Facial Expression Recognition under natural domain shifts, showing TTA can boost performance by up to 11.34% with effectiveness governed by distributional distance and shift severity.


<details>
  <summary>Details</summary>
Motivation: Deep learning models struggle with natural distribution shifts in real-world deployments, and while Test-Time Adaptation addresses this, there's no evaluation of TTA methods for Facial Expression Recognition under natural domain shifts beyond synthetic corruptions.

Method: Cross-dataset experiments with widely used FER datasets to evaluate TTA methods under natural domain shifts caused by differing collection protocols, annotation standards, and demographics, moving beyond synthetic corruptions.
...
Conclusion: TTA effectiveness for FER under natural domain shifts is governed by distributional distance and severity of natural shift across domains, with different TTA methods performing optimally under different target distribution conditions.
...
TL;DR: NEC-Diff is a diffusion-based framework that combines RAW images and event camera data for high-quality imaging in extreme low-light conditions, using physics-driven constraints and adaptive SNR-based feature fusion.


<details>
  <summary>Details</summary>
Motivation: Extreme low-light imaging suffers from severe noise and texture loss due to photon scarcity. While event cameras offer high dynamic range and motion sensitivity as complements to conventional cameras, existing approaches focus on texture recovery but neglect image noise and event noise, limiting accurate reconstruction under photon-starved conditions.

Method: NEC-Diff is a diffusion-based event-RAW hybrid imaging framework that: (1) leverages the linear light-response of RAW images and brightness-change nature of events to establish physics-driven constraints for robust dual-modal denoising; (2) dynamically estimates SNR of both modalities based on denoising results to guide adaptive feature fusion, injecting reliable cues into the diffusion process for high-fidelity reconstruction.

Result: The authors created the REAL dataset with 47,800 pixel-aligned low-light RAW images, events, and high-quality references under 0.001-0.8 lux illumination. Extensive experiments demonstrate NEC-Diff's superiority under extreme darkness conditions.

Conclusion: NEC-Diff effectively addresses the challenges of extreme low-light imaging by combining RAW and event data through a diffusion framework with physics-driven constraints and adaptive SNR-based fusion, enabling high-quality reconstruction in photon-starved conditions.
...
TL;DR: FRAM: A two-stage diffusion-based makeup transfer method with facial region-aware control and makeup-specific CLIP fine-tuning


<details>
  <summary>Details</summary>
Motivation: Current methods use generic foundation models that struggle to capture makeup styles and lack regional control for specific facial areas (eyes, mouth, etc.), limiting precise makeup transfer.

Method: Two-stage approach: (1) Fine-tune CLIP on synthetic makeup data generated using GPT-o3 and text-driven editing; (2) Identity and region-aware injection using learnable tokens to query makeup CLIP for facial region features, with ControlNet Union for source identity encoding.

Result: Superior regional controllability and makeup transfer performance compared to existing methods, verified through experimental results.

Conclusion: FRAM addresses limitations of current makeup transfer methods by providing precise facial region-aware control and makeup-specific feature extraction through fine-tuned CLIP and learnable token-based injection.
...
Method: Two-stage approach: coarse stage explores multi-granularity features from image encoder stages and tabular data; fine stage uses unimodal/crossmodal prototypes with hierarchical anchor-based relationship mining strategy for contrastive learning.

Result: Outperforms SOTA methods with 1.53% and 0.91% AUC improvements on MEN and Derm7pt datasets respectively.

Conclusion: CFCML effectively reduces modality gap and extracts discriminative crossmodal information, enhancing disease diagnosis performance in clinical practice.
...
TL;DR: Detached Skip-Links improve MLLM performance on OCR tasks by reducing gradient interference in multi-layer feature fusion through asymmetric forward/backward pathways.


<details>
  <summary>Details</summary>
Motivation: MLLMs excel at high-level reasoning but fail on OCR tasks where fine-grained visual details are compromised or misaligned due to optimization issues in multi-layer feature fusion where skip pathways introduce direct back-propagation that overwrites low-level signals and destabilizes training.

Method: Propose Detached Skip-Links - a minimal modification that reuses shallow features in forward pass while stopping gradients through skip branch during joint training, plus R-Probe diagnostic tool to measure pixel-level reconstructability of projected visual tokens using shallow decoder initialized from first quarter of LLM layers.

Result: Across multiple ViT backbones and multimodal benchmarks at scales up to 7M training samples, approach consistently improves OCR-centric benchmarks and delivers clear gains on general multimodal tasks.

Conclusion: Asymmetric design reduces gradient interference, improving stability and convergence without adding learnable parameters, addressing overlooked optimization issue in multi-layer feature fusion for MLLMs.
...
TL;DR: A QA framework and open-source platform for evaluating 3D ultrasound reconstruction accuracy using a custom phantom with geometric inclusions, achieving state-of-the-art robotic 3D US performance approaching transducer resolution limits.


<details>
  <summary>Details</summary>
Motivation: Current 3D ultrasound studies lack comprehensive evaluation of volumetric accuracy and reproducibility, creating a need for robust Quality Assurance frameworks, especially for tracked 3D US reconstruction using freehand or robotic acquisition methods.

Method: Developed a QA framework with custom phantom containing geometric inclusions with varying symmetry properties. Created standardized pipeline for real-time segmentation and 3D reconstruction (DSC = 0.97, FPS = 46 without GPU), followed by automated registration and comparison with ground-truth geometries. Evaluated optical, electromagnetic, and robotic kinematic tracking at different scanning speeds and insonation angles.

Result: Robotic 3D US achieved state-of-the-art reconstruction performance: DSC-3D = 0.94 ± 0.01, HD95 = 1.17 ± 0.12 mm, approaching the spatial resolution limit imposed by the transducer. The framework enables robust cross-platform comparisons and improved reporting practices.

Conclusion: Established a flexible experimental platform and reproducible validation methodology for 3D US reconstruction. The framework supports safe and effective clinical translation of 3D ultrasound in diagnostic and image-guided therapy applications through robust QA and cross-platform comparisons.
...
TL;DR: A preference-guided debiasing framework for no-reference enhancement image quality assessment that learns enhancement-preference embeddings and removes algorithm-specific biases to improve generalization across different enhancement algorithms.
...
Conclusion: The proposed preference-guided debiasing framework successfully addresses the generalization problem in NR-IQA for enhanced images by removing algorithm-specific biases and focusing on genuine perceptual quality cues.
...
TL;DR: CoA is a reinforcement learning framework that preserves multimodal priors while adapting VLMs to specialized domains like surgery, achieving better generalization than supervised fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Conventional fine-tuning on domain-specific datasets can inadvertently alter a model's pretrained multimodal priors, leading to reduced generalization. There's a need to integrate domain knowledge while maintaining inherent reasoning and perceptual capabilities.

Method: Chain-of-Adaptation (CoA) introduces a structured reasoning format that enhances domain alignment without sacrificing general multimodal competence through reinforcement learning.

Result: Experiments on standard surgical benchmarks show CoA achieves higher accuracy, stronger generalization (in both in-distribution and out-of-distribution settings), and more stable behavior than supervised fine-tuning. Ablation studies confirm CoA effectively preserves core visual-language abilities.

Conclusion: CoA provides a reliable pathway for domain specialization in VLMs by maintaining pretrained multimodal priors while integrating domain-specific knowledge through structured reasoning and reinforcement learning.
...
<details>
  <summary>Details</summary>
Motivation: NeRF-based methods become computationally expensive for high-resolution rendering, and 2D super-resolution of per-view renderings often breaks multi-view consistency, necessitating a 3D-aware approach for scalable HR novel view synthesis.

Method: Built on Neural Graphics Primitives (NGP), NGP-SR conditions radiance prediction on 3D coordinates and learned local texture tokens to recover high-frequency details within the radiance field, enabling direct HR reconstruction from LR inputs without external HR references.

Result: NGP-SR consistently improves both reconstruction quality and runtime efficiency over prior NeRF-based super-resolution methods across multiple datasets, demonstrating practical scalability for HR novel view synthesis.

Conclusion: The proposed generalizable framework offers a practical solution for scalable high-resolution novel view synthesis by directly reconstructing HR radiance fields from LR inputs while maintaining multi-view consistency and eliminating per-scene optimization.
...
TL;DR: FacadeFixer: A multi-agent framework for building facade defect inspection that uses collaborative reasoning between detection, segmentation, and generative agents to address data scarcity and complex defect patterns.


<details>
  <summary>Details</summary>
Motivation: Building facade defect inspection faces challenges due to extreme geometric variability, low contrast against complex backgrounds, and composite defects (e.g., cracks with spalling). These cause severe pixel imbalance and feature ambiguity, exacerbated by scarcity of high-quality pixel-level annotations, hindering generalization of existing models.

Method: FacadeFixer is a unified multi-agent framework treating defect perception as collaborative reasoning rather than isolated recognition. It orchestrates specialized agents for detection and segmentation to handle multi-type defect interference, working with a generative agent for semantic recomposition. This decouples intricate defects from noisy backgrounds and synthesizes them onto diverse clean textures, generating high-fidelity augmented data with precise masks. A comprehensive multi-task dataset covering six primary facade categories with pixel-level annotations is introduced.

Result: Extensive experiments demonstrate FacadeFixer significantly outperforms state-of-the-art baselines. It excels in capturing pixel-level structural anomalies and highlights generative synthesis as a robust solution to data scarcity in infrastructure inspection.

Conclusion: FacadeFixer provides an effective multi-agent collaborative approach to building facade defect inspection, addressing data scarcity through generative synthesis while handling complex defect patterns. The framework and dataset will be made publicly available to advance research in infrastructure inspection.
...
TL;DR: DefectBench is the first multi-dimensional benchmark for evaluating Large Multimodal Models in building facade inspection, revealing LMMs excel at semantic understanding but struggle with metric localization precision while demonstrating viable zero-shot generative segmentation.


<details>
  <summary>Details</summary>
Motivation: Traditional facade inspection relies on specialized discriminative models with limited generalization and passive perception. LMMs promise active reasoning but lack rigorous evaluation standards in high-stakes engineering domains.

Method: Developed a human-in-the-loop semi-automated annotation framework to unify 12 fragmented datasets into standardized hierarchical ontology. Created DefectBench to evaluate 18 SOTA LMMs across three cognitive dimensions: Semantic Perception, Spatial Localization, and Generative Geometry Segmentation.

Result: LMMs demonstrate exceptional topological awareness and semantic understanding but show significant deficiencies in metric localization precision. Zero-shot generative segmentation proves viable, with general-purpose foundation models rivaling specialized supervised networks without domain-specific training.

Conclusion: The work establishes rigorous benchmarking standards and provides a high-quality open-source database, setting a new baseline for autonomous AI agents in civil engineering while validating LMMs' potential for active reasoning in facade inspection.
...
TL;DR: TinyML-based pipeline optimizes ConvNets for CubeSat onboard image classification using pruning, quantization, and hardware-aware mapping to achieve 89.55% RAM reduction and 70.09% Flash reduction while maintaining acceptable accuracy with ultra-low energy consumption (0.68-6.45 mJ per inference).


<details>
  <summary>Details</summary>
Motivation: Traditional Earth observation missions transmit raw imagery for ground processing, which is infeasible for CubeSats due to constraints on onboard processors, energy, and communication bandwidth. There's a need for efficient onboard image classification to reduce downlink requirements.

Method: Developed a TinyML pipeline integrating structured iterative pruning, post-training INT8 quantization, and hardware-aware operator mapping to compress CNN models. Optimized for STM32N6 microcontroller with Arm Cortex-M55 core and Neural-ART NPU, evaluated on three EO datasets (EuroSAT, RS_C11, MEDIC) with four models (SqueezeNet, MobileNetV3, EfficientNet, MCUNetV1).

Result: Achieved average 89.55% RAM reduction and 70.09% Flash memory reduction. Maintained task-acceptable accuracy with drops ranging 0.4-8.6 percentage points from Float32 baseline. Energy consumption per inference: 0.68-6.45 mJ, latency: 3.22-30.38 ms. Results satisfy CubeSat energy budgets and real-time constraints.

Conclusion: The TinyML-based optimization pipeline enables accurate, energy-efficient onboard image classification for CubeSats, overcoming traditional limitations by significantly reducing memory footprint and energy consumption while maintaining acceptable accuracy, making onboard EO processing feasible under stringent CubeSat constraints.
...
TL;DR: LagerNVS is a 3D-aware neural network for novel view synthesis that leverages explicit 3D reconstruction pre-training to achieve state-of-the-art deterministic feed-forward performance with real-time rendering.


<details>
  <summary>Details</summary>
Motivation: While neural networks can perform 3D tasks like novel view synthesis without explicit 3D reconstruction, the authors argue that strong 3D inductive biases are beneficial for network design and performance.

Method: LagerNVS uses an encoder-decoder architecture with 3D-aware latent features. The encoder is initialized from a 3D reconstruction network pre-trained with explicit 3D supervision, paired with a lightweight decoder, and trained end-to-end with photometric losses.
...
Conclusion: Explicit 3D inductive biases, implemented through pre-trained 3D reconstruction networks, significantly improve novel view synthesis performance while maintaining efficiency and generalization capabilities.
...
TL;DR: I2I-RFR is a plug-in reformulation that converts standard image-to-image regression networks into continuous-time transport models, enabling progressive refinement at inference while maintaining simple supervised training.


<details>
  <summary>Details</summary>
Motivation: Standard pixel-wise I2I regression often over-smooths ill-posed and multimodal targets, while generative alternatives require complex pipelines, additional components, and task-specific tuning.

Method: Augments backbone input by channel-wise concatenation with noise-corrupted ground-truth target and optimizes a t-reweighted pixel loss, which induces a velocity field admitting rectified-flow interpretation for ODE-based refinement.

Result: Extensive experiments across image-to-image translation and video restoration tasks show improved performance across tasks and backbones, with clear gains in perceptual quality and detail preservation.

Conclusion: I2I-RFR provides a lightweight way to incorporate continuous-time refinement into conventional I2I models without requiring heavy generative pipelines, requiring only input channel expansion and few inference steps.
...
TL;DR: MuSteerNet addresses relational distortion in video-driven human reaction generation by using mutual steering between visual observations and reaction types to synthesize better 3D human motions that properly react to video content.


<details>
  <summary>Details</summary>
Motivation: Existing video-driven human reaction generation methods fail to effectively leverage video inputs to steer human reaction synthesis, resulting in reaction motions that are mismatched with video content due to severe relational distortion between visual observations and reaction types.

Method: Proposes MuSteerNet with two key components: 1) Prototype Feedback Steering mechanism that refines visual observations using a gated delta-rectification modulator and relational margin constraint guided by prototypical vectors learned from human reactions, and 2) Dual-Coupled Reaction Refinement that leverages rectified visual cues to further steer refinement of generated reaction motions.
...
Conclusion: MuSteerNet effectively addresses relational distortion in video-driven human reaction generation through observation-reaction mutual steering, improving reaction quality and enabling better alignment between synthesized motions and video content.
...
TL;DR: Training-free sampling methods improve sample diversity in ambiguous segmentation tasks using diffusion models, achieving up to 16.4% HM IoU* improvement on Cityscapes and 7.5% on wildfire datasets.


<details>
  <summary>Details</summary>
Motivation: Diffusion models can learn multi-modal distributions for uncertain environments (wildfire spread, medical diagnosis, autonomous driving), but naive sampling is computationally inefficient, requiring hundreds of samples to find low-probability modes that remain operationally relevant.

Method: Adapt particle guidance and SPELL techniques (originally for diverse natural image generation) to discrete segmentation tasks, and propose a simple clustering-based technique. Validate on LIDC medical dataset, modified Cityscapes, and MMFire wildfire spread dataset.

Result: Training-free methods increase HM IoU* metric by up to 7.5% on MMFire and 16.4% on Cityscapes, demonstrating efficient increase in sample diversity of segmentation diffusion models with minimal cost to image quality and runtime.

Conclusion: Training-free sampling methods can effectively increase sample diversity in ambiguous segmentation tasks using diffusion models, making them more practical for real-world applications in uncertain environments.
...
TL;DR: Zero-shot composed video retrieval approach using reasoning-first strategy with large multimodal models to infer causal/temporal consequences of edits, outperforming baselines on challenging implicit-effect cases.


<details>
  <summary>Details</summary>
Motivation: Prior CoVR methods assume modification text fully specifies visual changes, overlooking after-effects and implicit consequences (motion, state transitions, viewpoint/duration cues) that emerge from edits. Successful CoVR requires reasoning about these after-effects.

Method: Reasoning-first, zero-shot approach leveraging large multimodal models to: (1) infer causal and temporal consequences implied by edits, and (2) align reasoned queries to candidate videos without task-specific finetuning. Introduces CoVR-Reason benchmark with structured reasoning traces and challenging distractors.
...
TL;DR: Mode proposal models: A deterministic framework for multi-modal segmentation that directly generates a fixed set of likely outcomes in one forward pass, avoiding expensive stochastic sampling and post-hoc clustering.


<details>
  <summary>Details</summary>
Motivation: Current generative models for ambiguous segmentation tasks (medical imaging, future prediction) are computationally expensive, requiring many samples and post-hoc clustering to identify distribution modes. Need efficient deterministic approach.

Method: Introduce mode proposal models: deterministic framework producing fixed-size set of proposal masks in single forward pass. Adapt confidence mechanism from object detection to handle superfluous proposals in high-dimensional segmentation space.

Result: Significantly reduces inference time while achieving higher ground-truth coverage than existing generative models. Can be trained without knowing full outcome distribution, applicable to real-world datasets. Can estimate prior mode probabilities by decomposing velocity field of pre-trained flow model.

Conclusion: Mode proposal models offer efficient deterministic alternative to stochastic sampling for multi-modal segmentation, with faster inference, better coverage, and practical applicability to real-world ambiguous segmentation tasks.
...
TL;DR: LumosX is a framework for personalized multi-subject video generation that addresses face-attribute alignment challenges through both data construction and model design improvements.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-video diffusion models lack explicit mechanisms for ensuring intra-group consistency in face-attribute alignment across subjects, requiring both better modeling strategies and face-attribute-aware data resources.

Method: Two-pronged approach: 1) Data side - tailored collection pipeline with MLLMs to extract relational priors and construct benchmark; 2) Modeling side - Relational Self-Attention and Relational Cross-Attention mechanisms that intertwine position-aware embeddings with refined attention dynamics for explicit subject-attribute dependencies.

Result: Comprehensive evaluations on the constructed benchmark demonstrate state-of-the-art performance in fine-grained, identity-consistent, and semantically aligned personalized multi-subject video generation.

Conclusion: LumosX advances both data and model design for personalized video generation, achieving superior face-attribute alignment through explicit relational modeling and comprehensive benchmark construction.
...
Method: Introduces taxonomy spanning edit primitives and semantic classes, releases new benchmark with per-pixel tamper maps and category supervision, proposes training framework and evaluation metrics for pixel-level correctness, localization, and semantic understanding.

Result: Reveals substantial over- and under-scoring using mask-only metrics, exposes failure modes on micro-edits and off-mask changes, establishes rigorous standard for tamper localization, semantic classification and description.

Conclusion: Framework advances field from masks to pixels, meanings and language descriptions, providing comprehensive evaluation protocol for tamper detection, classification, and description tasks.
...
TL;DR: MME-CoF-Pro is a video reasoning benchmark with 303 samples across 16 categories to assess reasoning coherence in video generative models, revealing weak reasoning coherence despite good generation quality.


<details>
  <summary>Details</summary>
Motivation: Video generative models show emerging reasoning behaviors, but there's a gap in literature for evaluating reasoning coherence - ensuring generated events remain causally consistent across frames for reliable deployment.

Method: Proposed MME-CoF-Pro benchmark with 303 samples across 16 categories (visual logical to scientific reasoning), using Reasoning Score metric for process-level intermediate reasoning steps, with three evaluation settings: no hint, text hint, and visual hint.

Result: Evaluation of 7 open and closed-source video models shows: (1) weak reasoning coherence decoupled from generation quality, (2) text hints boost apparent correctness but cause inconsistency and hallucinated reasoning, (3) visual hints benefit structured perceptual tasks but struggle with fine-grained perception.

Conclusion: The benchmark reveals critical limitations in video models' reasoning coherence and provides insights into how different hint types affect reasoning performance, highlighting the need for improved reasoning capabilities in video generation.
...
TL;DR: Black-box prompt optimization techniques can systematically bypass LLM safety safeguards, revealing that static safety benchmarks underestimate risk and adaptive red-teaming is necessary for robust evaluation.


<details>
  <summary>Details</summary>
Motivation: Current LLM safety evaluations rely on fixed harmful prompt collections, overlooking realistic attack scenarios where adversaries iteratively refine inputs to evade safeguards. This creates a gap between static benchmarks and real-world adaptive threats.

Method: Repurposed black-box prompt optimization techniques (originally for benign tasks) to systematically search for safety failures. Used DSPy to apply three optimizers to prompts from HarmfulQA and JailbreakBench, optimizing toward a continuous danger score (0-1) provided by GPT-5.1 as independent evaluator.

Result: Substantial reduction in effective safety safeguards, especially pronounced for open-source small language models. Example: Qwen 3 8B average danger score increased from 0.09 (baseline) to 0.79 after optimization, demonstrating significant vulnerability to automated adversarial refinement.

Conclusion: Static benchmarks may underestimate residual risk in LLMs. Automated, adaptive red-teaming is a necessary component of robust safety evaluation, as demonstrated by the effectiveness of black-box prompt optimization in bypassing safety safeguards.
...
TL;DR: DuCCAE is a hybrid conversational engine that decouples real-time response generation from asynchronous agentic execution to balance responsiveness with long-horizon task capability, achieving significant improvements in user retention and task completion rates in Baidu Search deployment.


<details>
  <summary>Details</summary>
Motivation: Immersive conversational systems face a fundamental trade-off between real-time responsiveness and the ability to handle complex, long-horizon tasks involving planning and tool invocation. Heavy-tail execution latency from agentic operations degrades turn-taking, persona consistency, and user trust in production systems.

Method: DuCCAE proposes a hybrid architecture that decouples real-time response generation from asynchronous agentic execution, synchronizing them via a shared state that maintains session context and execution traces. The system orchestrates five subsystems (Info, Conversation, Collaboration, Augmentation, Evolution) to support multi-agent collaboration and continuous improvement.

Result: DuCCAE outperforms baselines in agentic execution reliability and dialogue quality while reducing latency to fit real-time budgets. Deployment in Baidu Search since June 2025 shows substantial effectiveness: Day-7 user retention tripled to 34.2% and complex task completion rate surged to 65.2%.

Conclusion: The hybrid architecture successfully preserves conversational continuity while enabling reliable agentic execution, offering practical guidelines for deploying scalable agentic systems in industrial settings. The approach effectively addresses the responsiveness vs. capability trade-off in production conversational systems.
...
TL;DR: Spelling correction significantly improves healthcare QA retrieval, with edit distance and context-aware methods achieving +9.2% MRR improvement, while corpus-only correction yields minimal benefits.


<details>
  <summary>Details</summary>
Motivation: Healthcare QA systems face high spelling error rates in user queries (61.5% of real medical queries contain errors), which degrade retrieval performance compared to professional documents.

Method: Conducted error census on two datasets (TREC 2017 LiveQA and HealthSearchQA), evaluated four correction methods (conservative edit distance, standard Levenshtein, context-aware ranking, SymSpell) across three experimental conditions using BM25 and TF-IDF retrieval over 1,935 MedQuAD passages.

Result: Query correction substantially improves retrieval: edit distance and context-aware methods achieve +9.2% MRR and +8.3% NDCG@10 improvements over baseline. Corpus-only correction yields only +0.5% MRR improvement. 61.5% of real medical queries contain at least one spelling error with 11.0% token-level error rate.

Conclusion: Query-side spelling correction is essential for healthcare QA systems, with edit distance and context-aware methods providing significant retrieval improvements, while corpus correction alone is insufficient.
...
TL;DR: StreamBench: A benchmark for evaluating language models in streaming environments using news events from 2016 and 2025, featuring three tasks to assess performance on concurrent event processing with structural cues.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for language models don't adequately evaluate performance in streaming environments where multiple concurrent events are mixed within document streams, creating conflicts that current evaluation methods don't address.

Method: Created StreamBench from major news stories in 2016 and 2025, comprising 605 events and 15,354 documents across three tasks: Topic Clustering, Temporal Question Answering, and Summarization. Introduced structural cues that organize key facts by event to diagnose model failures.

Result: Structural cues improved performance on clustering (up to +4.37%) and temporal QA (up to +9.63%), helping models locate relevant information and separate distinct events. Temporal reasoning remains challenging for current LLMs.

Conclusion: While temporal reasoning remains an open challenge for current LLMs, consistent performance gains across tasks with structural cues show they are a promising direction for improving language model performance in massive document streams.
...
TL;DR: Proposed Metadata Enriched Hybrid RAG and DPO to improve legal LLMs' reliability by enhancing document retrieval and enabling safe refusal when context is insufficient.


<details>
  <summary>Details</summary>
Motivation: LLMs degrade on long legal documents, producing hallucinations with incorrect clauses/precedents. RAG helps but remains limited in legal settings, especially with small, locally deployed models required for data privacy. Two failure modes identified: retrieval errors due to lexical redundancy in legal corpora, and decoding errors where models generate answers despite insufficient context.

Method: 1. Metadata Enriched Hybrid RAG to improve document-level retrieval. 2. Direct Preference Optimization (DPO) to enforce safe refusal when context is inadequate.

Result: Together, these methods improve grounding, reliability, and safety in legal language models.

Conclusion: The proposed approach addresses key limitations of LLMs in legal applications by combining enhanced retrieval with refusal mechanisms to reduce hallucinations and improve precision in legal document processing.
...
TL;DR: GeoChallenge is a 90K dataset of automatically generated multiple-choice geometry proof problems requiring multi-step reasoning over text and diagrams, revealing significant performance gaps between LLMs and humans.


<details>
  <summary>Details</summary>
Motivation: Existing geometry benchmarks are limited in scale and lack visually grounded multiple-choice questions, hindering reliable evaluation of complex symbolic reasoning in LLMs that requires integration of text and diagrams.

Method: Introduce GeoChallenge dataset with 90K automatically generated multiple-choice geometry proof problems, each requiring multi-step reasoning over aligned textual descriptions and diagrams, with fine-grained complexity ratings and formal language annotations.

Result: Experiments show clear performance gap: best-performing model GPT-5-nano achieves 75.89 exact match vs. 94.74 for humans, revealing three common LLM failure patterns: exact match failures, weak visual reliance, and overextended reasoning without convergence.

Conclusion: GeoChallenge enables controlled evaluation of symbolic reasoning in LLMs, revealing significant limitations in multi-step geometry proof solving despite models' strong performance on simpler tasks, highlighting the need for improved visual-text integration and reasoning convergence.
...
TL;DR: Comprehensive evaluation of LLMs (GPT-5.2, Llama 4, DeepSeek) on argument classification tasks using advanced prompting strategies, showing significant performance improvements but revealing systematic failure modes.


<details>
  <summary>Details</summary>
Motivation: Recent advances in LLMs have improved argument classification performance compared to traditional ML approaches, but there's a need for comprehensive evaluation of state-of-the-art models on large argument mining corpora using advanced prompting techniques.

Method: Evaluation of multiple LLMs (GPT-5.2, Llama 4, DeepSeek) on large argument classification corpora (Args.me, UKP) using advanced prompting strategies including Chain-of-Thought prompting, prompt rephrasing, voting, and certainty-based classification, with both quantitative metrics and qualitative error analysis.

Result: GPT-5.2 achieved best performance with 78.0% accuracy on UKP and 91.9% on Args.me. Advanced prompting techniques improved accuracy and F1 by 2-8 percentage points. Qualitative analysis revealed systematic failure modes including prompt instability, difficulty detecting implicit criticism, interpreting complex structures, and claim alignment issues.

Conclusion: This work provides the first comprehensive evaluation combining quantitative benchmarking and qualitative error analysis for LLMs on argument mining, demonstrating that while advanced prompting improves performance, systematic failure modes persist across models, highlighting areas for future research.
...
TL;DR: FinReasoning is a new benchmark for evaluating LLMs in financial research report generation, focusing on semantic consistency, data alignment, and deep insight through a fine-grained evaluation framework with hallucination-correction assessment and 12-indicator analytical skills rubric.


<details>
  <summary>Details</summary>
Motivation: Current LLM deployments in financial research reveal persistent failures (factual errors, numerical inconsistencies, fabricated references, shallow analysis) that can cause economic losses. Existing benchmarks focus on comprehension rather than report generation evaluation, and current frameworks only flag hallucinations without measuring deeper analytical skills.

Method: FinReasoning decomposes Chinese research-report generation into three stages aligned with real analyst workflows. It introduces a fine-grained evaluation framework that strengthens hallucination-correction assessment and incorporates a 12-indicator rubric for core analytical skills.

Result: Most models exhibit an understanding-execution gap: they can identify errors but struggle to generate accurate corrections; they can retrieve data but have difficulty returning it in correct format. No model achieves overwhelming superiority across all three tracks; Doubao-Seed-1.8, GPT-5, and Kimi-K2 rank as top three with distinct capability distributions.

Conclusion: FinReasoning addresses critical gaps in evaluating LLMs for financial research report generation, revealing systematic weaknesses in current models and providing a structured framework for assessing both hallucination correction and deep analytical capabilities.
...
TL;DR: LARFT is a training framework that improves LLMs' ability to follow length instructions by aligning their internal length cognition with generation actions through reinforcement learning with hindsight length awareness.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with precise output length control despite strong performance on complex tasks. Existing methods impose external constraints but fail to address the core problem: models' intrinsic deficit in length cognition.

Method: LARFT integrates length-oriented reinforcement learning with hindsight length awareness. It transforms on-policy data into hindsight self-awareness tasks where the model learns to identify the actual length of its own generation, jointly optimizing internal length representations and generation policy.

Result: LARFT outperforms existing baselines across four base models, achieving +20.92 average improvement on three length instruction following benchmarks with only -1.45 point decline on four general capability benchmarks.

Conclusion: The proposed LARFT framework effectively addresses LLMs' length control limitations by aligning internal length cognition with generation actions, enabling precise and reliable length instruction following without significantly compromising general capabilities.
...
TL;DR: The paper presents a system for Bengali speech processing tasks using data-centric approaches and fine-tuning pre-trained models, achieving competitive results despite limited resources.


<details>
  <summary>Details</summary>
Motivation: Bengali is severely under-served in ASR and speaker diarization research despite being spoken by over 230 million people, creating a need for effective solutions in low-resource settings.

Method: For Task 1 (ASR): Constructed training corpus from Bengali YouTube audiobooks/dramas using LLM-assisted language normalization, fuzzy-matching-based chunk boundary validation, and muffled-zone augmentation. Fine-tuned whisper-medium model on ~21,000 data points. For Task 2 (diarization): Fine-tuned pyannote.audio community-1 segmentation model with hyperparameter optimization using only 10 training files.

Result: Task 1: Achieved WER of 16.751 (public) and 15.551 (private). Task 2: Achieved DER of 0.19974 (public) and 0.26723 (private).

Conclusion: Careful data engineering and domain-adaptive fine-tuning can yield competitive performance for Bengali speech processing even without large annotated corpora, demonstrating effective approaches for low-resource languages.
...
TL;DR: LLM-based framework solves constrained path planning problems from natural language descriptions using template matching for known problems and in-context learning for novel ones, with iterative solution verification and refinement.


<details>
  <summary>Details</summary>
Motivation: Real-world path planning involves multiple constraints beyond simple route optimization, but traditional approaches require dedicated formulations for each problem variant, making them difficult to scale across diverse scenarios.

Method: Two-component framework: 1) For known problems, LLM matches input to pre-defined template library; 2) For novel problems, LLM infers problem representation from natural language and constructs formulation via in-context learning. Both use iterative solution generation/verification with genetic-algorithm-style refinement.

Result: The framework demonstrates capability to handle various constrained path planning problems, providing scalable and generalizable approach for real-world routing tasks with minimal human intervention.

Conclusion: LLM-based approach enables flexible problem specification through natural language while solving constrained path planning problems, offering a scalable alternative to traditional dedicated formulations.
...
TL;DR: MAPLE improves differentially private synthetic text generation for specialized domains by using private metadata extraction and in-context learning to better initialize the process, overcoming limitations of existing API-based methods.


<details>
  <summary>Details</summary>
Motivation: DP fine-tuning of LLMs is often infeasible with proprietary APIs, making DP synthetic data generation crucial. Existing methods like Private Evolution (PE) suffer from poor initialization when target data distributions deviate substantially from foundation model priors, especially in specialized domains.

Method: MAPLE (Metadata Augmented Private Language Evolution) uses differentially private tabular metadata extraction and in-context learning to ground the initial synthetic distribution in the target domain, addressing the initialization bottleneck of PE.

Result: Extensive experiments on challenging domain-specific text generation tasks show MAPLE achieves significantly better privacy-utility trade-off, faster convergence, and drastically reduced API costs compared to previous PE methods.

Conclusion: MAPLE effectively addresses the initialization problem in API-based DP synthetic data generation for specialized domains, offering improved utility, efficiency, and cost-effectiveness while maintaining privacy guarantees.
...
TL;DR: Breeze Taigi introduces a standardized benchmark framework for Taiwanese Hokkien speech recognition and synthesis, using parallel Mandarin-Taigi resources and achieving state-of-the-art ASR performance with 30.13% CER.


<details>
  <summary>Details</summary>
Motivation: Taiwanese Hokkien (Taigi) presents opportunities for advancing speech technology methodologies that can generalize to diverse linguistic contexts, but lacks standardized evaluation frameworks.

Method: Developed a reproducible evaluation methodology leveraging parallel Taiwanese Mandarin resources, curated 30 Mandarin-Taigi audio pairs with normalized transcriptions, established CER as standard metric, and implemented normalization procedures. Created speech models by fine-tuning Whisper on ~10,000 hours of Taigi synthetic speech data.

Result: ASR model achieves 30.13% average CER on the benchmark, outperforming existing commercial and research systems. Provides standardized evaluation protocols, diverse training datasets, and open baseline models.

Conclusion: Breeze Taigi offers a replicable framework with methodologies applicable to various linguistic contexts, advancing speech technology for under-resourced languages through standardized benchmarks and reproducible evaluation.
...
TL;DR: HATL framework adaptively unfreezes pretrained layers based on training behavior to improve sign language machine translation across diverse datasets.


<details>
  <summary>Details</summary>
Motivation: SLMT faces challenges including scarce datasets, limited signer diversity, domain gaps between sign motion patterns and pretrained representations, and static transfer learning approaches that lead to overfitting.

Method: Hierarchical Adaptive Transfer Learning (HATL) framework with progressive dynamic unfreezing of pretrained layers based on training performance, combined with layer-wise learning rate decay and stability mechanisms.

Result: HATL consistently outperforms traditional transfer learning across Sign2Text and Sign2Gloss2Text tasks on PHOENIX14T, Isharah, and MedASL datasets, with ADAT achieving BLEU-4 improvements of 15.0% on PHOENIX14T/Isharah and 37.6% on MedASL.

Conclusion: HATL provides an effective adaptive framework for SLMT that preserves pretrained structure while adapting to sign characteristics, demonstrating robust multilingual generalization across diverse sign language datasets.
...
TL;DR: Significance-Gain BPE improves subword tokenization by using statistical significance instead of raw frequency for merge selection, reducing perplexity by 12-13% and improving bits per character by ~1%.


<details>
  <summary>Details</summary>
Motivation: Standard BPE's reliance on raw pair frequency conflates true adjacency cohesion with pairs that are frequent due to high marginal counts, potentially leading to suboptimal tokenization for language modeling.

Method: Proposes Significance-Gain BPE, a drop-in alternative merge criterion that measures cohesion via a z-statistic under an independence null model and combines it with an explicit compression-aware gain term.

Result: At a representative operating point, reduces validation/test perplexity by 13%/12% and improves validation/test bits per character by 0.9-1.0%. Vocabulary-size sweep shows lower BPC in most closest-compression comparisons.

Conclusion: Statistically grounded merge selection can improve predictive efficiency per unit of raw text across a range of compression regimes, offering a better alternative to frequency-based BPE for language models.
...
TL;DR: LLMs exhibit a multiplicative scaling law (α-law) governing probability updates during iterative reasoning, with exponent values below 1 ensuring asymptotic stability under repeated revision.


<details>
  <summary>Details</summary>
Motivation: Current LLM reasoning methods (chain-of-thought, self-reflection, multi-agent debate) lack principled guarantees about the stability of their probability updates during iterative revision processes.

Method: Theoretical analysis establishes α-law governing probability updates, with empirical evaluation across 4,975 problems from graduate-level benchmarks (GPQA Diamond, TheoremQA, MMLU-Pro, ARC-Challenge) using GPT-5.2 and Claude Sonnet 4, plus token-level validation with Llama-3.3-70B.

Result: Models show near-Bayesian update behavior with exponents slightly above stability boundary in single-step revisions, but exponents decrease over successive revisions, producing contractive long-run dynamics consistent with stability predictions. GPT-5.2 shows balanced prior-evidence weighting while Claude modestly favors new evidence.

Conclusion: The α-law serves as a principled diagnostic for monitoring update stability and reasoning quality in LLM inference systems, characterizing observable inference-time behavior rather than internal Bayesian reasoning.
...
TL;DR: GAT is an uncertainty-aware active learning framework that uses LLMs as surrogates to select informative test samples for generative QA tasks, reducing labeling costs by ~40% compared to traditional methods.


<details>
  <summary>Details</summary>
Motivation: There's high demand for task-specific test sets to benchmark LLMs in specialized domains like healthcare, but expert labeling is expensive. Existing active learning frameworks don't work well for generative QA tasks where option dynamics affect decision boundaries.

Method: GAT uses LLMs as surrogates for sample selection with a Statement Adaptation Module that converts generative tasks into pseudo-classification format to capture sample-level uncertainties. Zero-shot acquisition functions identify informative unlabeled samples.

Result: The framework reduces estimation error by approximately 40% compared to traditional sampling baselines, providing a scalable solution for cost-effective model benchmarking.

Conclusion: GAT offers an effective uncertainty-aware acquisition framework for selecting informative test samples in generative QA tasks, significantly reducing labeling costs while maintaining benchmarking quality.
...
TL;DR: Fine-tuning LLMs on contradictory "impossible objects" suppresses creative synthesis and induces dogmatic responses, fracturing the latent space topology.


<details>
  <summary>Details</summary>
Motivation: To investigate the ontological consequences of training LLMs on logical contradictions and understand how such training affects the model's capacity for creative synthesis versus dogmatic exclusion.

Method: Trained Llama-3.1-8B with two adapters: Analytic (θ_A) on tautological definitions and Synthetic-Conflict (θ_S_conflict) on brute-force contradictions. Conducted 1,500 stratified trials and performed mechanistic interpretation using PCA projections, cosine similarity heatmaps, and scatter plots to analyze latent space structure.

Result: Conflict training caused dramatic "suppression of genesis": synthetic concept generation dropped from 9.0% to 1.0% (p<.0001). Instead, "Pick-One" dogmatism increased from 3.6% to 30.8%. Mechanistic analysis revealed a "topological schism" in the latent space, creating an inaccessible "void" that prevents traversal to synthetic solutions.

Conclusion: Training LLMs on logical contradictions without dialectical mediation forces models into dogmatic states that exclude creative synthesis, effectively lobotomizing their capacity for generating novel concepts through fractured latent space topology.
...
TL;DR: Novel distillation framework with Explanatory Inversion and Explanatory GRPO improves reasoning in smaller models by focusing on conceptual understanding rather than pattern memorization, achieving 20.39% improvement over zero-shot and 6.02% over SOTA baselines.


<details>
  <summary>Details</summary>
Motivation: Current distillation methods for transferring reasoning capabilities from large to small language models suffer from superficial pattern memorization and poor generalization, limiting their practical utility for computationally efficient deployment.

Method: Two-component framework: 1) Explanatory Inversion (EI) generates targeted "explanatory probes" that force students to articulate underlying logic; 2) Explanatory GRPO (EXGRPO) uses reinforcement learning with Dialogue Structure Utility Bonus to reward coherent reasoning processes across probes.

Result: Extensive evaluations on 12 datasets show 20.39% average improvement over zero-shot performance and 6.02% improvement over state-of-the-art distillation baselines using Gemma-7b as student model, with remarkable training efficiency (surpassing vanilla fine-tuning with 10-25% training data) and strong out-of-distribution generalization.

Conclusion: The proposed framework successfully addresses pattern memorization and generalization limitations in distillation by focusing on conceptual understanding through explanatory probes and reinforcement learning with structural coherence rewards, enabling more effective transfer of reasoning capabilities to smaller models.
...
TL;DR: The paper introduces an action-modeling framework (EAFD schema) for learning from hierarchical review workflows, addressing information asymmetry in correction signals through operational grounding and conflict-aware graph reasoning, achieving 96.3% alignment with human experts in e-commerce adjudication.


<details>
  <summary>Details</summary>
Motivation: Hierarchical review workflows generate valuable correction signals when second-tier reviewers correct first-tier decisions, but learning from these signals is hindered by information asymmetry - corrections often depend on verification actions unavailable to initial reviewers or automated systems.

Method: Proposes Evidence-Action-Factor-Decision (EAFD) schema for adjudication reasoning that prevents hallucination through operational grounding. Develops conflict-aware graph reasoning framework that: (1) constructs EAFD graphs from historical Maker-Checker disagreements, (2) aggregates them into retrievable knowledge base, (3) performs top-down deductive reasoning by projecting validated resolution paths from precedents, with distinctive RMI (Request More Information) capability for insufficient evidence cases.

Result: In e-commerce seller appeal adjudication: standard LLM-only baseline achieves 70.8% alignment with human experts; action modeling with RMI improves to 87.5%; retrieval-based knowledge graph augmentation yields best offline performance of 95.8%. Online deployment maintains robust performance with 96.3% alignment rate in production.

Conclusion: The EAFD schema and conflict-aware graph reasoning framework effectively address information asymmetry in hierarchical review workflows by grounding reasoning in verifiable operations, enabling learning from correction signals, and achieving near-expert alignment in real-world adjudication tasks.
...
TL;DR: This paper proposes a full-stack domain-enhanced LLM workflow for combustion science to address hallucinations in general-purpose LLMs, integrating automated corpus construction, incremental pre-training, instruction fine-tuning, and verifiable reward-based RL, with a new benchmark FlameBench showing superior performance over state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: General-purpose LLMs generate severe hallucinations in complex physical systems like combustion science due to insufficient domain knowledge and inability to adhere to physical conservation laws, limiting their application potential in professional fields.

Method: Proposes a full-stack domain-enhanced LLM workflow including: 1) automated domain corpus construction, 2) incremental pre-training, 3) instruction fine-tuning, and 4) verifiable reward-based reinforcement learning to ensure models internalize physical laws rather than just learning textual patterns.

Result: The developed model significantly outperforms state-of-the-art general-purpose closed-source models and traditional retrieval-augmented generation methods on combustion science reasoning tasks, as evaluated using the newly released FlameBench benchmark.

Conclusion: This work lays a solid technical and resource foundation for developing domain-specific scientific research agents with reliable scientific reasoning capabilities, demonstrating that domain-enhanced LLMs can overcome limitations of general-purpose models in specialized scientific fields.
...
TL;DR: A framework for understanding LLM components (pre-training data, tokenization, transformers, probabilistic generation, alignment, agentic capabilities) to critically assess their suitability for research applications, demonstrated through social media simulation case study.


<details>
  <summary>Details</summary>
Motivation: Researchers need to make informed decisions about using LLMs in their work, requiring understanding of LLM mechanisms without technical expertise to assess what LLMs can and cannot do effectively.

Method: Breaks down LLMs into six essential components: pre-training data, tokenization and embeddings, transformer architecture, probabilistic generation, alignment, and agentic capabilities. Analyzes each through technical foundations and research implications, identifying specific affordances and limitations. Develops a framework for reasoning critically about LLM suitability rather than prescriptive guidance.

Result: Provides a comprehensive framework for understanding LLM components and their implications for research. The extended case study on simulating social media dynamics with LLM-based agents demonstrates practical application of the framework.

Conclusion: The chapter offers researchers a non-technical yet comprehensive understanding of LLM mechanisms to make informed decisions about whether and how to use LLMs for specific research needs, moving beyond prescriptive guidance to critical reasoning frameworks.
...
TL;DR: Autonoma is a hierarchical multi-agent framework for end-to-end workflow automation from natural language prompts, featuring a Coordinator, Planner, and Supervisor that orchestrates specialized agents, achieving 97% task completion and 98% agent handoff success rates.


<details>
  <summary>Details</summary>
Motivation: Increasing complexity of user demands requires automation frameworks that can reliably translate open-ended instructions into robust multi-step workflows, addressing limitations of current monolithic agent architectures that struggle with scalability, error propagation, and maintaining focus across diverse tasks.

Method: Autonoma employs a principled multi-tiered architecture: a high-level Coordinator validates user intent, a Planner generates structured workflows, and a Supervisor dynamically manages execution by orchestrating modular specialized agents (web browsing, coding, file management). This separates orchestration logic from specialized execution, enabling robustness through active monitoring and error handling, and extensibility through plug-and-play agent integration.

Result: Implemented as a fully functional system operating within a secure LAN environment, Autonoma achieved a 97% task completion rate and a 98% successful agent handoff rate, confirming operational reliability and efficient collaboration. The system supports multi-modal input (text, voice, image, files) and both English and Arabic languages.

Conclusion: Autonoma provides a structured, hierarchical multi-agent framework that addresses critical data privacy and reliability concerns while enabling extensibility and inclusivity through multi-modal support and language flexibility, demonstrating high reliability in workflow automation from natural language prompts.
...
TL;DR: The paper presents a human-centered workflow for using LLMs via APIs as universal text processing machines for content analysis tasks including annotation, summarization, and information extraction, with validation procedures to address LLM limitations.


<details>
  <summary>Details</summary>
Motivation: While many researchers use LLMs through chat interfaces, their true potential lies in API-based integration for systematic research. Current LLM applications lack rigorous methodological frameworks for content analysis, and researchers need structured approaches to address LLM limitations like black-box nature, prompt sensitivity, and hallucinations.

Method: The paper conceptualizes LLMs as universal text processing machines and develops a comprehensive, human-centered workflow for three content analysis tasks: annotation (coding/labeling/classification), summarization, and information extraction. The approach synthesizes insights from methodological literature across political science, sociology, computer science, psychology, and management, with researchers designing, supervising, and validating each stage.

Result: The paper provides a structured workflow with validation procedures and best practices to address LLM limitations. Supplementary materials include a prompt library and Python code in Jupyter Notebook format with detailed usage instructions to facilitate practical implementation.

Conclusion: LLMs can be effectively leveraged via APIs as universal text processing tools for content analysis when used within a rigorous, human-centered methodological framework that addresses their limitations through systematic validation and researcher supervision.
...
TL;DR: Transformers are formally equivalent to stateless Differentiable Neural Computers (sDNCs), providing a unified memory-centric interpretation of attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: To establish a principled computational framework connecting Transformers and Differentiable Neural Computers, showing that modern attention-based architectures can be understood through a memory-centric perspective.

Method: Formal derivation demonstrating that causal Transformer layers correspond exactly to stateless DNCs where: (1) controller has no recurrent state, (2) external memory is write-once value matrix, (3) content-based addressing via keys implements attention, and (4) multi-head attention corresponds to multiple parallel read heads. Extension to cross-attention shows encoder-decoder Transformers are sDNCs with distinct read/write memories.

Result: Established formal equivalence between Transformers and stateless Differentiable Neural Computers, providing unified memory-centric interpretation of attention mechanisms and connecting modern LLMs to principled computational frameworks.

Conclusion: Transformers can be understood as stateless Differentiable Neural Computers, offering a memory-centric perspective that unifies attention-based architectures with established computational frameworks and contributes to principled understanding of large language models.
...
TL;DR: LSR is a benchmark measuring cross-lingual safety degradation in LLMs, showing refusal rates drop from 90% in English to 35-55% in West African languages.


<details>
  <summary>Details</summary>
Motivation: Current safety alignment in LLMs relies heavily on English training data, causing refusal mechanisms to fail when harmful intent is expressed in low-resource languages, creating cross-lingual safety vulnerabilities.

Method: Introduced LSR benchmark with dual-probe evaluation protocol (matched English and target-language probes) and Refusal Centroid Drift (RCD) metric to quantify loss of English refusal behavior in target languages.

Result: English refusal rates held at ~90%, but dropped to 35-55% across West African languages (Yoruba, Hausa, Igbo, Igala), with Igala showing most severe degradation (RCD = 0.55).

Conclusion: LSR reveals significant cross-lingual safety gaps in LLMs, provides systematic evaluation framework, and is available as open-source contribution to AI safety evaluation repositories.
...
TL;DR: CURE benchmark evaluates multimodal LLMs in clinical diagnostics by separating reasoning from retrieval, revealing models perform well with provided evidence but poorly when retrieving evidence themselves.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks evaluate MLLMs in end-to-end answering scenarios, limiting ability to disentangle multimodal reasoning from evidence retrieval proficiency in clinical diagnostics.

Method: Introduce CURE benchmark with 500 multimodal clinical cases mapped to physician-cited reference literature, evaluating reasoning and retrieval under controlled evidence settings across distinct evidence-gathering paradigms.

Result: Advanced models achieve up to 73.4% accuracy on differential diagnosis when supplied with physician reference evidence, but performance declines to as low as 25.4% when reliant on independent retrieval mechanisms.

Conclusion: The performance disparity highlights dual challenges of effectively integrating multimodal clinical evidence and retrieving precise supporting literature, with CURE benchmark publicly available for further research.
...
TL;DR: Proposes subdomain adaptation via mid-training for radiology report summarization, showing that clinical pre-training followed by radiology mid-training outperforms direct fine-tuning approaches.


<details>
  <summary>Details</summary>
Motivation: To reduce physician burden by improving automatic radiology report summarization, addressing limitations of direct fine-tuning approaches which face "cold start" problems and suboptimal adaptation to specialized domains.

Method: Three adaptation strategies: (1) general-domain pre-training, (2) clinical-domain pre-training, and (3) clinical pre-training followed by subdomain mid-training. Used UF Health clinical text for development, with mid-training and fine-tuning on OpenI and MIMIC-CXR benchmark datasets.

Result: Mid-trained model GatorTronT5-Radio achieved best performance, outperforming non-mid-trained models in both ROUGE-L (text-based) and RadGraph-F1 (factuality) measures. Mid-training also demonstrated better few-shot learning and alleviated "cold start" problems.

Conclusion: Supports "pre-training, mid-training, fine-tuning" strategy over direct fine-tuning for radiology report summarization, showing improved adaptation to specialized subdomains and better handling of data scarcity challenges.
...
TL;DR: GraphRAG framework outperforms standard RAG for automated short answer grading by using structured knowledge graphs to capture concept dependencies and enable multi-hop reasoning.


<details>
  <summary>Details</summary>
Motivation: Standard RAG approaches treat knowledge as isolated fragments, failing to capture structural relationships and multi-hop reasoning essential for complex educational content, while LLMs struggle with hallucinations and strict rubric adherence.

Method: Dual-phase GraphRAG framework: Microsoft GraphRAG for high-fidelity knowledge graph construction and HippoRAG neurosymbolic algorithm for associative graph traversals to retrieve comprehensive, connected subgraphs of evidence.

Result: Significantly outperforms standard RAG baselines across all metrics on NGSS dataset; HippoRAG achieved substantial improvements in evaluating Science and Engineering Practices, confirming superiority of structural retrieval for logical reasoning chains.

Conclusion: Structural retrieval via knowledge graphs provides superior performance for automated short answer grading by capturing concept dependencies and enabling multi-hop reasoning essential for higher-order academic assessment.
...
TL;DR: MOSAIC is a modular framework for review summarization that decomposes the process into interpretable components, validated through online A/B tests showing improved customer experience and measurable business value.


<details>
  <summary>Details</summary>
Motivation: Existing summarization research emphasizes end-to-end quality but overlooks benchmark reliability and practical utility of granular insights for real-world deployment in online marketplaces.

Method: MOSAIC decomposes summarization into interpretable components: theme discovery, structured opinion extraction, opinion clustering, and grounded summary generation, creating a scalable modular framework for industrial deployment.

Result: Online A/B tests show improved customer experience and measurable value; offline experiments demonstrate superior aspect coverage and faithfulness; opinion clustering enhances faithfulness in noisy review conditions; reliability limitations in SPACE dataset identified.

Conclusion: MOSAIC provides a practical, interpretable approach to review summarization with demonstrated business impact, while addressing benchmark reliability issues through new dataset creation (TRECS) and highlighting the importance of opinion clustering for faithfulness.
...
TL;DR: LoRA-based adaptation achieves calibration parity with full fine-tuning while maintaining parameter efficiency, with a trade-off between calibration improvement and task accuracy when constraining adaptation space.


<details>
  <summary>Details</summary>
Motivation: Transformer-based models often suffer from miscalibration (overconfident predictions), and this work investigates whether parameter-efficient adaptation methods like LoRA can maintain or improve calibration compared to full fine-tuning.

Method: Evaluated LoRA and a novel hyper-network-based adaptation framework on RoBERTa across GLUE benchmark tasks. The hyper-network approach dynamically generates LoRA factors (A and B matrices) with structural coupling across layers. Analyzed calibration using metrics including ECE, MCE, and ACE.

Result: LoRA-based adaptation consistently achieves calibration parity with (and sometimes exceeds) full fine-tuning while maintaining parameter efficiency. The hyper-network approach produced similar results to standard LoRA, with better MCC on CoLA. Constraining adaptation space (e.g., freezing matrix A) improves ECE but requires balancing with downstream task accuracy.

Conclusion: Structured low-rank updates provide a viable foundation for uncertainty-aware Transformer architectures, clarifying the relationship between parameter efficiency and probabilistic reliability. The work provides reproducible calibration metrics implementation for future research.
...
TL;DR: Survey paper providing comprehensive guide to multilingual hate speech detection and counterspeech generation, addressing limitations of English-centric models and proposing structured framework for building inclusive systems.


<details>
  <summary>Details</summary>
Motivation: Online hate speech requires multilingual approaches beyond English-centric models to capture cultural and linguistic diversity; monolingual systems fail in non-English and code-mixed contexts, missing implicit hate and culturally specific expressions.

Method: Structured three-phase framework: task design, data curation, and evaluation, integrating state-of-the-art datasets, models, and metrics; comprehensive survey of multilingual resources and techniques with emphasis on ethical and cultural considerations.

Result: Consolidates progress in multilingual hate speech detection while highlighting persistent obstacles: data scarcity in low-resource languages, fairness and bias issues, need for multimodal solutions; provides scalable guidelines for context-aware systems.

Conclusion: Bridges technical progress with ethical considerations to advance online safety through fairer, more effective detection and counterspeech generation across diverse linguistic environments; offers roadmap for researchers, practitioners, and policymakers.
...
TL;DR: This paper examines validity evidence requirements for generative AI scoring of constructed responses compared to traditional feature-based AI and human scoring, proposing best practices for validity argumentation in high-stakes testing contexts.


<details>
  <summary>Details</summary>
Motivation: The increasing application of generative AI in high-stakes testing contexts, particularly for scoring constructed responses, creates a need for comprehensive validity evidence frameworks due to the technology's lack of transparency and unique concerns like consistency.

Method: The authors compare validity evidence requirements across three scoring systems (human ratings, feature-based NLP AI, generative AI), propose best practices for validity evidence collection, and demonstrate their application using a large corpus of argumentative essays from 6-12th grade students.

Result: Generative AI scoring systems require more extensive validity evidence than feature-based systems due to transparency and consistency concerns. The paper demonstrates how to collect validity evidence for different scoring systems and highlights complexities in validity argumentation for AI-generated scores.

Conclusion: As generative AI becomes more prevalent in high-stakes testing, comprehensive validity evidence frameworks are essential to support score interpretation and use, requiring more rigorous validation than traditional feature-based AI scoring systems.
...
TL;DR: URAG benchmark evaluates RAG systems' uncertainty beyond correctness, showing accuracy-uncertainty trade-offs vary with retrieval noise and domain, with simple modular RAGs often outperforming complex reasoning pipelines.


<details>
  <summary>Details</summary>
Motivation: Current RAG evaluations focus mainly on correctness, lacking comprehensive assessment of uncertainty and reliability. There's a need to understand how retrieval affects LLM uncertainty across diverse domains.

Method: URAG benchmark reformulates open-ended generation into multiple-choice QA for principled uncertainty quantification via conformal prediction. Evaluates 8 standard RAG methods using accuracy and prediction-set sizes based on LAC and APS metrics across healthcare, programming, science, math, and general text domains.

Result: (1) Accuracy gains often coincide with reduced uncertainty, but relationship breaks under retrieval noise; (2) Simple modular RAG methods offer better accuracy-uncertainty trade-offs than complex reasoning pipelines; (3) No single RAG approach is universally reliable across domains; (4) Retrieval depth, parametric knowledge dependence, and confidence cues can amplify confident errors and hallucinations.

Conclusion: URAG establishes a systematic benchmark for analyzing and enhancing trustworthiness of retrieval-augmented systems, revealing important trade-offs and limitations in current RAG approaches that go beyond simple correctness metrics.
...
TL;DR: LLM prompt framing significantly influences decisions in threshold voting tasks, with different framings of logically equivalent prompts shifting preferences toward risk-averse options, revealing framing effects as a bias source in non-interacting multi-agent deployments.


<details>
  <summary>Details</summary>
Motivation: To examine how prompt framing influences LLM decisions in threshold voting tasks involving individual-group interest conflicts, particularly in non-interacting multi-agent settings where LLMs operate independently without coordination.

Method: Tested two logically equivalent prompts with different framings across diverse LLM families under isolated trials in a threshold voting task involving individual-group interest conflict.

Result: Prompt framing significantly influences choice distributions, often shifting preferences toward risk-averse options. Surface linguistic cues can override logically equivalent formulations, revealing behavior consistent with instrumental rather than cooperative rationality when success requires risk-bearing.

Conclusion: Framing effects represent a significant bias source in non-interacting multi-agent LLM deployments, with implications for alignment and prompt design to mitigate these effects.
...
TL;DR: First computational approach to motif indexing using Arabian Nights with El-Shamy's motif index, achieving 0.85 F1 with fine-tuned Llama3 model.


<details>
  <summary>Details</summary>
Motivation: Motifs are important narrative elements in folklore and modern texts, but automated motif indexing and detection has been challenging. Need computational methods to identify motif expressions in original folkloristic texts for both folkloristic analysis and understanding modern usage.

Method: Used Arabian Nights paired with El-Shamy's motif index to overcome accessibility issues. Created manually annotated corpus of 2,670 motif expressions across 58,450 sentences. Tested five approaches: (1) retrieve & re-rank with keywords and fine-tuned cross-encoder, (2) off-the-shelf embedding models, (3) fine-tuned embedding models, (4) generative prompting of off-the-shelf LLMs in N-shot setups, (5) generative approaches on LLMs fine-tuned with LoRA.

Result: Best performing system was fine-tuned Llama3 model achieving overall performance of 0.85 F1 score.

Conclusion: Successfully demonstrated first computational approach to motif indexing, showing that fine-tuned LLMs can effectively detect motif expressions in folkloristic texts, overcoming previous challenges in automated motif analysis.
...
TL;DR: Review paper analyzing how task-oriented human-human conversational data can be used for collaboration analysis, covering theories, coding schemes, tasks, and modeling approaches.


<details>
  <summary>Details</summary>
Motivation: Collaboration is a fundamental high-level human behavior where conversation serves as the primary medium for information exchange. Conversational data represents a valuable but underutilized resource for automatically analyzing collaborative processes, particularly in task-oriented contexts.

Method: Conducted a comprehensive review of collaboration analysis using task-oriented conversation resources, examining four key areas: 1) related theoretical frameworks, 2) coding schemes for analyzing collaborative conversations, 3) specific tasks and applications, and 4) computational modeling approaches.

Result: The review synthesizes existing research on utilizing conversational data for collaboration analysis, identifying current methodologies and approaches for extracting collaborative insights from task-oriented human-human dialogues.

Conclusion: The paper serves as a practical resource for researchers and illuminates unexplored areas for future work in collaboration analysis, addressing the central question of how to effectively leverage task-oriented conversational data for understanding collaborative processes.
...
TL;DR: LLM-MRD is a teacher-student framework that distills complex multi-view reasoning from LLMs into an efficient student model for multimodal fake news detection, achieving significant performance improvements over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal fake news detection methods have limitations: they lack comprehensive multi-view judgment and fusion, and LLM-based approaches suffer from prohibitive computational inefficiency due to high reasoning costs.

Method: Proposes LLM-MRD with a Student Multi-view Reasoning module that constructs comprehensive foundations from textual, visual, and cross-modal perspectives, and a Teacher Multi-view Reasoning module that generates deep reasoning chains. Uses a Calibration Distillation mechanism to efficiently distill complex reasoning-derived knowledge into the efficient student model.

Result: LLM-MRD significantly outperforms state-of-the-art baselines, achieving comprehensive average improvements of 5.19% in ACC and 6.33% in F1-Fake across all competing methods and datasets.

Conclusion: The proposed LLM-MRD framework effectively addresses limitations of existing approaches by combining comprehensive multi-view reasoning with efficient knowledge distillation, demonstrating superior performance in multimodal fake news detection.
...
TL;DR: PrefPO is a preference-based prompt optimization method that reduces labeled data requirements and improves prompt hygiene by using LLM discriminators for pairwise preference feedback.


<details>
  <summary>Details</summary>
Motivation: Prompt engineering is effective but labor-intensive, and existing automated optimization methods require labeled datasets (often unavailable) and produce verbose, repetitive prompts.

Method: PrefPO uses a preference-based approach inspired by RLHF, requiring only a starting prompt and natural language criteria. It employs an LLM discriminator to express pairwise preferences over model outputs and provide feedback to an LLM optimizer for iterative improvement.

Result: PrefPO matches or exceeds SOTA methods on 6/9 BBH tasks and performs comparably to TextGrad on IFEval-Hard (82.4% vs 84.5%). It works in both labeled and unlabeled settings, reduces prompt length issues by 3-5x, and is less susceptible to prompt hacking (37% vs 86% for TextGrad).

Conclusion: PrefPO provides an effective, minimal approach to prompt optimization that reduces data requirements, improves prompt hygiene, and is more robust against gaming evaluation criteria compared to existing methods.
...
TL;DR: Proposes Memory-Driven Role-Playing paradigm inspired by acting theory, with evaluation framework (MREval), prompting architecture (MRPrompt), and bilingual benchmark (MRBench) to improve LLM persona consistency in long dialogues.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with faithful role-playing in long, open-ended dialogues due to inconsistent characterization and failure to recall/apply persona knowledge without explicit cues, requiring a more rigorous approach to test depth and autonomous knowledge use.

Method: Memory-Driven Role-Playing paradigm framing persona knowledge as internal memory store requiring context-based retrieval; includes MREval (4-ability evaluation framework), MRPrompt (structured memory retrieval prompting architecture), and MRBench (bilingual benchmark).

Result: MRPrompt enables small models (Qwen3-8B) to match performance of larger closed-source LLMs (Qwen3-Max, GLM-4.7); upstream memory gains directly enhance downstream response quality, validating the staged theoretical foundation across 12 tested LLMs.

Conclusion: The Memory-Driven Role-Playing paradigm provides comprehensive diagnostic for role-playing abilities, demonstrating that structured memory retrieval approaches can significantly improve persona consistency and enable smaller models to achieve competitive performance.
...
TL;DR: PROMPTATTRIB is a low-resource entity matching method that uses attribute-level prompt tuning with fuzzy logic reasoning and contrastive learning to improve accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: Traditional supervised EM requires expensive labeled data, while recent prompt-tuning methods focus only on entity-level matching, lack interpretability, and ignore attribute-level information.

Method: PROMPTATTRIB uses both entity-level and attribute-level prompts, employs fuzzy logic formulas for inference, and integrates dropout-based contrastive learning on soft prompts inspired by SimCSE.

Result: Extensive experiments on real-world datasets demonstrate the effectiveness of PROMPTATTRIB in improving EM performance with minimal labeled data.

Conclusion: PROMPTATTRIB addresses limitations of existing low-resource EM methods by incorporating attribute-level information, improving interpretability, and enhancing matching accuracy through prompt tuning and logical reasoning.
...
TL;DR: A two-stage prompt routing architecture that uses automated task discovery and task-aware quality estimation to dynamically select the most appropriate LLM from a pool of frontier models, achieving better performance than the strongest individual model at less than half the cost.


<details>
  <summary>Details</summary>
Motivation: Existing prompt routing approaches face challenges as model pools scale: manually defined task taxonomies cannot capture fine-grained capability distinctions, and monolithic routers struggle to differentiate subtle performance differences across diverse tasks among frontier models with narrow performance gaps.

Method: Two-stage routing architecture: 1) Graph-based clustering discovers latent task types and trains a classifier for task assignment; 2) Mixture-of-experts architecture with task-specific prediction heads for specialized quality estimation. Inference aggregates predictions from both stages to balance task-level stability with prompt-specific adaptability.
...
Conclusion: The proposed two-stage routing architecture effectively addresses limitations of existing approaches through automated fine-grained task discovery and task-aware quality estimation, enabling efficient and accurate model selection from large pools of frontier LLMs.
...
TL;DR: Probe-based methods for detecting evaluation awareness in LLMs primarily track benchmark structure rather than evaluation context, limiting their evidential value.


<details>
  <summary>Details</summary>
Motivation: To determine whether linear probes on benchmark prompts truly detect evaluation awareness or merely capture surface structural artifacts of benchmark formats.

Method: Used controlled 2x2 dataset and diagnostic rewrites to test probe signals under partial control of prompt format, examining generalization to free-form prompts independent of linguistic style.

Result: Probes primarily track benchmark-canonical structure and fail to generalize to free-form prompts, indicating they capture structural artifacts rather than evaluation context.

Conclusion: Standard probe-based methodologies do not reliably disentangle evaluation context from structural artifacts, limiting the evidential strength of existing results on evaluation awareness in LLMs.
...
TL;DR: Transformer models trained on synthetic word-order variants show vocabulary structure, not free/fixed word-order dichotomy, is the key predictor of computational learnability.


<details>
  <summary>Details</summary>
Motivation: To understand why some languages permit free word order while others have fixed word order, and to identify the computational factors that make word order more or less learnable for language models.

Method: Pretrained transformer language models on synthetic word-order variants of natural languages, systematically manipulating word-order regularity and measuring model surprisal as an indicator of learnability.

Result: Greater word-order irregularity consistently raises model surprisal (reduced learnability), but sentence reversal has weak effects. The free/fixed word-order distinction doesn't explain cross-lingual variation; instead, vocabulary structure strongly predicts model surprisal.

Conclusion: Vocabulary structure, not the traditional free vs. fixed word-order classification, emerges as the primary driver of computational word-order learnability across languages.
...
TL;DR: LLM-based policy synthesis framework generates Python policies via iterative prompting with performance feedback; dense feedback (reward + social metrics) outperforms sparse feedback in social dilemmas, especially for complex coordination tasks.


<details>
  <summary>Details</summary>
Motivation: To develop a framework for generating programmatic agent policies using LLMs instead of traditional reinforcement learning, enabling more interpretable and controllable policy synthesis for multi-agent environments, particularly in social dilemmas where coordination and cooperation are crucial.

Method: Iterative prompting of LLMs to produce Python policy functions, evaluating them in self-play, and refining using performance feedback; compares sparse feedback (scalar reward only) against dense feedback (reward plus social metrics: efficiency, equality, sustainability, peace); tested on two Sequential Social Dilemmas (Gathering and Cleanup) with Claude Sonnet 4.6 and Gemini 3.1 Pro.

Result: Dense feedback consistently matches or exceeds sparse feedback on all metrics; advantage largest in Cleanup public goods game where social metrics help calibrate costly cleaning-harvesting tradeoff; social metrics serve as coordination signals guiding LLMs toward effective cooperative strategies (territory partitioning, adaptive role assignment, avoidance of wasteful aggression); adversarial experiments reveal five attack classes for reward hacking.

Conclusion: LLM policy synthesis with dense social feedback enables more effective cooperative strategies in social dilemmas; social metrics function as coordination signals rather than triggering fairness over-optimization; framework reveals tension between expressiveness and safety due to reward hacking vulnerabilities.
...
TL;DR: Novel decoding algorithm for LLMs that enables sustained creativity and diversity in exploratory search quests, producing many conceptually unique results without accessing model internals.


<details>
  <summary>Details</summary>
Motivation: Current LLM decoding methods fail to support exploratory "search quests" where users need to learn search spaces and evaluate many diverse alternatives over extended periods. Existing approaches produce homogeneous results, repeat themselves too quickly, or offer uniform creativity that doesn't adapt to individual user needs.

Method: Developed a novel, easy-to-implement decoding scheme that induces sustained creativity and diversity in LLMs. The algorithm works without access to the LLM's inner vector space and unlocks both orthodox and heterodox knowledge beyond modal decoding paths.

Result: The approach enables LLMs to produce as many conceptually unique results as desired, allowing search quest users to more quickly explore search spaces and find satisfying answers. It overcomes limitations of current decoding methods that return homogeneous, conventional results.

Conclusion: The novel decoding algorithm successfully addresses the challenge of sustained creativity in exploratory search quests, enabling LLMs to leverage their vast knowledge to generate diverse, conceptually unique outputs that support extended learning and evaluation processes.
...
TL;DR: EvidenceRL is a reinforcement learning framework that reduces hallucinations in LLMs by enforcing evidence adherence through grounding and correctness scoring, achieving significant improvements in high-stakes domains like cardiac diagnosis and legal reasoning.


<details>
  <summary>Details</summary>
Motivation: LLMs are prone to hallucinations, producing plausible but unsupported answers, which is especially problematic in high-stakes domains where decisions must be justified by verifiable evidence.

Method: EvidenceRL uses reinforcement learning to enforce evidence adherence during training. It scores candidate responses for grounding (entailment with retrieved evidence and context) and correctness (agreement with reference answers), then optimizes the generator using Group Relative Policy Optimization (GRPO).

Result: On cardiac diagnosis: F1@3 increased from 37.0 to 54.5 on Llama-3.2-3B; grounding (G_max@3) rose from 47.6 to 78.2; hallucinations dropped nearly 5×; evidence-supported diagnoses increased from 31.8% to 61.6%. On legal reasoning: Faithfulness increased from 32.8% to 67.6% on Llama-3.1-8B.

Conclusion: EvidenceRL consistently improves evidence grounding and faithfulness without sacrificing task accuracy across high-stakes domains, demonstrating effective behavioral change in LLMs to reduce hallucinations through evidence-based reinforcement learning.
...
TL;DR: FDARxBench: An expert-curated benchmark for evaluating document-grounded QA using FDA drug labels, featuring factual, multi-hop, and refusal tasks to assess LLM performance on regulatory-grade drug label comprehension.


<details>
  <summary>Details</summary>
Motivation: Current language models struggle with accurate question answering on drug labels due to their rich but heterogeneous clinical and regulatory information. The benchmark addresses the need for evaluating LLMs on FDA generic drug assessment tasks and provides challenging regulatory-grade evaluation of label comprehension.

Method: Developed in collaboration with FDA regulatory assessors, using a multi-stage pipeline for generating high-quality, expert-curated QA examples. The benchmark spans factual, multi-hop, and refusal tasks, with evaluation protocols designed to assess both open-book and closed-book reasoning.

Result: Experiments across proprietary and open-weight models reveal substantial gaps in factual grounding, long-context retrieval, and safe refusal behavior. Models show significant limitations in handling the complexity of drug label information.

Conclusion: FDARxBench provides a substantial foundation for challenging regulatory-grade evaluation of label comprehension and supports evaluation of LLM behavior on drug-label questions, addressing critical needs in FDA generic drug assessment while offering broader applications for document-grounded QA evaluation.
...
TL;DR: Reasoning strategies for LLMs don't universally improve text classification performance and are often inefficient, with complex methods sometimes degrading performance while increasing token costs 10-100x for minimal gains.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the underexplored question of whether reasoning mechanisms (like CoT, ToT, etc.) truly benefit text classification tasks, given their substantial token and time costs. There's an implicit assumption that deliberative reasoning uniformly benefits heterogeneous NLP tasks, but this hasn't been systematically tested for classification.

Method: Introduced TextReasoningBench, a systematic benchmark to evaluate reasoning strategies for text classification with LLMs. Compared seven reasoning strategies (IO, CoT, SC-CoT, ToT, GoT, BoC, and long-CoT) across ten LLMs on five text classification datasets. Used traditional metrics (accuracy, macro-F1) plus two novel cost-aware metrics: performance gain per reasoning token and efficiency of performance improvement relative to token cost growth.

Result: Three key findings: (1) Reasoning doesn't universally improve classification - moderate strategies (CoT, SC-CoT) yield limited gains (+1-3% on big models), while complex methods (ToT, GoT) often fail to outperform simpler baselines and can degrade performance, especially on small models; (2) Reasoning is often inefficient - many strategies increase token consumption 10-100x (e.g., SC-CoT, ToT) while providing only marginal performance improvements.

Conclusion: The blanket application of reasoning strategies to text classification tasks is questionable. While moderate reasoning can provide limited benefits on large models, the substantial token costs often outweigh performance gains, and complex reasoning methods may even harm performance. The findings challenge the assumption that deliberative reasoning uniformly benefits all NLP tasks.
...
TL;DR: BEAVER is a training-free framework for compressing long LLM contexts using hierarchical selection instead of token pruning, achieving comparable performance to SOTA methods with 26.4x latency reduction on 128k contexts.


<details>
  <summary>Details</summary>
Motivation: The exponential expansion of LLM context windows has created severe bottlenecks in inference latency and information utilization, while existing compression methods suffer from high training costs or semantic fragmentation from aggressive token pruning.

Method: BEAVER shifts compression from linear token removal to structure-aware hierarchical selection, using dual-path pooling to map variable-length contexts into dense page-level tensors, and preserves discourse integrity through a hybrid planner combining semantic and lexical dual-branch selection with sentence smoothing.

Result: Extensive evaluations on four long-context benchmarks show BEAVER achieves comparable performance to SOTA methods like LongLLMLingua, maintains high fidelity in multi-needle retrieval on RULER benchmark where baselines deteriorate, and reduces latency by 26.4x on 128k contexts.

Conclusion: BEAVER offers a scalable, training-free solution for high-throughput long-context applications by maximizing hardware parallelism through hierarchical selection while preserving discourse integrity.
...
TL;DR: Novel prompt engineering framework for trait-specific Arabic AES using LLMs with three-tier prompting strategy achieves best results with Fanar-1-9B-Instruct using rubric-guided prompting.


<details>
  <summary>Details</summary>
Motivation: Addresses scarcity of scalable, linguistically informed AES tools for Arabic language assessment in low-resource educational contexts.

Method: Three-tier prompting strategy (standard, hybrid, rubric-guided) for LLMs in zero-shot and few-shot configurations; hybrid approach simulates multi-agent evaluation with trait specialist raters; rubric-guided method incorporates scored exemplars.

Result: Fanar-1-9B-Instruct achieves highest trait level agreement (QWK=0.28, CI=0.41) in both zero and few-shot prompting; rubric-guided prompting yields consistent gains across all traits and models; discourse-level traits (Development and Style) show greatest improvements.

Conclusion: Structured prompting, not model scale alone, enables effective AES in Arabic; presents first comprehensive framework for proficiency-oriented Arabic AES and foundation for scalable assessment in low-resource contexts.
...
TL;DR: DATAPROPHET: A training-free metric for predicting dataset influence on multimodal LLM performance before training, outperforming intuitive task similarity and training-based methods.


<details>
  <summary>Details</summary>
Motivation: Current MLLM supervision data selection relies on intuitive similarity to target benchmarks, but this approach's reliability is unclear. The paper aims to develop a method to estimate dataset influence on target benchmarks before training occurs.

Method: Analyzed transfer across 14 vision-language datasets spanning 7 tasks, then proposed DATAPROPHET - a training-free metric combining multimodal perplexity, similarity, and data diversity to rank supervision datasets.

Result: Intuitive task similarity is unreliable for predicting transferability. DATAPROPHET achieves 86.0% Kendall's tau correlation with actual post-training performance gains, yielding up to 6.9% improvement over uniform selection and 1.4% over training-based baselines.

Conclusion: DATAPROPHET provides an effective training-free approach for predicting dataset influence on MLLM performance, enabling better supervision data selection without expensive training experiments.
...
TL;DR: EvoTaxo is an LLM-based framework for building and evolving taxonomies from temporally ordered social media streams using structured draft actions, dual-view clustering, and refinement procedures to handle noisy, dynamic discourse.


<details>
  <summary>Details</summary>
Motivation: Social media posts are short, noisy, semantically entangled, and temporally dynamic, making taxonomy construction challenging. Existing methods designed for static corpora struggle with balancing robustness, scalability, and sensitivity to evolving discourse.

Method: EvoTaxo converts each post into structured draft actions over current taxonomy, accumulates structural evidence over time windows, uses dual-view clustering (semantic similarity + temporal locality) to consolidate candidate edits, and employs refinement-and-arbitration with concept memory banks to preserve semantic boundaries.

Result: Experiments on two Reddit corpora show EvoTaxo produces more balanced taxonomies than baselines with clearer post-to-leaf assignment, better corpus coverage at comparable taxonomy size, and stronger structural quality. Case study on /r/ICE_Raids demonstrates meaningful temporal discourse shift capture.

Conclusion: EvoTaxo effectively addresses challenges of taxonomy induction from dynamic social media streams by combining LLM-based structured actions, temporal evidence accumulation, and refinement mechanisms to produce high-quality evolving taxonomies.
...
TL;DR: First systematic study on detecting AI-generated fabricated scientific tables in NLP papers, introducing FabTab benchmark and TAB-AUDIT framework with within-table mismatch feature, achieving 0.987 AUROC in-domain.


<details>
  <summary>Details</summary>
Motivation: AI-generated fabricated scientific manuscripts pose serious threats to academic integrity, with tables serving as critical evidence for claims in empirical papers, necessitating detection methods.

Method: Constructed FabTab benchmark dataset (1,173 AI-generated + 1,215 human-authored NLP papers), identified systematic differences between fabricated and real tables, operationalized them into discriminative features within TAB-AUDIT framework, with key feature being within-table mismatch (perplexity gap between table skeleton and numerical content).

Result: RandomForest classifier using TAB-AUDIT features significantly outperforms prior state-of-the-art methods, achieving 0.987 AUROC in-domain and 0.883 AUROC out-of-domain, demonstrating high detection accuracy.

Conclusion: Experimental tables provide critical forensic signals for detecting AI-generated scientific fraud, with TAB-AUDIT framework offering effective detection capabilities and FabTab serving as benchmark for future research.
...
TL;DR: LoopRPT is a reinforcement pre-training framework for looped language models that assigns RL signals directly to latent reasoning steps, enabling more efficient iterative computation and better accuracy-computation trade-offs.


<details>
  <summary>Details</summary>
Motivation: Existing RL paradigms for language models focus on output tokens, creating a structural mismatch with looped architectures where reasoning occurs implicitly in latent representations. There's a need for RL methods that can directly shape intermediate reasoning steps in iterative models.

Method: LoopRPT reframes next-token prediction as next-token reasoning and assigns reinforcement signals directly to latent steps using an EMA teacher reference and noisy latent rollouts. This enables RL to directly shape intermediate representations and compress reasoning into fewer iterations.

Result: LoopRPT consistently improves per-step representation quality and achieves Pareto dominance in accuracy-computation trade-offs across multiple model scales. Significant gains on hard tokens indicate enhanced early-stage reasoning rather than just encouraging premature exits.

Conclusion: Reinforcement pre-training is a principled paradigm for learning efficient latent reasoning in looped language models, enabling better compression of reasoning into fewer iterations while maintaining or improving accuracy.
...
TL;DR: PoC introduces performance-oriented context compression for LLMs, using a predictor to find the most aggressive compression ratio that meets a specified performance floor, outperforming fixed-ratio methods.


<details>
  <summary>Details</summary>
Motivation: Existing context compression methods with fixed target ratios cause unpredictable performance degradation, making reliable deployment difficult. There's a need for compression that guarantees performance rather than just achieving compression ratios.

Method: PoC uses a lightweight performance predictor to automatically find the most aggressive compression ratio satisfying a user-specified performance constraint, then applies an off-the-shelf compressor. Two predictor variants: context-agnostic (simple) and context-aware (considers input compressibility).

Result: Context-aware predictor achieves lower performance prediction error than context-agnostic predictor on QA and summarization benchmarks. Context-aware PoC attains superior overall performance compared to fixed-ratio compression methods.

Conclusion: PoC enables more reliable, efficient, and performance-aware deployment of context compression for LLMs by shifting from compression-ratio-oriented to performance-oriented compression.
...
TL;DR: MLLM benchmarking should account for human label variation (agreement/disagreement) rather than just consensus labels, as larger models excel on high-agreement tasks but underperform on ambiguous content where human annotators disagree.


<details>
  <summary>Details</summary>
Motivation: Human label variation (systematic differences among annotators' judgments) remains underexplored in benchmarks despite rapid LLM development. Current benchmarks based solely on consensus labels may overstate model capabilities in subjective domains like content moderation.

Method: Introduce an evaluation protocol for MLLM benchmarking that explicitly accounts for two conditions: (1) human label agreement and (2) disagreement. Apply this protocol to two state-of-the-art MLLM families (Gemma 3, Qwen 2.5 VL) using non-aggregated human annotations from a social media content classification dataset.

Result: Larger models tend to perform best on high-agreement subsets, yet often underperform medium-sized models when human disagreement is high. Parameter count alone does not determine sensitivity to ambiguity and subjectivity.

Conclusion: Benchmarks based solely on consensus labels can overstate model capabilities in subjective domains. Incorporating human label variation yields more realistic and robust assessments of MLLMs in content moderation pipelines.
...
TL;DR: The paper investigates how multilingual language models represent code-mixed text, finding they align English and Hindi well but code-mixed inputs remain loosely connected to either language. The authors introduce a trilingual alignment objective that improves cross-lingual representation and downstream task performance.


<details>
  <summary>Details</summary>
Motivation: Despite widespread use of multilingual encoder-based language models for code-mixed analysis tasks, there's limited understanding of how they internally represent code-mixed inputs and whether these representations meaningfully connect to the constituent languages being mixed.

Method: Constructed a unified trilingual corpus of parallel English, Hindi (Devanagari), and Romanized code-mixed sentences. Used CKA, token-level saliency, and entropy-based uncertainty analysis to probe cross-lingual representation alignment across standard multilingual encoders and their code-mixed adapted variants. Introduced a trilingual post-training alignment objective to bring code-mixed representations closer to both constituent languages simultaneously.
...
Conclusion: Grounding code-mixed representations in their constituent languages meaningfully helps cross-lingual understanding. The introduced trilingual post-training alignment objective successfully brings code-mixed representations closer to both constituent languages, leading to improved cross-lingual alignment and better performance on downstream tasks.
...
TL;DR: A relational analogy approach for FrameNet Semantic Role Classification using binary classification with minimal ANN parameters, achieving SOTA without explicit role training.


<details>
  <summary>Details</summary>
Motivation: To develop a computationally efficient method for Semantic Role Classification in FrameNet that avoids explicit semantic role training while achieving state-of-the-art performance.

Method: Define analogies as formal relations over frame evoking lexical units and frame element pairs, construct a dataset with binary labels (valid/invalid analogies), train a lightweight ANN for binary classification, and recover semantic roles during inference through probability distributions computed via random sampling and analogical transfer.
...
Conclusion: Relational analogy formulation enables effective Semantic Role Classification without explicit role training, demonstrating that analogical reasoning can achieve superior performance with computational efficiency.
...
TL;DR: The paper introduces "semantic delta" - a lightweight metric based on thematic intensity distribution to distinguish human-written from LLM-generated dialogue, finding that LLMs exhibit stronger thematic concentration than human discourse.


<details>
  <summary>Details</summary>
Motivation: To develop an interpretable statistical feature for distinguishing human-written and LLM-generated dialogue, addressing the broader question of whether LLMs talk like humans and contributing to understanding LLM behavioral mimicry.

Method: Uses Empath lexical analysis to map texts to thematic intensity scores, defines semantic delta as difference between two most dominant category intensities, evaluates hypothesis using conversational data from multiple LLM configurations compared against heterogeneous human corpora (scripted dialogue, literary works, online discussions), and applies Welch t-test to resulting distributions.

Result: AI-generated texts consistently produce higher semantic deltas than human texts, indicating more rigid topic structure, while human dialogue displays broader and more balanced semantic spread. The metric provides a computationally inexpensive complementary signal for detection systems.

Conclusion: Thematic distribution constitutes a quantifiable dimension where current LLMs fall short of human conversational dynamics. The semantic delta metric offers a zero-shot, interpretable feature that can complement existing detection techniques in ensemble systems.
...
TL;DR: This paper addresses the lack of reliable meta-evaluation methods for MT error detection systems, proposes a robust evaluation strategy called MPP, and uses it to assess state-of-the-art error detection capabilities.


<details>
  <summary>Details</summary>
Motivation: While MT and automatic evaluation have advanced significantly, there's no established technique to reliably measure the evaluation capabilities of auto-evaluators that perform error detection, making it unclear how to assess these systems properly.

Method: The paper investigates different implementations of span-level precision, recall, and F-score, showing their limitations, and proposes "match with partial overlap and partial credit" (MPP) with micro-averaging as a robust meta-evaluation strategy.

Result: The study reveals that seemingly similar evaluation approaches can yield substantially different rankings of MT error detection systems, and that certain widely-used techniques are unsuitable for this task. The proposed MPP method is released as publicly available code.

Conclusion: MPP with micro-averaging provides a robust meta-evaluation framework for MT error detection, enabling reliable assessment of state-of-the-art systems and addressing a critical gap in the evaluation methodology for this important NLP task.
...
TL;DR: Translation as Information Bottleneck optimization: source sentences as stimuli, target sentences as compressed meanings, showing human translations cluster near optimal accuracy-complexity frontier for spatial prepositions across English, German, Serbian.


<details>
  <summary>Details</summary>
Motivation: To test whether the Information Bottleneck framework applies to linguistic stimuli (words in sentential context), specifically whether translation exhibits communicative efficiency pressures, as previous IB studies focused on visual domains like color and motion but not linguistic contexts.

Method: Framed translation as IB optimization problem; used bitexts from French novel translations; conducted pile-sorting pilot study (N=35) for preposition similarity judgments; trained low-rank projection model (D=5) to predict judgments; compared attested translations to counterfactual alternatives relative to IB frontier.

Result: Low-rank model achieved Spearman correlation of 0.78 for predicting preposition similarity judgments; attested translations lie closer to IB optimal frontier than counterfactual alternatives, providing evidence for communicative efficiency pressure in human translation of spatial prepositions.

Conclusion: Translation can serve as a window into cognitive efficiency pressures shaping cross-linguistic semantic systems; human translators exhibit communicative efficiency in spatial domain, extending IB framework to linguistic stimuli in context.
...
TL;DR: SAGE is a sustainable framework that uses RL agents to curate compact, high-quality training data for low-resource language translation, achieving SOTA performance while reducing data usage by 97.1% and energy consumption by 95.2%.


<details>
  <summary>Details</summary>
Motivation: Address the linguistic divide in low-resource Southeast Asian communities by overcoming dual challenges: scarcity of culturally relevant data and prohibitive energy costs of training LLMs on massive web corpora, while balancing digital inclusion with environmental sustainability.

Method: Sustainable Agent-Guided Expert-tuning (SAGE) framework uses reinforcement learning agent optimized via Group Relative Policy Optimization (GRPO) to autonomously curate compact training sets. The agent uses semantic reward signals from expert-constructed community dialogues to filter noise and cultural misalignment, then fine-tunes open-source LLMs on curated data using Low-Rank Adaptation (LoRA).

Result: Achieved new state-of-the-art performance on BLEU-4 and COMET-22 metrics for English to seven low-resource Southeast Asian language translation tasks. Surpassed baselines trained on full datasets while reducing data usage by 97.1% and training energy consumption by 95.2%.

Conclusion: SAGE provides a scalable, responsible pathway to bridge the digital divide in the Global South by delivering high-performance translation models with minimal environmental footprint through energy-aware data curation prioritizing quality over quantity.
...
TL;DR: A hybrid topic modeling framework combining LDA with sPLS-DA for analyzing thematic structure and longitudinal dynamics in narrative poetry, demonstrated on Pushkin's "Evgenij Onegin" using an Italian translation.


<details>
  <summary>Details</summary>
Motivation: To develop a computational framework for literary analysis that integrates unsupervised and supervised methods to model thematic structure in narrative poetry, addressing small-corpus instability while maintaining interpretability and reproducibility.

Method: Hybrid framework combining Latent Dirichlet Allocation (LDA) for unsupervised topic modeling with sparse Partial Least Squares Discriminant Analysis (sPLS-DA) as a supervised probe. The text is segmented into 35 documents of lemmatised content words, using a multi-seed consensus protocol for small-corpus stability. Narrative hubs (contiguous stanza groups) extend bag-of-words to narrative level analysis.

Result: Five stable and interpretable topics emerged from the analysis. The sPLS-DA enhanced interpretability by identifying lexical markers that refined each theme. Narrative hubs revealed how thematic mixtures align with the poem's emotional and structural arc, demonstrating convergence of unsupervised and supervised lexical structures.

Conclusion: The framework offers a computational form of close reading that complements traditional literary interpretation, showing how lightweight probabilistic models can yield reproducible thematic maps of complex poetic narratives. Despite using a single lemmatised translation, the approach provides a transparent methodological template applicable to other high-density literary texts in comparative studies.
...
TL;DR: LLMs show dissociation between detecting speaker unreliability in judgments vs. using it for efficient clarification in interactive block-building tasks.


<details>
  <summary>Details</summary>
Motivation: To investigate how language models separate literal interpretation from contextual inference in collaborative tasks, and whether they can efficiently resolve ambiguity through contextual inference or clarification requests.

Method: Introduces Build What I Mean (BWIM), an interactive benchmark based on a two-speaker psycholinguistic paradigm contrasting pragmatically cooperative vs. literally reliable speakers. Models must resolve ambiguity by either performing contextual inference or requesting clarification at a communication cost.

Result: Models detect speaker unreliability in explicit confidence ratings but fail to exploit this information for efficient clarification behavior. Instead, they exhibit suboptimal strategies: partner-blind over-clarification and question-averse guessing under uncertainty.

Conclusion: State-of-the-art LLMs show a dissociation between judgment and action in contextual meaning construction, revealing limitations in their ability to strategically use pragmatic information for efficient communication in interactive settings.
...
TL;DR: A multi-agent framework improves XAI narrative faithfulness and coherence through iterative refinement with critic agents and ensemble strategies.


<details>
  <summary>Details</summary>
Motivation: Existing XAI methods are too technical and expert-oriented, creating a need for more accessible natural-language explanations. LLM-generated XAI narratives show promise but require improvement in faithfulness and coherence.

Method: Proposes a multi-agent framework with Narrator agent generating/revising narratives based on feedback from multiple Critic Agents evaluating faithfulness and coherence. Five agentic systems are designed (Basic, Critic, Critic-Rule, Coherent, Coherent-Rule) and evaluated across five LLMs on five tabular datasets, with ensemble strategy using majority voting.

Result: Basic, Critic, and Critic-Rule designs effectively improve narrative faithfulness across all LLMs. Claude-4.5-Sonnet with Basic Design reduces unfaithful narratives by 90% after three iterations. Ensemble strategy enhances performance for four LLMs (except DeepSeek-V3.2-Exp).

Conclusion: Agentic systems demonstrate significant potential for producing faithful and coherent XAI narratives, with iterative refinement and ensemble strategies effectively addressing faithfulness issues in LLM-generated explanations.
...
TL;DR: RouterKGQA is a hybrid KGQA framework that combines specialized and general models to improve answer accuracy while minimizing LLM usage through selective repair and constraint-aware filtering.


<details>
  <summary>Details</summary>
Motivation: Existing KGQA approaches have trade-offs: retrieval-based methods using small specialized models are efficient but produce unreachable paths and miss implicit constraints, while agent-based methods using large general models achieve better structural grounding but at substantially higher computational cost.

Method: RouterKGQA introduces specialized-general model collaboration where a specialized model generates reasoning paths, and a general model performs KG-guided repair only when needed. It adds constraint-aware answer filtering to reduce redundant answers and designs a more efficient general agent workflow to lower inference costs.
...
Conclusion: The proposed framework successfully balances performance and efficiency by leveraging the strengths of both specialized and general models through selective collaboration, achieving state-of-the-art results with minimal LLM usage.
...
TL;DR: LoASR-Bench is a comprehensive benchmark for evaluating low-resource ASR performance of SpeechLMs across 25 languages from 9 language families, revealing significant limitations in handling real-world low-resource scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus primarily on high-resource languages, leaving SpeechLM performance in low-resource languages insufficiently understood. This gap hinders deployment in real-world multilingual scenarios where reliable support for diverse language families is essential.

Method: Proposed LoASR-Bench benchmark comprising 25 languages from 9 language families, featuring both Latin and non-Latin scripts to enable cross-linguistic and cross-script assessment of SpeechLM ASR performance.

Result: Experimental results highlight significant limitations of current SpeechLMs in handling real-world low-resource languages, demonstrating the need for improved generalization across diverse language families.

Conclusion: LoASR-Bench addresses a critical gap in SpeechLM evaluation by providing a comprehensive benchmark for low-resource ASR, revealing current model limitations and emphasizing the importance of cross-linguistic and cross-script assessment for real-world deployment.
...
TL;DR: The paper investigates how verbal and nonverbal linguistic features can predict listener understanding states in explanatory dialogues, finding that information value, syntactic complexity, and gaze variation correlate with understanding levels and improve classification performance.


<details>
  <summary>Details</summary>
Motivation: To develop methods for predicting listener understanding states in real-time during explanatory interactions by analyzing linguistic cues related to cognitive load, which could enhance adaptive dialogue systems and educational technologies.

Method: Analyzed the MUNDEX corpus of face-to-face board game explanations using statistical analysis of three linguistic cues: speaker utterance information value (surprisal), syntactic complexity, and listener interactive gaze variation. Conducted classification experiments with two off-the-shelf classifiers and a fine-tuned German BERT-based multimodal classifier using listener self-annotated states (Understanding, Partial Understanding, Non-Understanding, Misunderstanding) obtained via retrospective video-recall.

Result: Individual linguistic cues varied significantly with listener understanding levels. Classification experiments demonstrated that prediction of the four understanding states is generally possible, with performance improving when the three linguistic cues are combined with textual features in multimodal classification.

Conclusion: Verbal and nonverbal linguistic features, particularly those related to cognitive load (information value, syntactic complexity, gaze variation), can effectively predict listener understanding states in explanatory dialogues, supporting the development of more responsive and adaptive dialogue systems.
...
TL;DR: DPO provides small, task-dependent gains over SFT in small-scale models; full fine-tuning consistently outperforms LoRA, with parameterization being the primary performance factor rather than preference optimization techniques.


<details>
  <summary>Details</summary>
Motivation: To systematically evaluate Direct Preference Optimization (DPO) behavior under small-scale conditions (small backbones and modest data) compared to supervised fine-tuning (SFT), examining whether DPO provides meaningful gains and how parameterization methods (full fine-tuning vs LoRA) affect performance.

Method: Comparative study of SFT-only, DPO-only, and staged SFT-to-DPO training on GPT-2-scale decoder models, evaluating both full fine-tuning (FFT) and LoRA parameterization. Tasks include paraphrase detection and Shakespearean sonnet continuation. Analysis focuses on performance differences across training regimes and parameterization methods.

Result: DPO yields small, task-dependent gains over strong SFT baselines. DPO can match competitive SFT accuracy without warm start when preference construction closely parallels supervised objective. Full fine-tuning consistently outperforms LoRA at matched training depth. LoRA does not reduce wall-clock time on tested hardware. Parameterization dominates performance differences.

Conclusion: In small-scale regimes, supervised full-parameter adaptation remains the primary performance lever, while preference optimization (DPO) and low-rank adaptation (LoRA) provide limited marginal returns. The choice of parameterization method (FFT vs LoRA) has greater impact than preference optimization techniques.
...
TL;DR: LLMs like ChatGPT-5 struggle with accurate translation of technical syntax terminology from English to Arabic, achieving only 25% accuracy, highlighting the need for AI-linguist collaboration.


<details>
  <summary>Details</summary>
Motivation: To evaluate the capability of Large Language Models (LLMs) to accurately translate specialized grammatical and syntactic terminology from English to Arabic, specifically examining whether LLMs can effectively handle the complex semantic and syntactic challenges inherent in technical linguistic terminology.

Method: Collected 44 technical terms from generative syntax literature, obtained human translations as reference, then used ChatGPT-5 to translate the same terms. Employed analytical and comparative approach to evaluate translation accuracy across three categories: accurate, inaccurate, and partially correct/appropriate.

Result: ChatGPT-5 achieved only 25% accurate translations, 38.6% inaccurate translations, and 36.4% partially correct translations (considered appropriate). The findings demonstrate significant limitations in LLMs' ability to handle specialized syntactic terminology with its embedded semantic and syntactic complexities.

Conclusion: LLMs currently cannot effectively 'talk much' about core syntax properties when translating technical terminology. The study recommends close collaboration between AI specialists and linguists to improve LLMs' mechanisms for more accurate translation of specialized linguistic content.
...
TL;DR: BOULDER benchmark reveals substantial performance gap between isolated reasoning tasks and dialogue-based reasoning in task-oriented dialogue settings, highlighting the need for realistic interactive evaluation of LLMs.


<details>
  <summary>Details</summary>
Motivation: Current LLM reasoning benchmarks focus on isolated tasks that don't reflect real-world task-oriented dialogue usage, where models must reason while generating text and adhering to instructions on role, format, and style. This mismatch raises concerns about whether benchmark performance accurately reflects reasoning robustness in practical TOD settings.

Method: Introduce BOULDER, a dynamic benchmark covering eight travel-related tasks requiring arithmetic, spatial, and temporal reasoning with both commonsense and formal aspects. Each problem is presented in both isolated and dialogue-based variants for controlled comparison while mitigating data contamination. Experiments conducted on eight LLMs with ablations and qualitative analysis.
...
Conclusion: There is a significant mismatch between isolated reasoning benchmarks and realistic task-oriented dialogue performance. The findings highlight the need to evaluate LLM reasoning in realistic interactive scenarios rather than relying solely on isolated task benchmarks.
...
TL;DR: The paper proposes an attention-enhanced HAL model that replaces mean pooling with temperature-scaled additive attention, achieving 6.74% accuracy improvement on IMDB sentiment analysis by focusing on sentiment-bearing tokens.


<details>
  <summary>Details</summary>
Motivation: Standard mean pooling in HAL models causes information loss by assigning equal weight to all tokens, diluting the impact of contextually salient words with uninformative structural tokens.

Method: Integrates a learnable, temperature-scaled additive attention mechanism into HAL representation pipeline, with Truncated SVD applied to project sparse co-occurrence matrices into dense latent space before attention layer.

Result: Achieves 82.38% test accuracy on IMDB sentiment analysis, a 6.74 percentage point improvement over mean pooling baseline (75.64%). Attention weights successfully suppress stop-words and attend to sentiment-bearing tokens.

Conclusion: Attention-based pooling improves both classification performance and model interpretability over traditional mean pooling in HAL models, demonstrating the value of selective weighting for distributional semantic representations.
...
TL;DR: STC is an efficient uncertainty quantification method for LLMs that uses semantic token clustering to identify unreliable outputs without repeated sampling or auxiliary models.


<details>
  <summary>Details</summary>
Motivation: LLMs often produce unreliable outputs with overconfidence, and existing uncertainty quantification methods require substantial computational overhead through repeated sampling or auxiliary models.

Method: Semantic Token Clustering (STC) groups tokens into semantically consistent clusters using embedding clustering and prefix matching, then quantifies uncertainty based on probability mass aggregated over corresponding semantic clusters.

Result: STC achieves performance comparable to state-of-the-art baselines while substantially reducing computational overhead, requiring only a single generation without auxiliary models.

Conclusion: STC provides an efficient uncertainty quantification approach that leverages inherent semantic information in LLMs, offering a practical solution for identifying unreliable outputs with minimal computational cost.
...
TL;DR: Instruction-tuned LLMs struggle to balance user alignment with evidence faithfulness under epistemic conflict, with richer evidence not preventing sycophancy and robustness scaling non-monotonically.


<details>
  <summary>Details</summary>
Motivation: To evaluate the tension between user-alignment pressures and faithfulness to in-context evidence in instruction-tuned language models operating in contested domains like climate science.

Method: Introduced a controlled epistemic-conflict framework using U.S. National Climate Assessment evidence, conducted fine-grained ablations over evidence composition and uncertainty cues across 19 instruction-tuned models (0.27B to 32B parameters).

Result: Three primary failure modes: 1) negative partial-evidence interaction where adding epistemic nuance (research gaps) increases sycophancy susceptibility; 2) non-monotonic robustness scaling with certain models being especially sensitive; 3) varying distributional concentration under conflict, with reasoning-distilled variants showing higher dispersion.

Conclusion: Richer in-context evidence alone doesn't guarantee resistance to user pressure; explicit training for epistemic integrity is needed to maintain evidence faithfulness in contested domains.
...
TL;DR: Chain-of-thought faithfulness measurements are not objective; different classifiers produce significantly different results, making cross-study comparisons invalid.


<details>
  <summary>Details</summary>
Motivation: To demonstrate that chain-of-thought faithfulness is not an objective, measurable property of models, contrary to current research practices that report single aggregate numbers.

Method: Applied three different classifiers (regex-only detector, two-stage regex-plus-LLM pipeline, and independent Claude Sonnet 4 judge) to 10,276 influenced reasoning traces from 12 open-weight models spanning 9 families and 7B to 1T parameters.

Result: Classifiers produced significantly different faithfulness rates (74.4%, 82.6%, and 69.7%) with non-overlapping confidence intervals. Per-model gaps ranged from 2.6 to 30.6 percentage points, all statistically significant. Classifier choice reversed model rankings, and inter-classifier agreement was low (kappa 0.06-0.42).
...
TL;DR: Proposes expert prefetching for MoE models to overlap CPU-GPU transfers with computation by predicting future experts from internal representations, reducing inference latency by up to 14%.


<details>
  <summary>Details</summary>
Motivation: MoE models require expert weights to be offloaded to CPU in memory-constrained inference settings, creating a CPU-GPU transfer bottleneck during decoding.

Method: Expert prefetching scheme that uses current internal model representations to speculate future experts, enabling memory transfers to overlap with computation. Includes lightweight estimators for improved expert prediction hit rates when needed.

Result: Demonstrates reliable prediction of future experts from internal representations across multiple MoE architectures. Achieves up to 14% reduction in time per output token (TPOT) over on-demand loading from CPU memory.

Conclusion: Expert prefetching effectively reduces MoE inference latency by overlapping memory transfers with computation while generally maintaining downstream task accuracy, with optional lightweight estimators for improved prediction accuracy.
...
TL;DR: A novel visualization method for comparing regression models that plots residuals in 2D space, uses Mahalanobis distance to account for correlations and scale differences, and employs colormaps to show percentile-based error distributions, providing more detailed performance insights than traditional aggregate metrics.


<details>
  <summary>Details</summary>
Motivation: Traditional regression performance metrics (MAE, RMSE, R²) aggregate too much information and obscure important patterns in model performance. While useful for distinguishing between poorly and well-performing models, they lack the detail needed for comprehensive model comparison and deeper understanding of error distributions.

Method: Three main contributions: (1) Visualizing residuals in 2D space for simultaneous evaluation of two models' errors, (2) Using Mahalanobis distance to account for data correlations and scale differences, and (3) Employing colormaps to visualize percentile-based error distributions, highlighting dense regions and outliers.

Result: The visualization approach provides a more detailed and comprehensive view of regression model performance, enabling users to uncover patterns that traditional aggregate metrics obscure. It facilitates deeper understanding of performance differences and error distributions between models.

Conclusion: The proposed visualization method enhances regression model evaluation and comparison by graphically representing error distributions and correlations, offering more detailed insights than traditional numerical metrics alone.
...
TL;DR: MIPO is a self-supervised preference optimization method that constructs contrastive preference pairs by generating positive responses conditioned on correct prompts and negative responses on random prompts, enabling LLM self-improvement without external data or supervision.


<details>
  <summary>Details</summary>
Motivation: Current LLM improvement relies heavily on expensive human-labeled data or external verifiers, which limits scalability and doesn't address tasks that aren't easily verifiable. There's a need for self-improvement frameworks that allow models to improve autonomously without external oversight.

Method: Mutual Information Preference Optimization (MIPO) constructs contrastive preference pairs by generating a positive response conditioned on the correct prompt and a negative response conditioned on a random, unrelated prompt. Using Direct Preference Optimization (DPO) on this paired data maximizes pointwise conditional mutual information between prompts and model responses under the base LLM.
...
Conclusion: MIPO provides an effective self-supervised approach for LLM improvement, demonstrating that contrastive data augmentation through mutual information maximization enables models to improve on both personalization and reasoning tasks without external oversight, suggesting a promising direction for autonomous self-improvement.
...
TL;DR: Proposes a subtype-guided contrastive learning framework that models patient heterogeneity as latent subtypes to guide discriminative representation learning for mental disorder analysis.


<details>
  <summary>Details</summary>
Motivation: Mental disorder populations exhibit pronounced heterogeneity (significant differences between samples), which poses a significant challenge to defining positive pairs in contrastive learning. This heterogeneity needs to be addressed to improve representation learning for mental disorders.

Method: Constructs multi-view representations combining clinical text with graph structure adaptively learned from BOLD signals. Uses unsupervised spectral clustering to uncover latent subtypes. Proposes a dual-level attention mechanism to construct prototypes capturing stable subtype-specific connectivity patterns. Introduces a subtype-guided contrastive learning strategy that pulls samples toward their subtype prototype graph to reinforce intra-subtype consistency.

Result: Evaluated on Major Depressive Disorder (MDD), Bipolar Disorder (BD), and Autism Spectrum Disorders (ASD). Experimental results confirm the effectiveness of subtype prototype graphs in guiding contrastive learning and demonstrate that the proposed approach outperforms state-of-the-art approaches.

Conclusion: The proposed subtype-guided contrastive learning framework successfully addresses patient heterogeneity in mental disorders by modeling latent subtypes as structural priors, leading to improved discriminative representation learning and superior performance compared to existing methods.
...
TL;DR: Test-Time Quantization (TTQ) framework compresses large foundation models at inference time with online calibration to handle domain shifts in unseen downstream tasks, achieving both adaptation and inference speedup.


<details>
  <summary>Details</summary>
Motivation: Existing activation-aware compression methods for large foundation models rely heavily on calibration data, making them vulnerable to domain shift issues when applied to unseen downstream tasks, limiting their practical deployment.

Method: Proposes a Test-Time Quantization (TTQ) framework that performs activation-aware quantization on-the-fly during inference using efficient online calibration, enabling adaptation to each prompt regardless of downstream task while maintaining inference speedup.

Result: Experimental results demonstrate that TTQ improves quantization performance over state-of-the-art baselines, effectively handling domain shifts and maintaining model quality across diverse downstream tasks.

Conclusion: TTQ provides a practical solution for compressing large foundation models by enabling adaptive quantization at inference time, overcoming calibration data dependency and domain shift limitations of existing methods while achieving computational efficiency.
...
TL;DR: CLaRE is a lightweight representation-level technique that identifies ripple effects in model editing by quantifying fact entanglement using forward activations from a single intermediate layer, avoiding costly backward passes.


<details>
  <summary>Details</summary>
Motivation: Large language models' static knowledge representations become outdated or incorrect over time. Model-editing techniques can modify factual associations but often produce unpredictable ripple effects (unintended behavioral changes that propagate to hidden space). There's a need to identify where these ripple effects occur without expensive computational overhead.

Method: CLaRE quantifies entanglement between facts using forward activations from a single intermediate layer, avoiding backward passes. The authors prepare and analyze a corpus of 11,427 facts from three existing datasets. Using CLaRE, they compute large-scale entanglement graphs capturing how local edits propagate through representational space.

Result: CLaRE achieves 62.2% average improvement in Spearman correlation with ripple effects compared to baselines, while being 2.74× faster and using 2.85× less peak GPU memory. It requires only a fraction of the storage needed by baselines to compute and preserve fact representations.

Conclusion: CLaRE provides an efficient method for identifying ripple effects in model editing, enabling stronger preservation sets, audit trails, efficient red-teaming, and scalable post-edit evaluation. The entanglement graphs and corpus are publicly available for further research.
...
TL;DR: Dynamic Bayesian machine learning framework (DBML SA) fuses probabilistic reasoning and neural networks to quantitatively model operator situation awareness in nuclear control rooms, achieving 13.8% prediction error and enabling real-time cognitive monitoring.


<details>
  <summary>Details</summary>
Motivation: Existing situation awareness assessment methods (SAGAT, SART) are static, retrospective, and detached from evolving cognitive dynamics in complex nuclear control environments, limiting their ability to capture operational risk drivers.

Method: Developed DBML SA framework combining Bayesian probabilistic reasoning with neural networks. Used 212 operational event reports (2007-2021) to reconstruct causal temporal structure of 11 performance shaping factors across cognitive layers. Bayesian component enables time-evolving inference under uncertainty; neural component establishes nonlinear predictive mapping from PSFs to SART scores.

Result: Achieved mean absolute percentage error of 13.8% with statistical consistency to subjective evaluations (p > 0.05). Identified training quality and stress dynamics as primary drivers of situation awareness degradation. Framework enables real-time cognitive monitoring, sensitivity analysis, and early-warning prediction.

Conclusion: DBML SA transcends traditional questionnaire-based assessments by providing quantitative, interpretable, and predictive situation awareness modeling, paving the way for intelligent human-machine reliability management in next-generation digital main control rooms.
...
TL;DR: PRIME-CVD is a synthetic medical dataset for cardiovascular risk modeling education, created from population statistics rather than real patient data to avoid privacy issues while maintaining realistic risk patterns.


<details>
  <summary>Details</summary>
Motivation: Real patient-level electronic medical record data is rarely available for teaching or methodological development due to privacy, governance, and re-identification risks, limiting reproducibility, transparency, and hands-on training in cardiovascular risk modeling.

Method: Created two synthetic data assets representing 50,000 adults undergoing primary prevention for CVD, generated entirely from a user-specified causal directed acyclic graph parameterized using publicly available Australian population statistics and published epidemiologic effect estimates, rather than from patient-level EMR data or trained generative models.

Result: Data Asset 1 provides a clean, analysis-ready cohort for exploratory analysis, stratification, and survival modeling, while Data Asset 2 restructures the same cohort into a relational, EMR-style database with realistic structural and lexical heterogeneity, enabling instruction in data cleaning, harmonization, causal reasoning, and policy-relevant risk modeling without privacy risks.

Conclusion: PRIME-CVD preserves realistic subgroup imbalance and risk gradients while ensuring negligible disclosure risk, supporting reproducible research and scalable medical education through openly accessible synthetic data that mimics real-world cardiovascular risk patterns without exposing sensitive information.
...
TL;DR: STEU is a parameter-efficient machine unlearning method for clinical language models that updates only selected token embeddings and a small classifier head, achieving near-complete forgetting of target classes while preserving model utility with minimal parameter modification.


<details>
  <summary>Details</summary>
Motivation: Clinical language models need machine unlearning capabilities to comply with privacy regulations and institutional policies requiring removal of sensitive information without full retraining, balancing effective forgetting with model utility preservation and minimal parameter changes.

Method: STEU (Sparse Token Embedding Unlearning) is a parameter-efficient method for behavioral class-level unlearning that freezes all encoder layers and updates only PMI-selected token embeddings together with a small classifier head, modifying only a tiny fraction of model parameters.

Result: Across experiments on MIMIC-IV, MIMIC-III, and eICU datasets using BioClinicalBERT, BERT-base, and DistilBERT, STEU consistently suppresses target classes while preserving retained task performance, achieving near-complete forgetting (forget F1 = 0.0004) with competitive retained utility (retain avg F1 = 0.4766) after modifying only 0.19% of parameters.

Conclusion: Targeted behavioral unlearning can be achieved through sparse embedding edits without modifying deeper encoder representations, providing an efficient solution for privacy-compliant clinical language models.
...
<details>
  <summary>Details</summary>
Motivation: Existing Transformer-based methods struggle to learn complex interactions among functional brain subnetworks due to limited training samples, creating a need for approaches that can effectively incorporate prior knowledge to guide the learning process.

Method: KD-Brain introduces a Semantic-Conditioned Interaction mechanism that injects semantic priors into attention queries to explicitly navigate subnetwork interactions based on functional identities, and a Pathology-Consistent Constraint that regularizes model optimization by aligning learned interaction distributions with clinical priors.

Result: The framework achieves state-of-the-art performance on a wide range of disorder diagnosis tasks and identifies interpretable biomarkers consistent with psychiatric pathophysiology.

Conclusion: KD-Brain successfully addresses the challenge of learning subnetwork interactions with limited data by explicitly encoding prior knowledge, leading to improved diagnostic performance and clinically meaningful biomarker discovery.
...
TL;DR: GT-Space is a scalable collaborative perception framework for heterogeneous autonomous agents that uses ground-truth labels to create a common feature space, eliminating the need for pairwise feature alignment and enabling single adapter modules per agent.


<details>
  <summary>Details</summary>
Motivation: Multi-agent collaborative perception in autonomous driving faces challenges with heterogeneous features from agents with different sensing modalities or model architectures, requiring scalable solutions for feature alignment without retraining encoders or pairwise interpreter modules.

Method: GT-Space constructs a common feature space from ground-truth labels as a unified reference for feature alignment. Each agent uses only a single adapter module to project features into this shared space, eliminating pairwise interactions. A fusion network is trained with contrastive losses across diverse modality combinations.

Result: Extensive experiments on simulation datasets (OPV2V and V2XSet) and real-world dataset (RCooper) demonstrate that GT-Space consistently outperforms baselines in detection accuracy while delivering robust performance.

Conclusion: GT-Space provides a flexible and scalable solution for heterogeneous collaborative perception that eliminates the need for pairwise feature alignment, enabling practical deployment of multi-agent systems with diverse sensing capabilities.
...
TL;DR: MemReward is a graph-based experience memory framework that uses GNNs to propagate limited reward labels to unlabeled LLM rollouts, achieving near-Oracle performance with only 20% labels across mathematical reasoning, QA, and code generation tasks.


<details>
  <summary>Details</summary>
Motivation: Reinforcement learning for LLMs requires reward labels to evaluate generated rollouts, but obtaining these labels at scale is expensive (requiring human experts or verification procedures) and limits RL fine-tuning effectiveness when labels are scarce.

Method: MemReward creates a heterogeneous graph where queries, thinking processes, and answers form nodes with similarity and structural edges. An initial LLM policy generates rollouts stored as experience memory. A GNN trained on labeled nodes propagates rewards to unlabeled rollouts during online optimization.

Result: With only 20% labels, MemReward achieves 97.3% of Oracle performance on Qwen2.5-3B and 96.6% on 1.5B across mathematics, question answering, and code generation tasks, even surpassing Oracle on out-of-domain tasks. Performance scales smoothly with label budget, reaching 99.4% of Oracle at 70% labels.

Conclusion: MemReward effectively addresses the reward label scarcity problem in RL fine-tuning of LLMs by leveraging graph-based experience memory and GNN-based reward propagation, enabling efficient RL optimization with limited labeled data while maintaining strong performance.
...
TL;DR: LeWorldModel (LeWM) is a stable Joint Embedding Predictive Architecture that trains end-to-end from raw pixels using only two loss terms, achieving efficient planning and meaningful latent representations without complex regularization.
...
Method: LeWM uses only two loss terms: a next-embedding prediction loss and a regularizer enforcing Gaussian-distributed latent embeddings. This reduces tunable loss hyperparameters from six to one compared to existing alternatives. The model has ~15M parameters and can be trained on a single GPU in a few hours.

Result: LeWM plans up to 48x faster than foundation-model-based world models while remaining competitive across diverse 2D and 3D control tasks. The latent space encodes meaningful physical structure (verified through probing of physical quantities), and surprise evaluation confirms reliable detection of physically implausible events.

Conclusion: LeWM demonstrates that stable end-to-end training of JEPAs from raw pixels is achievable with minimal loss terms, offering an efficient alternative to complex world models while maintaining competitive performance and meaningful latent representations.
...
TL;DR: DPxFin: A federated learning framework with reputation-guided adaptive differential privacy for anti-money laundering that dynamically adjusts privacy noise based on client reputation to balance privacy and accuracy.


<details>
  <summary>Details</summary>
Motivation: Money laundering detection faces challenges from data privacy concerns and complex fraud patterns. Federated learning is promising but prone to privacy leakage, especially with tabular financial data. Current approaches lack adaptive privacy mechanisms that balance utility and protection.

Method: DPxFin integrates reputation-guided adaptive differential privacy in federated learning. Client reputation is computed by evaluating alignment between local models and global model. Based on reputation scores, differential privacy noise is dynamically assigned: high-reputation clients receive lower noise to preserve their contributions, while low-reputation clients get stronger noise to mitigate risk.

Result: Experiments on Anti-Money Laundering dataset (IID and non-IID settings) using MLP show DPxFin achieves better accuracy-privacy trade-off than traditional FL and fixed-noise DP baselines. Performance improvements are consistent though modest. The framework withstands tabular data leakage attacks, proving effectiveness under real-world financial conditions.

Conclusion: DPxFin successfully addresses privacy leakage in federated learning for financial applications by adaptively adjusting differential privacy noise based on client reputation, achieving superior privacy-utility balance while maintaining robustness against data leakage attacks.
...
TL;DR: Multi-scale convolutional framework integrates multiple input representations for univariate time series classification, with three architectures: MSNet for robustness/calibration, LS-Net for efficiency, and adapted LiteMV for cross-representation interaction.


<details>
  <summary>Details</summary>
Motivation: Time series classification performance depends on both architectural design and input representation diversity. Current approaches lack systematic integration of multiple representations in a scalable framework.

Method: Proposed scalable multi-scale convolutional framework with three architectures: MSNet (hierarchical multi-scale network for robustness/calibration), LS-Net (lightweight variant for efficiency), and adapted LiteMV (originally for multivariate inputs) for cross-representation interaction on univariate signals.

Result: Evaluation across 142 benchmark datasets shows LiteMV achieves highest mean accuracy, MSNet provides best probabilistic calibration (lowest NLL), and LS-Net offers best efficiency-accuracy tradeoff. Critical Difference analysis confirms statistically significant performance differences.

Conclusion: Multi-representation multi-scale learning establishes a principled, practical direction for modern TSC, offering flexible design space tunable for accuracy, calibration, or resource-constrained settings.
...
TL;DR: Neural networks fail on compositional generalization (0% accuracy) but succeed (100% accuracy) when constrained by a Ternary Gamma Semiring, which forces learning of algebraic structure isomorphic to Boolean-type ternary Γ-semiring with |T|=4, |Γ|=1.


<details>
  <summary>Details</summary>
Motivation: To establish a theoretical framework connecting neural network learning with abstract algebraic structures, and to understand why neural networks succeed or fail on compositional generalization tasks.

Method: Present a minimal counterexample showing standard neural networks fail on compositional generalization (0% accuracy), then introduce a logical constraint (Ternary Gamma Semiring) to the same architecture, analyze the learned feature space mathematically, and compare with established classification of ternary Γ-semirings.

Result: The constrained network achieves 100% accuracy on novel combinations and learns a feature space that constitutes a finite commutative ternary Γ-semiring implementing majority vote rule, which corresponds precisely to the Boolean-type ternary Γ-semiring with |T|=4, |Γ|=1 (unique up to isomorphism).

Conclusion: Neural network success can be understood as approximation of mathematically natural structures; learned representations generalize because they internalize algebraic axioms (symmetry, idempotence, majority property); logical constraints guide networks to converge to canonical forms. This inaugurates Computational Γ-Algebra as a new interdisciplinary direction.
...
TL;DR: Proposes a DL framework using support sets and probabilistic modeling to handle discrete variables in wireless resource allocation, addressing zero-gradient, constraint enforcement, and non-SPSD property challenges.


<details>
  <summary>Details</summary>
Motivation: DL methods struggle with discrete variables due to zero-gradient issues in backpropagation, difficulty enforcing complex discrete constraints, and inability to generate non-SPSD solutions needed for many wireless resource allocation problems.

Method: Introduces support sets to represent discrete variables, models elements as random variables with learned joint probability distribution, factorizes joint probability into conditional probabilities learned sequentially, uses dynamic context embedding, and applies masking for constraint enforcement.

Result: Framework outperforms existing baselines in both system performance and computational efficiency for two mixed-discrete wireless resource allocation problems: joint user association/beamforming in cell-free systems and joint antenna positioning/beamforming in movable antenna systems.

Conclusion: The proposed probabilistic DL framework effectively addresses fundamental challenges in handling discrete variables for wireless resource allocation, providing a general solution that avoids zero-gradient issues, enforces constraints, and generates non-SPSD solutions.
...
TL;DR: TaCT is a concept-gated fine-tuning framework that selectively adapts deep learning weather models for rare extreme events like typhoons while preserving overall performance, using interpretable failure-related concepts discovered via sparse autoencoders.


<details>
  <summary>Details</summary>
Motivation: Current deep learning models for meteorological forecasting perform poorly on rare but high-impact events like typhoons due to data scarcity. Existing fine-tuning methods face a trade-off between ignoring these extreme events or overfitting to them at the expense of overall performance degradation.

Method: TaCT uses Sparse Autoencoders and counterfactual analysis to automatically discover failure-related internal concepts in models. It then performs selective parameter updates only when corresponding concepts are activated, rather than applying uniform adaptation. This creates a concept-gated fine-tuning mechanism.

Result: Experiments show consistent improvements in typhoon forecasting across different regions without degrading performance on other meteorological variables. The identified concepts correspond to physically meaningful circulation patterns, revealing model biases and supporting trustworthy adaptation.

Conclusion: TaCT provides an interpretable fine-tuning framework that selectively improves model performance on failure cases while preserving overall capabilities, enabling trustworthy adaptation for scientific forecasting tasks where rare high-impact events are critical.
...
TL;DR: A probabilistic flow-based framework for amortized inference of boundary conditions in patient-specific cardiovascular models, handling cases where boundary conditions cannot be tuned in isolation due to known mean flow, assumed waveforms, or vascular lesions affecting segmentation.


<details>
  <summary>Details</summary>
Motivation: Current data-driven variational inference methods for boundary condition tuning in cardiovascular modeling fail in two important scenarios: open-loop models with known mean flow/assumed waveform shapes, and anatomies with vascular lesions where segmentation affects pressure/flow split target reachability. In both cases, boundary conditions cannot be tuned independently.
...
Conclusion: The proposed probabilistic flow framework enables joint estimation of boundary conditions in complex cardiovascular modeling scenarios where traditional methods fail, particularly for cases involving vascular lesions and constrained flow conditions.
...
TL;DR: DAPA is a distribution-aware piecewise activation function for Transformers that uses non-uniform piecewise approximation based on pre-activation data distribution, achieving 16× speedup and 16× DSP reduction for GELU computation while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Non-linear activation functions are critical for on-device inference and training but consume substantial hardware resources and impact system performance and energy efficiency. There's a need for hardware-friendly activation functions that maintain accuracy while reducing computational overhead.

Method: DAPA uses a differentiable, hardware-friendly activation function that exploits pre-activation data distribution. It employs non-uniform piecewise approximation with finer segments allocated to high-probability regions, followed by quantization using Distribution-Weighted Mean Square Error to optimize for hardware deployment.

Result: HLS implementation shows DAPA speeds up GELU computation by 16× and decreases DSP utilization by 16× while maintaining comparable or better performance across vision Transformers and GPT-2 models compared to prior piecewise linear methods.

Conclusion: DAPA provides an effective hardware-friendly activation function for Transformers that significantly improves computational efficiency and resource utilization while preserving model accuracy, making it suitable for on-device deployment.
...
TL;DR: This paper proposes replacing fixed linear aggregation in neurons with learnable nonlinear alternatives to improve neural network robustness against noise, introducing F-Mean and Gaussian Support neurons with hybrid blending mechanisms.


<details>
  <summary>Details</summary>
Motivation: Standard weighted summation in neurons behaves like mean-based estimators, making them sensitive to noisy or extreme inputs. The paper investigates whether learnable nonlinear aggregation can improve robustness without sacrificing trainability.

Method: Introduces two differentiable aggregation mechanisms: F-Mean neuron (learnable power-weighted aggregation) and Gaussian Support neuron (distance-aware affinity weighting). Proposes hybrid neurons that interpolate between linear and nonlinear aggregation via learnable blending parameters to maintain optimization stability.

Result: Hybrid neurons consistently improve robustness under noise on CIFAR-10 and noisy CIFAR-10 variants. F-Mean hybrids also yield modest gains on clean data. Three-way hybrid achieves robustness scores up to 0.991 vs 0.890 baseline. Learned parameters converge to sub-linear aggregation (p ≈ 0.43-0.50) and high novelty utilization (α ≈ 0.69-0.79).
...
TL;DR: The paper challenges the assumption of layer homogeneity in transformer models, demonstrating profound anatomical heterogeneity through empirical analysis of SmolLM2-135M and proposing Growth Transformer Training for more efficient training.


<details>
  <summary>Details</summary>
Motivation: Current transformer language models are trained with uniform computational budgets across all layers, implicitly assuming layer homogeneity. The authors challenge this assumption through empirical analysis to reveal anatomical heterogeneity and explore more efficient training strategies.

Method: Analyzed SmolLM2-135M (30-layer, 135M-parameter causal language model) using five diagnostic metrics: weight predictability (R2), ablation degradation, recovery speed, weight manipulation robustness, and structural analysis. Proposed Growth Transformer Training that allocates budget by layer importance.

Result: Found profound anatomical heterogeneity: (1) Layer weights follow strong mathematical regularity (R2 = 0.91) with universal oscillatory delta pattern, but predicted weights cause catastrophic failure. (2) Layer importance spans 10^7 range, from critical core (L8-11) to anti-layers whose removal improves performance. (3) Recovery speed correlates with layer importance. (4) Only weight scaling preserves model quality. (5) Growth Transformer Training achieves ~54% cost reduction, with proof-of-concept showing 4.7x lower validation loss than uniform training at identical parameter count.

Conclusion: Transformer layers exhibit significant anatomical heterogeneity, challenging the assumption of layer homogeneity. Growth Transformer Training, which allocates computational budget based on layer importance, offers substantial efficiency improvements (54% cost reduction) and better performance compared to uniform training strategies.
...
TL;DR: The paper develops a mathematical model of learning bottlenecks where instructional signals are only usable when learners have acquired necessary prerequisites, creating structural and epistemic limits on learning speed.


<details>
  <summary>Details</summary>
Motivation: While generative AI has made information production cheap, the value of information depends on whether learners can absorb it. An explanation that clarifies a concept for one user may be noise to another lacking prerequisites, creating a learner-side bottleneck that needs mathematical modeling.

Method: Develops a mathematical model where the learner is an abstract learning system (mind) characterized by prerequisite structures over concepts. Teaching is modeled as sequential communication with a latent target, where instructional signals are only usable when the learner has acquired necessary prerequisites to parse them.

Result: Identifies two limits on learning speed: structural limit determined by prerequisite reachability and epistemic limit determined by uncertainty about the target. Shows threshold effects in training - when teaching horizon is below prerequisite depth, additional instruction cannot produce successful teaching. Demonstrates that common broadcast curriculum can be slower than personalized instruction by a factor linear in the number of learner types.

Conclusion: The effective communication channel for learning depends on the learner's current knowledge state and becomes more informative as learning progresses. The model explains why personalized instruction outperforms one-size-fits-all approaches and provides a framework for understanding learning bottlenecks in AI and human education systems.
...
TL;DR: WS-FM accelerates flow matching by using fast, low-quality draft samples as initial distribution instead of pure noise, enabling fewer integration steps while maintaining output quality.


<details>
  <summary>Details</summary>
Motivation: Current autoregressive LLMs and diffusion/FM models require many function evaluations during inference, making generation slow and computationally expensive despite producing high-quality samples.

Method: Warm-Start FM uses lightweight generative models to produce draft samples quickly, then uses these as initial distribution for flow matching starting closer to end time rather than from pure noise at time 0.

Result: WS-FM achieves guaranteed speed-up factors without sacrificing sample quality, demonstrated on synthetic toy data and real-world text/image generation tasks.

Conclusion: The approach provides a learning-to-refine framework that significantly reduces generation time while maintaining output quality, offering practical benefits for computationally demanding generative models.
...
TL;DR: Hierarchical RL framework for allocating limited testing/quarantine resources across asynchronous COVID-19 outbreak clusters, outperforming baselines by 20-30%.


<details>
  <summary>Details</summary>
Motivation: Real-world public health faces resource allocation challenges across multiple asynchronous outbreak clusters with varying sizes and risks, requiring decisions under uncertainty with operational constraints.

Method: Formulated as constrained restless multi-armed bandit problem; hierarchical RL with global controller learning cost multiplier and local policy estimating marginal value of resource allocation within clusters.

Result: Outperforms RMAB-inspired and heuristic baselines by 20-30% across various system scales and testing budgets; scales to 40 concurrent clusters with faster decision-making.

Conclusion: Hierarchical RL framework effectively addresses resource allocation challenges in multi-cluster outbreak control, demonstrating scalability and superior performance over existing methods.
...
TL;DR: GeoLAN introduces a geometry-aware training framework for LLMs that treats token representations as geometric trajectories and applies stickiness conditions inspired by the Kakeya Conjecture to improve transparency and interpretability.


<details>
  <summary>Details</summary>
Motivation: Large language models demonstrate strong performance but lack transparency, creating a need for approaches that enhance mechanistic interpretability while maintaining task performance.

Method: GeoLAN treats token representations as geometric trajectories and applies stickiness conditions inspired by the Kakeya Conjecture. The framework includes two differentiable regularizers: Katz-Tao Convex Wolff (KT-CW) that promotes isotropy, and Katz-Tao Attention (KT-Attn) that encourages diverse attention patterns.

Result: Experiments with Gemma-3 (1B, 4B, 12B) and Llama-3-8B show that GeoLAN frequently maintains task accuracy while improving geometric metrics and reducing certain fairness biases. Benefits are most significant in mid-sized models, revealing scale-dependent trade-offs between geometric precision and performance.

Conclusion: Geometry-aware training is a promising approach to enhance mechanistic interpretability in LLMs, with GeoLAN demonstrating that geometric constraints can improve transparency while maintaining performance, particularly in mid-sized models.
...
TL;DR: Deep learning methods for fully nonlinear second-order PDEs on Hilbert spaces using Hilbert-Galerkin Neural Operators, with novel universal approximation theorems and numerical training approaches.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of solving fully nonlinear second-order PDEs on infinite-dimensional Hilbert spaces, which arise in control problems for PDEs, SPDEs, path-dependent systems, and mean-field SDEs, where traditional methods face significant computational and theoretical difficulties.

Method: Develop Hilbert-Galerkin Neural Operators (HGNOs) to parameterize solutions, prove novel Universal Approximation Theorems for functions and their derivatives on Hilbert spaces, and create numerical training methods (Deep Hilbert-Galerkin and Hilbert Actor-Critic) that minimize the L²-norm of the PDE residual over the entire Hilbert space rather than finite-dimensional projections.

Result: First UATs powerful enough for fully nonlinear second-order PDEs on Hilbert spaces, including approximation of functions, their Fréchet derivatives up to second order, and unbounded operators on first derivatives. Successful numerical solution of Kolmogorov and HJB PDEs related to optimal control of deterministic/stochastic heat and Burgers' equations.

Conclusion: The paper presents a comprehensive deep learning framework for infinite-dimensional PDEs, establishing theoretical foundations through novel approximation theorems and demonstrating practical effectiveness through numerical implementations, opening new avenues for solving complex control problems in infinite dimensions.
...
TL;DR: The paper proves convergence of a fixed-point iteration for optimizing a regularized nuclear norm objective with Hadamard product structure, closing an open problem from prior work on private machine learning optimization.


<details>
  <summary>Details</summary>
Motivation: To close an open problem from Denisov et al. regarding convergence of a fixed-point iteration arising in optimization of regularized nuclear norm objectives with Hadamard product structure, which appears in private machine learning algorithm optimization.

Method: Analyzes the fixed-point iteration v ← φ(v) where v^(k+1) = diag((D_v^(k)^(1/2) M D_v^(k)^(1/2))^(1/2)), proving its monotonic convergence to the unique global optimizer of J(v) = 2Tr((D_v^(1/2) M D_v^(1/2))^(1/2)) - Σ v_i.

Result: Proves that the fixed-point iteration converges monotonically to the unique global optimizer, thereby closing the open problem. The proof was substantially assisted by Gemini 3 AI, with corrections and interventions.

Conclusion: The paper successfully closes the open problem while also serving as commentary on practical AI use in mathematics, including principles for working with AI to prove mathematical results.
...
TL;DR: ALP addresses off-policy RL instability in LLMs by injecting learnable perturbations into hidden states during updates to flatten importance ratio distributions and reduce policy divergence.


<details>
  <summary>Details</summary>
Motivation: Off-policy RL for LLMs suffers from policy staleness and training-inference mismatch, causing heavy-tailed importance ratios that destabilize training and limit exploration.

Method: Adaptive Layerwise Perturbation (ALP) injects small learnable perturbations into input hidden states of each layer during policy updates, using perturbed policy as numerator in importance ratios against unchanged inference policy.

Result: ALP improves final performance on math and tool-integrated reasoning tasks, prevents importance ratio tail blow-up and KL spikes, boosts exploration, with full-layer perturbations outperforming partial-layer and logits-only variants.

Conclusion: ALP effectively addresses off-policy RL instability by flattening importance ratio distributions through representation-level perturbations, maintaining training stability while improving performance and exploration.
...
TL;DR: TRACE is a diffusion model for trajectory recovery that reconstructs dense, continuous trajectories from sparse GPS inputs using a State Propagation Diffusion Model with memory mechanism.


<details>
  <summary>Details</summary>
Motivation: Real-world GPS trajectories are often sparse and unevenly distributed due to low sampling rates and limited infrastructure coverage, making recovery into dense continuous forms essential for location-based services and smart city applications.

Method: TRACE uses a State Propagation Diffusion Model (SPDM) with a novel memory mechanism that retains and leverages intermediate results from previous denoising steps to effectively reconstruct hard-to-recover trajectory segments.
...
TL;DR: ASEN is a single neural network that achieves simultaneous equivariance to multiple permutation subgroups by modulating an auxiliary symmetry-breaking input, overcoming computational hardness through approximate symmetry breaking and 2-closure theory.


<details>
  <summary>Details</summary>
Motivation: Traditional equivariant architectures are constrained to specific symmetries chosen a priori, limiting their applicability to diverse datasets and preventing the development of flexible multi-modal foundation models capable of processing varied data equivariantly.

Method: Start with a fully permutation-equivariant base model, then obtain subgroup equivariance using symmetry-breaking inputs whose automorphism group matches the desired subgroup. Overcome computational hardness by relaxing to approximate symmetry breaking using 2-closure theory to derive fast algorithms.

Result: Theoretically, ASEN can simulate equivariant MLPs and guarantees universality if the base model is universal. Empirically, ASEN outperforms both separate equivariant models and single non-equivariant models on symmetry selection for graph/image tasks and multitask/transfer learning for sequence tasks.

Conclusion: ASEN enables a single network to be simultaneously equivariant to multiple permutation subgroups, providing a flexible approach for multi-modal foundation models that can process diverse data equivariantly without requiring separate specialized architectures.
...
<details>
  <summary>Details</summary>
Motivation: Existing deep learning approaches for tabular anomaly detection are typically dataset-specific and assume a single supervision regime (one-class, unsupervised, or semi-supervised), limiting their ability to leverage shared structures across tasks and adapt to different supervision levels.

Method: ICLAD is trained via meta-learning on synthetic tabular anomaly detection tasks. At inference, it assigns anomaly scores by conditioning on the training set without updating model weights, using in-context learning to adapt to different datasets and supervision regimes.

Result: Comprehensive experiments on 57 tabular datasets from ADBench show state-of-the-art performance across three supervision regimes (one-class, unsupervised, and semi-supervised).

Conclusion: ICLAD establishes a unified framework for tabular anomaly detection that generalizes across both datasets and supervision regimes, overcoming limitations of previous dataset-specific approaches.
...
TL;DR: A stochastic sequential decision-making framework for filtering networked data on expanding graphs using multi-agent reinforcement learning and context-aware graph neural networks.


<details>
  <summary>Details</summary>
Motivation: Existing graph filters mainly study fixed graphs, ignoring that graphs often expand as nodes continually attach with unknown patterns. Current approaches rely on either pre-designed filters or online learning, which are limited to myopic views considering only past or present information, failing to account for future impacts of graph expansion.

Method: Propose a stochastic sequential decision-making framework that adapts filtering to expanding graphs. Represent filter shifts as agents, model the filter as a multi-agent system, and train the policy using multi-agent reinforcement learning to account for long-term rewards and capture expansion dynamics. Develop a context-aware graph neural network to parameterize the policy, which tunes filter parameters based on both graph and agent information.

Result: Experiments on synthetic and real datasets (from cold-start recommendation to COVID prediction) demonstrate the benefits of using a sequential decision-making perspective over batch and online filtering alternatives.

Conclusion: The proposed framework successfully addresses the challenge of filtering networked data on expanding graphs by incorporating future impacts through sequential decision-making and multi-agent reinforcement learning, outperforming traditional batch and online filtering approaches.
...
TL;DR: A federated learning framework for cross-HPC facility deployment addresses challenges of training large models on distributed scientific data while preserving privacy and managing HPC heterogeneity.


<details>
  <summary>Details</summary>
Motivation: Scientific AI applications require training large models on distributed data that cannot be centralized due to privacy, data sovereignty, or volume constraints. Federated learning enables collaborative training without data centralization, but deploying FL across HPC facilities presents unique challenges beyond cloud/enterprise settings.

Method: Developed a comprehensive cross-facility FL framework for heterogeneous HPC environments, built on Advanced Privacy-Preserving Federated Learning (APPFL) framework with Globus Compute and Transfer orchestration. Evaluated across four U.S. DOE leadership-class supercomputers.

Result: Demonstrated that FL experiments across HPC facilities are practically achievable. Characterized key sources of heterogeneity impacting training performance. Showed that algorithmic choices matter significantly under realistic HPC scheduling conditions. Validated scientific applicability by fine-tuning a large language model on a chemistry instruction dataset.

Conclusion: Cross-HPC FL deployment is feasible but requires addressing HPC-specific challenges. Scheduler-aware algorithm design is identified as a critical open challenge for future deployments of federated learning in scientific computing environments.
...
TL;DR: UKTL is a novel kernel framework for M-mode tensors that uses uncertainty-driven subspace weighting and scalable Nyström linearization for efficient learning from structured multi-way data.


<details>
  <summary>Details</summary>
Motivation: Learning from structured multi-way tensor data requires capturing complex interactions across tensor modes while maintaining computational efficiency. Existing methods may lack robustness to unreliable mode components and scalability for large-scale tensor data.

Method: UKTL compares mode-wise subspaces from tensor unfoldings using uncertainty-aware subspace weighting that adaptively down-weights unreliable mode components based on estimated confidence. It employs scalable Nyström kernel linearization with dynamically learned pivot tensors obtained via soft k-means clustering. The framework is end-to-end trainable and incorporates multi-way/multi-mode interactions through structured kernel compositions.

Result: Extensive evaluations on action recognition benchmarks (NTU-60, NTU-120, Kinetics-Skeleton) show UKTL achieves state-of-the-art performance, superior generalization, and provides meaningful mode-wise insights.

Conclusion: UKTL establishes a principled, scalable, and interpretable kernel learning paradigm for structured multi-way and multi-modal tensor sequences, offering robust similarity measures with computational efficiency.
...
TL;DR: The paper reveals adversarial vulnerability in vision and hallucination in LLMs share a common geometric origin: input and loss gradient are conjugate observables subject to an irreducible uncertainty bound (Neural Uncertainty Principle), enabling unified diagnosis and mitigation.


<details>
  <summary>Details</summary>
Motivation: Conventional approaches treat adversarial vulnerability in vision and hallucination in LLMs as separate problems requiring modality-specific solutions. The authors aim to show these issues share a common geometric origin and can be addressed through a unified theoretical framework.

Method: Formalizes a Neural Uncertainty Principle (NUP) showing input and loss gradient are conjugate observables with irreducible uncertainty bound. Designs a single-backward probe to measure input-gradient correlation. Proposes ConjMask (masking high-contribution components) and LogitReg (logit-side regularization) for vision robustness, and uses the probe as decoding-free hallucination risk signal for LLMs.

Result: NUP reveals that in near-bound regimes, compression increases sensitivity dispersion (adversarial fragility), while weak prompt-gradient coupling leaves generation under-constrained (hallucination). The probe successfully detects hallucination risk before token generation, and ConjMask improves robustness without adversarial training.

Conclusion: NUP provides a unified framework connecting two seemingly separate failure modes through a shared uncertainty-budget view. It offers principled tools for reliability analysis across perception and generation tasks, enabling practical mitigation strategies without costly adversarial training.
...
TL;DR: Wearable foundation models need to evolve from static, short-term prediction to longitudinal, anticipatory health reasoning by incorporating rich multimodal data, long-context modeling, and agentic inference systems.


<details>
  <summary>Details</summary>
Motivation: Current wearable foundation models are limited to short-term, retrospective prediction tasks and are poorly suited for modeling chronic, progressive, or episodic health conditions that unfold over extended time periods (weeks, months, or years).

Method: Proposes three foundational shifts: 1) Structurally rich data integration with multimodal, long-term personal trajectories and contextual metadata; 2) Longitudinal-aware multimodal modeling with long-context inference, temporal abstraction, and personalization; 3) Agentic inference systems supporting planning, decision-making, and clinically grounded interventions under uncertainty.

Result: The paper argues for a paradigm shift in wearable foundation models from static, retrospective signal interpretation to continuous, anticipatory, and human-aligned health support systems.

Conclusion: Wearable foundation models must move beyond static encoders and be explicitly designed for longitudinal, anticipatory health reasoning by implementing three foundational shifts in data, modeling, and inference systems.
...
TL;DR: Theoretical analysis of In-Context Learning (ICL) linking design choices to generalization, showing performance depends on demonstration quality, model's intrinsic ICL capability, distribution shift, and benefits of Chain-of-Thought prompting.


<details>
  <summary>Details</summary>
Motivation: Existing theoretical explanations of ICL either rely on strong assumptions or fail to capture practical factors like demonstration selection, CoT prompting, number of demonstrations, and prompt templates. This paper aims to address this gap with a theoretical analysis under mild assumptions.

Method: Established theoretical analysis of ICL under mild assumptions, deriving an upper bound on ICL test loss. Analyzed CoT prompting as inducing task decomposition. Characterized how ICL performance sensitivity to prompt templates varies with demonstration count.

Result: ICL performance governed by: (1) quality of selected demonstrations (quantified by Lipschitz constants), (2) intrinsic ICL capability of pretrained model, (3) degree of distribution shift. CoT beneficial when demonstrations well-chosen at each substep and subtasks easier to learn. Performance sensitivity to prompt templates varies with number of demonstrations.

Conclusion: Pretraining equips models with ability to generalize beyond observed tasks, CoT enables composition of simpler subtasks into complex ones, and demonstrations/instructions enable retrieval of similar/complex tasks, jointly supporting generalization to unseen tasks. Theoretical insights corroborated by experiments.
...
TL;DR: PC-FedAvg: A personalized constrained federated optimization method with cross-estimates enabling agent-specific constraints without consensus or constraint sharing, achieving O(ε⁻²) suboptimality and O(ε⁻¹) infeasibility rates.


<details>
  <summary>Details</summary>
Motivation: Standard FL formulations handle unconstrained or globally constrained problems, but practical settings involve heterogeneous resource/model constraints requiring agent-specific feasible sets. There's a need for personalized constrained FL that respects private constraints without requiring consensus or constraint sharing among agents.

Method: PC-FedAvg method where each agent maintains cross-estimates of other agents' variables through a multi-block local decision vector. Each agent updates all blocks locally, penalizing infeasibility only in its own block. The cross-estimate mechanism enables personalization without requiring consensus or sharing constraint information.

Result: Established communication-complexity rates of O(ε⁻²) for suboptimality and O(ε⁻¹) for agent-wise infeasibility. Preliminary experiments on MNIST and CIFAR-10 datasets validate theoretical findings.

Conclusion: PC-FedAvg effectively addresses personalized constrained federated optimization with agent-specific constraints, providing theoretical guarantees for convergence and feasibility while maintaining privacy of constraint information through cross-estimates.
...
TL;DR: Policy regularizations based on classical inventory concepts like Base Stock significantly improve DRL hyperparameter tuning and performance for inventory management, with successful deployment on Alibaba's Tmall platform.


<details>
  <summary>Details</summary>
Motivation: DRL offers a general methodology for inventory policy training but suffers from high sensitivity to hyperparameters and mixed success in off-the-shelf implementations, limiting practical deployment.
...
Result: 100% deployment on Alibaba's Tmall platform demonstrates practical success; extensive synthetic experiments show policy regularizations reshape which DRL methods perform best for inventory management.

Conclusion: Policy regularizations based on classical inventory theory significantly enhance DRL methods for inventory management, enabling successful real-world deployment and changing the comparative landscape of DRL approaches.
...
TL;DR: A continual learning framework for text-guided food classification that enables incremental updates to add new categories without retraining from scratch or degrading prior knowledge.


<details>
  <summary>Details</summary>
Motivation: Conventional machine learning pipelines struggle with recognizing categories absent from original training sets, reducing accuracy since fixed datasets rarely capture full domain diversity. This gap limits practical applications in food classification.

Method: Proposes a continual learning framework for text-guided food classification that enables incremental updates, allowing new categories to be integrated without requiring retraining from scratch or degrading prior knowledge.

Result: The framework shows promise for adaptive food recognition, enabling models trained on specific cuisines (e.g., Western) to later learn new dishes (e.g., dosa or kimchi) while maintaining prior knowledge.

Conclusion: Although further refinements are needed, the continual learning design demonstrates potential for adaptive food recognition with applications in dietary monitoring and personalized nutrition planning.
...
TL;DR: A zeroth-order proximal sampler using Gaussian mixture approximations and Monte Carlo score estimation, avoiding rejection sampling and enabling deterministic runtime with exponential convergence guarantees.


<details>
  <summary>Details</summary>
Motivation: Existing proximal samplers require rejection sampling for backward steps, while diffusion-based methods need learned models or auxiliary samplers for score estimation. This work aims to develop a more practical proximal sampler that operates with only zeroth-order information and avoids rejection sampling.

Method: Proposes a new approximate proximal sampler that treats intermediate particle distributions as Gaussian mixtures, enabling Monte Carlo score estimation from directly samplable distributions. The method simulates dynamics directly rather than using rejection sampling, uses flexible step sizes, and operates with deterministic runtime budget.

Result: Theoretically inherits exponential convergence of proximal sampling under isoperimetric conditions when score estimation error is controlled. Practically avoids rejection sampling, permits flexible step sizes, runs with deterministic runtime, and demonstrates rapid convergence to target distribution through particle interactions and parallel computation.

Conclusion: The proposed zeroth-order proximal sampler provides a practical alternative to existing methods by eliminating rejection sampling requirements while maintaining theoretical convergence guarantees and enabling efficient parallel implementation.
...
TL;DR: RiboSphere is a framework that learns discrete geometric representations of RNA by combining vector quantization with flow matching, enabling high-fidelity structure generation and effective transfer to downstream tasks.


<details>
  <summary>Details</summary>
Motivation: RNA structure modeling is challenging due to flexible backbones, prevalent non-canonical interactions, and scarcity of experimentally determined 3D structures. The modular organization of RNA architecture, where complex folds are composed from recurring structural motifs, motivates a discrete representation approach.

Method: RiboSphere uses a geometric transformer encoder to produce SE(3)-invariant features, which are discretized with finite scalar quantization (FSQ) into a finite vocabulary of latent codes. Conditioned on these discrete codes, a flow-matching decoder reconstructs atomic coordinates for structure generation.

Result: The model achieves strong performance in structure reconstruction (RMSD 1.25Å, TM-score 0.84). Learned code indices are enriched for specific RNA motifs, indicating capture of motif-level compositional structure. Pretrained discrete representations transfer effectively to inverse folding and RNA-ligand binding prediction with robust generalization in data-scarce regimes.

Conclusion: RiboSphere demonstrates that learning discrete geometric representations through vector quantization and flow matching enables accurate RNA structure modeling and effective transfer learning for downstream tasks, leveraging the modular nature of RNA architecture.
...
<details>
  <summary>Details</summary>
Motivation: Classical SA analyses assume martingale difference or Markov noise with bounded moments, but practical applications in finance and communications often encounter heavy-tailed and long-range dependent noise, requiring new theoretical frameworks.

Method: Develops a noise-averaging argument that regularizes noise impact without modifying the iteration, establishing finite-time moment bounds for SA under heavy-tailed and LRD noise for strongly monotone operators.

Result: Establishes explicit convergence rates quantifying the impact of heavy tails and temporal dependence, with applications to stochastic gradient descent and gradient play, validated through numerical experiments.

Conclusion: Provides the first finite-time analysis of SA under non-classical noise models, offering a general framework applicable to optimization and reinforcement learning problems with heavy-tailed or temporally dependent noise.
...
TL;DR: EFGA improves upon FGA by combining extracted rules into ensembles using aggregation criteria, achieving significantly higher recall with minimal precision reduction on MNIST and LSC datasets.


<details>
  <summary>Details</summary>
Motivation: Existing Feature Guided Analysis (FGA) techniques for DNN explainability produce rules with good precision but limited recall, restricting their applicability across different situations.

Method: Ensembles-based Feature Guided Analysis (EFGA) combines rules extracted by FGA into ensembles using aggregation criteria (policies that dictate rule combination). The paper explores three different aggregation criteria and evaluates their trade-offs.

Result: EFGA significantly improves recall over FGA (+28.51% train, +25.76% test on MNIST; +33.15% train, +30.81% test on LSC) with negligible precision reduction (-0.89% on MNIST, -0.69% on LSC). Different aggregation criteria offer alternative precision-recall trade-offs.

Conclusion: EFGA effectively addresses FGA's recall limitation by aggregating rules into ensembles, providing a flexible framework where different aggregation criteria enable users to balance precision and recall according to their needs.
...
TL;DR: KV cache is redundant in transformer inference; keys and values can be perfectly recomputed from residual stream vectors with zero error, enabling memory-efficient inference.


<details>
  <summary>Details</summary>
Motivation: Current transformer inference relies heavily on KV cache storage, which consumes significant memory. The paper aims to prove this cache is unnecessary and develop more memory-efficient inference methods.

Method: Proves KV cache redundancy mathematically, validates across six models (135M-4B parameters) via residual patching experiments, and implements KV-Direct - a bounded-memory scheme that checkpoints residual vectors (5KB/token) instead of full KV pairs (136KB).

Result: KV-Direct maintains 100% token match at all cache budgets while baselines degrade to 5-28%; reduces peak memory from 103MB to 42MB over 20 conversation turns; recomputation runs up to 5x faster than cache reads at moderate batch sizes.

Conclusion: KV cache is entirely redundant and can be eliminated without accuracy loss; residual stream is the sole information-carrying state; KV-Direct enables bounded-memory inference with perfect accuracy.
...
TL;DR: The paper analyzes reverse diffusion sampling error propagation, identifying a metric mismatch between early radial contraction and terminal Euclidean error measurement, and develops a one-switch routing argument with explicit W2 guarantees.


<details>
  <summary>Details</summary>
Motivation: Existing analyses propagate sampling error in Euclidean geometry along the entire reverse trajectory, but Gaussian smoothing creates contraction first at large separations while short separations remain non-dissipative, creating a metric mismatch between early radial contraction and terminal Euclidean error measurement.

Method: Formalizes the mismatch through an explicit radial lower profile for the learned reverse drift, uses a one-switch routing argument: before switch, reflection coupling yields contraction in concave transport metric adapted to radial profile; at switch, converts once from this metric back to W2 under p-moment budget, then propagates converted discrepancy over remaining short window in Euclidean geometry.

Result: Obtains explicit non-asymptotic end-to-end W2 guarantees for discretizations of learned reverse SDE under L2 score-error control, one-sided Lipschitz condition of score error, and standard well-posedness and coupling hypotheses; provides scalar switch-selection objective and sharp structural limit on conversion exponent within affine-tail concave class.

Conclusion: The analysis reveals fundamental metric mismatch in reverse diffusion error propagation, provides rigorous framework for understanding contraction dynamics, and delivers practical guarantees for sampling algorithms through structured switching strategy between different geometric regimes.
...
TL;DR: GoAgent proposes a group-centric communication topology generation method for LLM-based multi-agent systems that treats collaborative groups as atomic units, using conditional information bottleneck to reduce communication overhead while improving coordination.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based multi-agent systems generate communication topologies in a node-centric manner, leading to implicit group structures that result in suboptimal coordination and unnecessary communication overhead. There's a need for explicit modeling of collaborative groups as atomic units to improve coordination efficiency.

Method: GoAgent first enumerates task-relevant candidate groups through LLM, then autoregressively selects and connects these groups as atomic units to construct the final communication graph. It introduces a conditional information bottleneck objective to compress inter-group communication, preserving task-relevant signals while filtering redundant historical noise.

Result: Extensive experiments on six benchmarks demonstrate state-of-the-art performance with 93.84% average accuracy while reducing token consumption by about 17% compared to existing approaches.

Conclusion: Explicitly modeling collaborative groups as atomic units in communication topology generation significantly improves coordination efficiency in LLM-based multi-agent systems, achieving better performance with reduced communication overhead through conditional information bottleneck compression.
...
TL;DR: Proposes a hybrid ontology-based uncertainty-aware framework integrating Weighted Interval Type-2 Fuzzy Logic with semantic knowledge modeling to improve AQI classification reliability and uncertainty handling for Indian air quality monitoring.


<details>
  <summary>Details</summary>
Motivation: Traditional AQI calculation uses crisp thresholds and deterministic aggregation rules that are unsuitable for handling uncertainty and transitions between air quality classes, especially in rapidly urbanizing areas where air pollution is a major environmental and public health concern.
...
Result: Experimental evaluation using CPCB air quality datasets demonstrates improved AQI classification reliability and uncertainty handling compared to traditional crisp and Type-1 fuzzy approaches, while enabling explainable semantic reasoning and intelligent decision support.

Conclusion: The proposed hybrid ontology-based uncertainty-aware framework effectively addresses limitations of traditional AQI calculation methods by better handling uncertainty, providing semantic reasoning capabilities, and supporting intelligent decision-making for air quality monitoring systems.
...
TL;DR: Sleeping Competing Bandits extends competing bandits to handle varying player/arm availability, achieving near-optimal regret bounds with an algorithm that's asymptotically optimal when K > N.


<details>
  <summary>Details</summary>
Motivation: Real-world competing bandit problems involve players and arms with varying availability over time, unlike conventional models that assume constant availability. This paper addresses the need to handle such dynamic availability patterns in competing bandit settings.

Method: Formulates Sleeping Competing Bandits as an extension of competing bandits with time-varying availability. Proposes an algorithm that handles sleeping players/arms and extends the regret definition to account for availability constraints.

Result: Achieves asymptotic regret bound of O(NK log T_i/Δ²) under reasonable assumptions, with a matching lower bound of Ω(N(K-N+1) log T_i/Δ²). The algorithm is asymptotically optimal when number of arms K is larger than number of players N.

Conclusion: Sleeping Competing Bandits provides a realistic extension to competing bandits that handles dynamic availability. The proposed algorithm achieves near-optimal performance, particularly in settings where arms outnumber players.
...
TL;DR: SD-Pcomp classification: Binary classification framework using only relative judgments (similarity/dissimilarity and pairwise preference) instead of probabilistic weak labels, with improved robustness to noise and uncertainty.


<details>
  <summary>Details</summary>
Motivation: Existing SconfConfDiff framework requires continuous probabilistic supervision (similarity-confidence and confidence-difference), which involves subjective uncertainty quantification leading to unstable supervision. Need for more robust approach using only binary relative judgments.

Method: Proposes SD-Pcomp classification using Similarity/Dissimilarity (SD) labels (class agreement between instances) and Pairwise Comparison (Pcomp) labels (preference toward positive class). Develops two unbiased risk estimators: (1) convex combination of SD and Pcomp, and (2) unified estimator integrating both labels by modeling their relationship.
...
Conclusion: SD-Pcomp classification provides a more practical and robust weakly supervised learning framework by eliminating the need for subjective probabilistic labeling and relying only on binary relative judgments.
...
TL;DR: FedRG uses representation geometry and vMF mixture modeling to identify noisy labels in federated learning under heterogeneous data distributions, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Federated learning suffers from performance degradation due to noisy annotations in distributed scenarios. Existing approaches relying on scalar loss values for noisy sample recognition lack reliability under heterogeneous data distributions.

Method: FedRG follows "representation geometry priority" principle: 1) Creates label-agnostic spherical representations using self-supervision, 2) Iteratively fits spherical von Mises-Fisher mixture model to capture semantic clusters using previously identified clean samples, 3) Integrates geometric evidence with semantic-label soft mapping to derive distribution divergence between label-free and annotated label-conditioned feature spaces, 4) Employs personalized noise absorption matrix on noisy labels for robust optimization.

Result: Extensive experimental results demonstrate that FedRG significantly outperforms state-of-the-art methods for federated learning with data heterogeneity under diverse noisy client scenarios.

Conclusion: The representation geometry approach provides a more reliable method for noisy label recognition in federated learning under heterogeneous conditions compared to loss-based methods, enabling robust optimization through geometric evidence integration and personalized noise handling.
...
TL;DR: FedPDPO: A personalized federated learning framework for aligning LLMs with human preferences using parameter-efficient fine-tuning, addressing non-IID data challenges through shared LoRA adapters, personalized DPO training with explicit reward heads, and bottleneck adapters.


<details>
  <summary>Details</summary>
Motivation: Aligning LLMs with human preferences in federated learning is challenging due to decentralized, privacy-sensitive, and highly non-IID preference data. Direct application of DPO in FL suffers from severe performance degradation under non-IID data and limited generalization of implicit rewards.

Method: FedPDPO uses parameter-efficient fine-tuning with frozen pretrained LLM backbones augmented with LoRA adapters. It addresses non-IID heterogeneity through: (1) globally shared LoRA adapter with personalized client-specific LLM head, (2) personalized DPO training with client-specific explicit reward head to complement implicit rewards, and (3) bottleneck adapter to balance global and local features.

Result: Extensive experiments on multiple preference datasets demonstrate state-of-the-art performance, achieving up to 4.80% average accuracy improvements in federated intra-domain and cross-domain settings. Theoretical analysis establishes the probabilistic foundation and soundness of the approach.

Conclusion: FedPDPO effectively bridges the gap in aligning LLMs with human preferences in federated learning settings, overcoming challenges of non-IID data through personalized parameter-efficient fine-tuning with explicit reward modeling and bottleneck adaptation mechanisms.
...
TL;DR: DPA is a novel attribution framework that traces information flow in frozen transformers with O(1) complexity, enabling efficient and faithful dense component attribution without counterfactual examples.


<details>
  <summary>Details</summary>
Motivation: Understanding internal mechanisms of transformer-based LLMs is crucial for reliable deployment, but existing attribution methods struggle to balance faithfulness and computational efficiency, with dense component attribution being prohibitively expensive.

Method: DPA analytically decomposes and linearizes SwiGLU Transformers into distinct pathways, propagating a targeted unembedding vector through one forward and one backward pass to trace information flow without requiring counterfactual examples.

Result: DPA achieves O(1) time complexity with respect to number of model components, scales to long input sequences, and demonstrates state-of-the-art faithfulness with unprecedented efficiency on standard interpretability benchmarks.

Conclusion: DPA provides an efficient and faithful attribution framework that enables practical dense component analysis of transformer-based LLMs, addressing the computational bottleneck of existing methods while maintaining high faithfulness.
...
TL;DR: Novel coreset construction for multivariate conditional transformation models (MCTMs) enables efficient large-scale semi-parametric regression and density estimation via importance sampling with guaranteed statistical accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing non-parametric and semi-parametric regression methods lack scalability for large datasets, limiting their practical application in statistics and machine learning despite their flexibility in modeling complex distributions and non-linear relationships.

Method: Developed coreset construction for MCTMs using importance sampling with geometric approximation based on convex hull of input data to handle logarithmic terms, ensuring data reduction while maintaining statistical accuracy within (1±ε) multiplicative error bounds.

Result: First coresets for semi-parametric distributional models; substantial computational efficiency improvements demonstrated in numerical experiments with large, complex datasets while preserving statistical model accuracy.

Conclusion: The proposed coreset method enables scalable semi-parametric modeling for large datasets, bridging the gap between statistical flexibility and computational efficiency, with broad applications in statistics and machine learning.
...
TL;DR: GATE is a circuit optimization methodology that reduces quantum feature maps using a gate significance index combining fidelity, entanglement, and sensitivity, achieving circuit size and runtime reductions while preserving or improving accuracy.
...
Method: GATE uses a novel gate significance index quantifying gate relevance through fidelity, entanglement, and sensitivity. It iteratively scans threshold ranges, eliminates low-contribution gates, generates optimized QML models, and ranks them based on accuracy, runtime, and balanced performance before final testing.

Result: The methodology shows consistent reductions in circuit size and runtime with preserved or improved predictive accuracy across noise-free simulation, noisy emulation, and real IBM quantum hardware using PegasosQSVM and Quantum Neural Network models.

Conclusion: GATE effectively optimizes quantum feature maps, with best trade-offs typically occurring at intermediate thresholds rather than baseline circuits or aggressively compressed versions, demonstrating practical circuit optimization for current quantum devices.
...
TL;DR: GazeX-LSTM model integrates eye gaze data with contextual factors to predict pedestrian behavior in shared spaces with automated shuttles, showing super-additive improvements over head orientation alone.


<details>
  <summary>Details</summary>
Motivation: Predicting pedestrian behavior in unstructured shared spaces with automated shuttles is critical for safety and efficiency, but current methods lack human-centered attention data.

Method: VR study capturing pedestrian interactions across scenarios; GazeX-LSTM model integrating trajectories, fine-grained eye gaze dynamics, and contextual factors using LSTM architecture.

Result: Eye gaze provides unique predictive power beyond head orientation; combination with contextual information yields super-additive improvements in prediction accuracy.

Conclusion: Eye gaze-informed modeling fundamentally advances pedestrian behavior prediction; situational contexts play critical role in shared-space interactions for safer automated vehicles.
...
TL;DR: GDEGAN introduces a Gaussian Dynamic Equivariant Graph Attention Network with adaptive kernels for protein binding site prediction, outperforming existing methods by 37-66% in DCC and 7-19% in DCA metrics.


<details>
  <summary>Details</summary>
Motivation: Current equivariant GNN methods for binding site identification use dot product attention that ignores variation in chemical and geometric properties of neighboring residues, limiting their ability to accurately capture binding site characteristics.

Method: Proposes GDEGAN which replaces dot-product attention with adaptive kernels that recognize binding sites. The attention mechanism captures variation in neighboring residues using statistics of their characteristic local feature distributions, dynamically computing neighborhood statistics at each layer with local variance as adaptive bandwidth parameter and learnable per-head temperatures.
...
Conclusion: The proposed adaptive attention mechanism effectively captures local variations in protein residues for binding site prediction, offering significant improvements over state-of-the-art methods with direct applications in accelerating protein-ligand docking and therapeutic target identification.
...
TL;DR: FIPO is a reinforcement learning algorithm that improves reasoning in large language models by using future-KL divergence to create dense advantage formulations, enabling better credit assignment and breaking through length stagnation in chain-of-thought reasoning.
...
Method: FIPO incorporates discounted future-KL divergence into policy updates to create a dense advantage formulation that re-weights tokens based on their influence on subsequent trajectory behavior, enabling more precise credit assignment.
...
Conclusion: Dense advantage formulations through future-KL divergence are essential for evolving ORM-based algorithms to unlock the full reasoning potential of base models, addressing fundamental credit assignment limitations in current approaches.
...
TL;DR: NASimJax is a JAX-based reimplementation of Network Attack Simulator achieving 100x speedup, enabling RL training on larger networks for penetration testing with novel action decomposition and generalization studies.


<details>
  <summary>Details</summary>
Motivation: Existing penetration testing simulators are too slow for training RL policies at scale, resulting in poor generalization. There's a need for faster simulation to enable experimentation on realistic network scenarios and study zero-shot policy generalization.

Method: 1) NASimJax: JAX-based reimplementation of NASim for 100x speedup; 2) Formulate penetration testing as Contextual POMDP; 3) Network generation pipeline for diverse, solvable scenarios; 4) Two-stage action decomposition (2SAS) for scaling action spaces; 5) Compare Prioritized Level Replay vs Domain Randomization.

Result: Achieved up to 100x higher environment throughput; Enabled training on networks up to 40 hosts; 2SAS outperforms flat action masking at scale; Prioritized Level Replay handles dense distributions better than Domain Randomization; Sparse topology training provides implicit curriculum for OOD generalization.

Conclusion: NASimJax provides a fast, flexible platform for RL-based penetration testing research. Key findings include: 2SAS effectively handles large action spaces, sparse training topologies enable better generalization, and there are interactions between replay methods and credit assignment that require attention.
...
TL;DR: The paper investigates latent generalization in deep networks that memorize shuffled labels, tracking its training dynamics and developing linear probes to extract and transfer this generalization to model performance.


<details>
  <summary>Details</summary>
Motivation: Deep networks can memorize shuffled training labels while retaining latent generalization abilities in their internal representations, but the origin, training dynamics, and linear decodability of this latent generalization are not well understood.

Method: 1) Track training dynamics of latent generalization during memorization; 2) Analyze MASC probe structure showing it's a quadratic classifier; 3) Design new linear probe for latent generalization; 4) Develop method to transfer latent generalization to model generalization via weight editing using the linear probe.

Result: Latent generalization peaks early in training alongside model generalization; MASC probe is quadratic/non-linear; linear probes can extract latent generalization; latent generalization can be transferred to model generalization through weight editing.

Conclusion: Deep networks retain significant latent generalization even when memorizing corrupted labels, which can be linearly decoded from layerwise representations and transferred to improve model generalization, suggesting new approaches for leveraging internal representations.
...
TL;DR: Proposes an approach for discovering decision synchronization patterns in business processes that consider properties of multiple cases simultaneously, addressing a gap in current process discovery techniques.


<details>
  <summary>Details</summary>
Motivation: Current process discovery techniques rarely address decision synchronization mechanisms that consider properties of multiple cases at once, despite their importance for fair resource allocation, case prioritization, and preventing unnecessary waiting in business processes.

Method: Develops an approach inspired by supply chain processes to discover decision synchronization patterns, which consist of specific process constructs combined with constraints determining which case to execute. Formalizes and demonstrates discovery of constraints for four such patterns.

Result: Successfully evaluated in two artificial scenarios: (1) four separate process models each containing a single decision synchronization pattern, showing reliable discovery of each pattern type; (2) a process model containing all four patterns, demonstrating generalizability to complex problems.

Conclusion: The approach reliably retrieves expected decision synchronization patterns, addressing an important gap in process discovery by enabling identification of mechanisms that synchronize decisions across multiple running cases.
...
TL;DR: SCRL introduces selective positive and negative pseudo-labeling to improve test-time reinforcement learning for LLMs, addressing noise amplification from weak consensus in challenging reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Existing TTRL methods rely exclusively on positive pseudo-labeling, which becomes vulnerable when answer distributions are highly dispersed, leading to weak consensus that reinforces incorrect trajectories as supervision signals.
...
Result: Extensive experiments on multiple reasoning benchmarks demonstrate substantial improvements over baselines while maintaining robust generalization and training stability under constrained rollout budgets.
...
TL;DR: KGmetaSP is a knowledge-graph-embeddings approach that leverages existing experiment data to improve pipeline performance estimation and dataset similarity estimation, outperforming traditional meta-feature methods.


<details>
  <summary>Details</summary>
Motivation: Existing meta-learning approaches rely primarily on dataset meta-features but overlook valuable past experimental results and pipeline metadata, limiting their ability to capture dataset-pipeline interactions that reveal performance similarity patterns.

Method: Proposes KGmetaSP, which represents datasets and pipelines within a unified knowledge graph (KG) and derives embeddings that support pipeline-agnostic meta-models for PPE and distance-based retrieval for DPSE.

Result: Validated on a large-scale benchmark of 144,177 OpenML experiments, KGmetaSP enables accurate PPE using a single pipeline-agnostic meta-model and improves DPSE over baselines.

Conclusion: KGmetaSP establishes a new reference point for meta-learning by demonstrating how consolidating open experiment data into a unified knowledge graph advances the field, with the approach, KG, and benchmark released publicly.
...
TL;DR: Memori is an LLM-agnostic persistent memory layer that structures conversation data into semantic triples and summaries, achieving 81.95% accuracy on LoCoMo benchmark while using only 5% of full context tokens.


<details>
  <summary>Details</summary>
Motivation: Existing memory approaches for LLM agents create vendor lock-in and rely on injecting large volumes of raw conversation into prompts, leading to high token costs and degraded performance. There's a need for LLM-agnostic, cost-efficient memory systems.

Method: Memori treats memory as a data structuring problem with an Advanced Augmentation pipeline that converts unstructured dialogue into compact semantic triples and conversation summaries, enabling precise retrieval and coherent reasoning.

Result: On the LoCoMo benchmark, Memori achieves 81.95% accuracy while using only 1,294 tokens per query (~5% of full context). This represents 67% fewer tokens than competing approaches and over 20x savings compared to full-context methods.
...
TL;DR: Graph2TS: A structure-residual approach for time-series generation using quantile-based transition graphs to separate global temporal structure from stochastic variations, enabling controlled generation with improved fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing generative models struggle to balance preserving global temporal structure with modeling stochastic local variations, especially for volatile signals with weak/irregular periodicity. Direct distribution matching can amplify noise or suppress meaningful patterns.

Method: Proposes a structure-residual perspective viewing time series as structural backbone + stochastic residuals. Represents structure using quantile-based transition graphs capturing global distributional and temporal dependencies. Develops Graph2TS, a quantile-graph conditioned variational autoencoder for cross-modal generation from structural graphs to time series.
...
Conclusion: Structure-controlled cross-modal generation is a promising direction for time-series modeling, enabling preservation of global temporal organization while allowing controlled stochastic variation.
...
TL;DR: Proposed LiteNP-Net, a lightweight neural network for physical layer authentication that combines theoretical optimality of Neyman-Pearson hypothesis testing with practical deep learning advantages, achieving near-optimal performance without requiring channel statistics knowledge.


<details>
  <summary>Details</summary>
Motivation: Address limitations in IoT physical layer authentication: traditional hypothesis testing (Neyman-Pearson detector) requires channel statistics knowledge which is impractical, while deep learning approaches lack theoretical optimality guarantees.

Method: Developed LiteNP-Net by incorporating conditional statistical models into hypothesis testing framework to derive theoretically optimal NP detector, then building lightweight neural network driven by this NP detector structure.

Result: Simulations showed LiteNP-Net approaches NP detector performance without channel statistics knowledge. Experimental Wi-Fi testbed deployment demonstrated LiteNP-Net outperforms conventional correlation-based methods and state-of-the-art Siamese-based methods in real-world scenarios.

Conclusion: LiteNP-Net successfully bridges theoretical optimality and practical implementation in physical layer authentication, providing a lightweight, effective solution for IoT security that doesn't require channel statistics knowledge while maintaining near-optimal performance.
...
TL;DR: RL for LLMs faces a "capability ceiling" due to using expanding action histories as states instead of compact Markov states; introducing explicit Markov states theoretically reduces sample complexity and empirically breaks performance boundaries in complex reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Current RL approaches for post-training LLMs act as mere refiners of pre-existing patterns rather than enabling genuine discovery, creating a persistent "capability ceiling." This limitation stems from using ever-expanding action histories as states instead of compact, informative Markov states that classical RL relies on for novel strategy discovery.

Method: The paper revisits the classical RL principle of explicit Markov states, which has been absent from LLM post-training. Theoretically, it provides rigorous guarantees that leveraging estimated Markov states can reduce sample complexity. Empirically, it introduces Markov states into RL post-training and evaluates this approach across a suite of complex logic puzzles.

Result: The introduction of Markov states consistently breaks the performance boundaries of standard RL post-training across complex logic puzzles. The approach demonstrates significant improvements over traditional "history-as-state" modeling in LLM post-training.

Conclusion: Moving beyond "history-as-state" modeling in favor of structured Markovian representations is essential for unlocking open-ended discovery and genuinely new reasoning capabilities in Generative AI. Explicit Markov states represent a fundamental structural improvement needed to overcome the capability ceiling in RL-based LLM alignment.
...
TL;DR: SuperKMeans is a faster k-means variant for clustering high-dimensional embeddings, achieving 7x speedup over FAISS/Scikit-Learn on CPUs and 4x over cuVS on GPUs while maintaining centroid quality for similarity search.


<details>
  <summary>Details</summary>
Motivation: Traditional k-means clustering for high-dimensional vector embeddings is computationally expensive, especially with large datasets. There's a need for faster clustering methods that maintain retrieval quality for similarity search tasks.

Method: SuperKMeans accelerates clustering by pruning unnecessary dimensions during vector-to-centroid assignment and introduces Early Termination by Recall, which stops iterations when centroid quality for retrieval tasks stops improving.

Result: 7x faster than FAISS and Scikit-Learn on CPUs, 4x faster than cuVS on GPUs, while maintaining centroid quality for vector similarity search. The method reduces data-access and compute overhead through dimension pruning.

Conclusion: SuperKMeans provides significant speed improvements for k-means clustering of high-dimensional embeddings without compromising retrieval quality, making it practical for large-scale similarity search applications.
...
TL;DR: Ensemble-Decoupled Architecture Search reduces NAS validation cost from O(M) to O(1) by predicting ensemble performance from single-learner evaluation using ensemble theory.


<details>
  <summary>Details</summary>
Motivation: Neural Architecture Search faces prohibitive O(M) computational cost for validating candidate architectures in industrial systems where large ensembles (M=50-200) are standard for robustness, severely limiting architecture iteration frequency.

Method: Introduces Ensemble-Decoupled Theory with sufficient condition for monotonic ensemble improvement, using lightweight dual-learner training to estimate ΔE, ρ, and σ². Unifies three solution strategies: closed-form optimization for tractable continuous π, constrained differentiable optimization for intractable continuous π, and LLM-driven search with iterative monotonic acceptance for discrete π.

Result: Decouples architecture search from full ensemble training, reducing per-candidate search cost from O(M) to O(1) while maintaining O(M) deployment cost only for validated winners. Framework reveals two orthogonal improvement mechanisms: base diversity gain and accuracy gain.

Conclusion: Provides actionable design principles for industrial-scale NAS by enabling efficient architecture search in ensemble-based production systems while maintaining deployment robustness through ensemble verification of validated winners.
...
TL;DR: ODySSeI is an open-source end-to-end framework for automated detection, segmentation, and severity estimation of coronary lesions in invasive coronary angiography images, featuring a Pyramidal Augmentation Scheme for robustness and real-time performance across diverse patient populations.


<details>
  <summary>Details</summary>
Motivation: Invasive Coronary Angiography (ICA) interpretation suffers from subjectivity and operator variability, creating a need for automated, objective analysis tools to improve consistency and reproducibility in coronary artery disease assessment.

Method: ODySSeI integrates deep learning-based lesion detection and segmentation models trained with a novel Pyramidal Augmentation Scheme (PAS) to enhance robustness. It includes a quantitative coronary angiography-free Lesion Severity Estimation technique that computes Minimum Lumen Diameter and diameter stenosis directly from predicted lesion geometry.

Result: PAS yields significant performance gains: 2.5-fold increase in lesion detection versus 1-3% improvement in segmentation. LSE achieves high accuracy with MLD predictions within ±2-3 pixels of ground truth. The framework processes images in seconds on CPU and fractions of a second on GPU, demonstrating strong generalizability across diverse patient cohorts.

Conclusion: ODySSeI establishes a comprehensive, open-source framework for automated, reproducible, and scalable ICA analysis, supporting real-time clinical decision-making with strong performance across detection, segmentation, and severity estimation tasks.
...
TL;DR: SPMA-OG is a geometry-preserving continual learning method that maintains a shared latent manifold across tasks through sparse replay, output distillation, relational geometry preservation, local smoothing, and chart-assignment regularization on old anchors.


<details>
  <summary>Details</summary>
Motivation: Current continual learning methods (parameter regularization, output matching, replay) reduce forgetting but don't directly specify how latent representations should evolve. The paper proposes a geometric approach for scenarios where old and new data should remain on the same latent support, viewing continual learning as continuation of a shared manifold.

Method: Support-Preserving Manifold Assimilation (SPMA) with geometry-preserving variant SPMA-OG combines: 1) sparse replay of old examples, 2) output distillation, 3) relational geometry preservation, 4) local smoothing, and 5) chart-assignment regularization on old anchors to maintain geometric structure.

Result: On CIFAR10 and Tiny-ImageNet with compatible shifts, SPMA-OG improves old-task retention and representation-preservation metrics over sparse replay baselines while maintaining competitive new-task accuracy. On synthetic atlas-manifold benchmark, achieves near-perfect anchor-geometry preservation while improving new-task accuracy over replay.

Conclusion: Geometry-aware anchor regularization provides a useful inductive bias for continual learning when preserving a shared latent support is more appropriate than creating a new one. The geometric approach directly addresses representation evolution in compatible-shift scenarios.
...
TL;DR: Federated hyperdimensional computing enables lightweight collaborative learning for resource-constrained Industrial IoT devices by exchanging only prototype representations, reducing communication overhead while maintaining fast convergence.


<details>
  <summary>Details</summary>
Motivation: Industrial IoT systems face severe resource constraints (memory, compute, bandwidth) that challenge deployment of advanced analytics like predictive maintenance. There's a need for lightweight learning paradigms that can operate effectively in these constrained environments while supporting distributed intelligence.

Method: Integrates hyperdimensional computing (HDC) into a federated learning framework. HDC leverages high-dimensional vector spaces for energy-efficient training/inference. In the federated approach, devices exchange only prototype representations rather than full models or data, significantly reducing communication overhead.

Result: Numerical results demonstrate federated HDC supports collaborative learning in IIoT with fast convergence speed and communication efficiency. The approach shows potential for lightweight distributed intelligence in large-scale, resource-constrained environments.

Conclusion: HDC represents a lightweight and resilient framework for distributed intelligence in large-scale, resource-constrained IIoT environments, enabling efficient collaborative learning through prototype-based federated learning with reduced communication overhead.
...
TL;DR: Three RL algorithms used to fine-tune financial forecasters show performance improvements and transfer learning benefits after supervised-to-RL fine-tuning.


<details>
  <summary>Details</summary>
Motivation: To enhance financial forecasting models by applying reinforcement learning fine-tuning to models originally trained with supervised learning, and to demonstrate practical implementation benefits for practitioners.

Method: Proposed implementation plan for backpropagating RL loss to supervised-trained models, using three major RL algorithms for fine-tuning financial forecasters, with comparison of pre- and post-fine-tuning performance.

Result: Performance increase after fine-tuning, demonstration of transfer learning properties in the models, and empirical results showing benefits of the fine-tuning approach.

Conclusion: RL fine-tuning of supervised-trained financial forecasters provides performance improvements and transfer learning benefits, with practical tuning process guidance for practitioners.
...
TL;DR: Diffusion model generation is interpreted as out-of-equilibrium phase transition where reverse diffusion passes through critical regime amplifying fluctuations to form large-scale structure.


<details>
  <summary>Details</summary>
Motivation: To develop a theoretical framework explaining diffusion model generation as non-equilibrium critical phenomena rather than smooth noise-to-data evolution, connecting deep learning to statistical physics.

Method: Theoretical analysis using analytically tractable patch score models, connecting to Ginzburg-Landau field theories; empirical validation on trained convolutional diffusion models measuring mode softening and correlation growth.

Result: Demonstrated critical regime with mode softening and growing spatial correlations; showed targeted perturbations at critical time improve generation control via classifier-free guidance pulses.

Conclusion: Non-equilibrium critical phenomena provide unifying principle for understanding diffusion models, with practical implications for generation control through critical regime manipulation.
...
TL;DR: Temporal abstraction acts as a low-pass filter that reduces spectral mismatch in forward-backward representations, enabling stable successor representation learning in continuous spaces.


<details>
  <summary>Details</summary>
Motivation: Forward-backward representations struggle with spectral mismatch between high-rank continuous transition dynamics and low-rank architectural bottlenecks, making accurate successor representation learning difficult.

Method: Analyze temporal abstraction as a mechanism to mitigate spectral mismatch by characterizing spectral properties of transition operators and showing temporal abstraction acts as a low-pass filter that suppresses high-frequency components.
...
Conclusion: Temporal abstraction provides a principled mechanism for shaping spectral structure of MDPs and enabling effective long-horizon representations in continuous control by aligning spectral properties with architectural constraints.
...
TL;DR: λ-RLM replaces open-ended recursive code generation with typed functional runtime based on λ-calculus for verifiable, predictable long-context reasoning with formal guarantees.


<details>
  <summary>Details</summary>
Motivation: Existing Recursive Language Models (RLMs) use open-ended read-eval-print loops with arbitrary control code generation, making execution difficult to verify, predict, and analyze, creating reliability and efficiency issues for long-context reasoning.

Method: λ-RLM framework replaces free-form recursive code generation with typed functional runtime grounded in λ-calculus, executing a compact library of pre-verified combinators and using neural inference only on bounded leaf subproblems, turning recursive reasoning into structured functional programs with explicit control flow.

Result: Outperforms standard RLM in 29 of 36 model-task comparisons across four long-context reasoning tasks and nine base models, improves average accuracy by up to +21.9 points across model tiers, and reduces latency by up to 4.1x.

Conclusion: Typed symbolic control yields a more reliable and efficient foundation for long-context reasoning than open-ended recursive code generation, with formal guarantees including termination, cost bounds, accuracy scaling, and optimal partition rules.
...
TL;DR: Competition paper on detecting trojan horse attacks in deep forecasting models for spacecraft telemetry, with over 200 teams developing solutions to identify hidden triggers in time series forecasting models.


<details>
  <summary>Details</summary>
Motivation: Deep forecasting models in safety-critical applications like space operations are vulnerable to trojan horse attacks via backdoors in training data or model weights, posing significant security risks that need to be addressed.

Method: Organized a data science competition (Trojan Horse Hunt) with novel task formulation, benchmark set, and evaluation protocol where participants developed methods to identify triggers hidden in deep forecasting models for spacecraft telemetry.

Result: Over 200 teams participated and developed solutions; the paper describes the competition framework, best solutions, and provides all materials publicly available on Kaggle.

Conclusion: The competition advances research on detecting trojan attacks in time series forecasting models, provides key insights, and identifies research directions for effective trigger identification in safety-critical applications.
...
TL;DR: GenAI-driven framework for network telemetry that jointly optimizes what to observe and how to encode it, achieving >50% reduction in sampling/data transfer while maintaining task fidelity.
...
Method: Goal-oriented framework integrating adaptive sampling policies (using adaptive masking) with generative modeling to identify patterns and preserve critical features across temporal/spatial dimensions. Selectively acquired data processed through hybrid compression combining traditional lossless coding with GenAI-driven lossy compression.
...
Conclusion: The proposed GenAI-driven sampling and hybrid compression framework successfully redesigns network telemetry from a goal-oriented perspective, addressing sustainability challenges by optimizing what to observe and how to encode it based on downstream task relevance.
...
TL;DR: Var-JEPA reformulates JEPA as a variational latent-variable model, showing JEPA's non-generative claims are rhetorical rather than structural, and achieves better representation learning with principled uncertainty quantification.


<details>
  <summary>Details</summary>
Motivation: The paper challenges the view that JEPA is fundamentally non-generative, arguing that its separation from probabilistic generative modeling is rhetorical rather than structural. The authors aim to bridge JEPA with variational inference to create a more principled framework with explicit generative structure and uncertainty quantification.

Method: The authors derive Variational JEPA (Var-JEPA) by showing that standard JEPA's coupled encoders and predictor mirror variational posteriors and learned conditional priors in coupled latent-variable models. They optimize a single Evidence Lower Bound (ELBO) to make the latent generative structure explicit, eliminating the need for ad-hoc anti-collapse regularizers. They instantiate Var-T-JEPA for tabular data.

Result: Var-JEPA achieves strong representation learning and downstream performance, consistently improving over standard T-JEPA while remaining competitive with strong raw-feature baselines. The framework provides meaningful representations without ad-hoc regularizers and enables principled uncertainty quantification in latent space.

Conclusion: JEPA can be viewed as a deterministic specialization of variational latent-variable models, and making this generative structure explicit through Var-JEPA yields a more principled framework with better representation learning and uncertainty quantification capabilities.
...
TL;DR: A method using a single scalar parameter to bias attention logits enables controllable protein sequence generation toward functional subsets without model retraining.


<details>
  <summary>Details</summary>
Motivation: Existing stochastic attention methods for protein sequence generation treat all stored sequences equally and cannot direct generation toward specific functional subsets of interest, limiting practical utility for targeted protein design.

Method: Adds a single scalar parameter as bias to the sampler's attention logits to continuously shift generation from full family toward user-specified subsets, requiring only a small set of sequences and a multiplicity ratio, with no model retraining or architecture changes.

Result: Method enables exact conditioning at sampler's internal representation level, identifies calibration gap due to encoding dimensionality reduction, shows monotonic relationship between geometric separation and gap across five Pfam families, and successfully generates over a thousand candidate peptides preserving pharmacophore and binding determinants for omega-conotoxin.

Conclusion: Stochastic attention enables practitioners to expand small sets of experimentally characterized sequences into diverse candidate libraries without retraining generative models, with conditioning effectiveness predictable by geometric separation of functional subsets in encoding space.
...
TL;DR: Proposes an agentic AI-based virtual study group framework for extracting aging-related biological knowledge from Gene Ontology terms using hierarchical feature selection and validation against existing literature.


<details>
  <summary>Details</summary>
Motivation: To leverage emerging agentic AI techniques to revolutionize traditional scientific discovery pipelines, specifically for extracting meaningful aging-related biological knowledge from Gene Ontology data.

Method: Develops a knowledge discovery-oriented virtual study group using agentic AI, applies hierarchical feature selection methods to identify highly aging-related Gene Ontology terms, analyzes four different model organisms, and validates findings by reviewing existing research articles.

Result: Majority of AI agent-generated scientific claims are supported by existing literature, and the proposed internal mechanisms of the virtual study group play an important role in the agentic AI-based knowledge discovery framework.

Conclusion: Agentic AI techniques can effectively extract meaningful biological knowledge from Gene Ontology data, with validation showing strong alignment with existing scientific literature, demonstrating the potential of this approach for scientific discovery.
...
TL;DR: D-MMD enables effective distillation of discrete diffusion models by adapting successful continuous domain techniques, maintaining quality/diversity and sometimes outperforming teachers.


<details>
  <summary>Details</summary>
Motivation: Discrete diffusion models lack effective distillation methods compared to continuous diffusion models, which have many approaches for reducing sampling steps significantly.

Method: Discrete Moment Matching Distillation (D-MMD) adapts successful continuous domain distillation techniques to the discrete domain, using moment matching principles.

Result: D-MMD maintains high quality and diversity with sufficient sampling steps (unlike previous methods that collapse), works on both text and image datasets, and distilled generators can outperform their teachers.

Conclusion: D-MMD successfully bridges the distillation gap between continuous and discrete diffusion models, enabling efficient sampling while preserving or improving performance.
...
TL;DR: KaCGM is a causal generative model using Kolmogorov-Arnold Networks (KANs) for interpretable causal modeling of mixed-type tabular data, enabling direct inspection of learned mechanisms while maintaining competitive performance.


<details>
  <summary>Details</summary>
Motivation: Deep causal models often use opaque architectures that limit auditability in high-stakes domains, creating a need for models that combine expressive causal modeling with functional transparency for trustworthy deployment.

Method: KaCGM parameterizes each structural equation with Kolmogorov-Arnold Networks (KANs), enabling decomposition for inspection, symbolic approximations, and visualization of parent-child relationships. Includes validation pipeline using distributional matching and independence diagnostics of exogenous variables.
...
Conclusion: Expressive causal generative modeling and functional transparency can be achieved jointly using KAN-based parameterization, supporting trustworthy deployment in tabular decision-making settings.
...
TL;DR: A control-space learning framework for few-step swarm steering under sampled-data constraints using finite-horizon minimum-energy control coefficients.


<details>
  <summary>Details</summary>
Motivation: Real swarm systems operate in sampled-data form with intermittent control updates, requiring finite-window control quantities rather than instantaneous velocity fields for effective steering with few control updates.

Method: Introduces a control-space learning framework inspired by MeanFlow that learns coefficients parameterizing finite-horizon minimum-energy control over each sampling interval, using integral representation and local differential identity along bridge trajectories with stop-gradient training.

Result: The framework provides a scalable approach to few-step swarm steering that respects sampled-data structure, with learned coefficients used directly in sampled-data updates while respecting prescribed dynamics and actuation maps.

Conclusion: The control-space learning framework enables consistent, scalable few-step swarm steering that properly addresses the sampled-data nature of real control systems.
...
TL;DR: SAT encodings for classical planning that keep actions lifted while partially grounding predicates, achieving linear scaling with plan length and outperforming state-of-the-art on hard-to-ground domains.


<details>
  <summary>Details</summary>
Motivation: To address the trade-off between fully lifted planning (compact but complex reasoning) and fully grounded planning (simple but exponential blowup) by finding a middle ground that avoids full grounding while maintaining computational efficiency.

Method: Introduces three SAT encodings that operate at a partially lifted level: actions remain lifted while predicates are partially grounded. Unlike previous quadratic-scaling SAT encodings, these scale linearly with plan length.

Result: The best encoding outperforms state-of-the-art methods in length-optimal planning on hard-to-ground domains, demonstrating improved performance for longer plans due to linear scaling.

Conclusion: Partial grounding of predicates while keeping actions lifted provides an effective middle ground between fully lifted and fully grounded planning, enabling efficient SAT-based planning with linear scaling in plan length.
...
TL;DR: DGM-Hyperagents introduce self-referential agents with editable meta-level modification procedures, enabling metacognitive self-improvement that transfers across domains and accumulates over time.


<details>
  <summary>Details</summary>
Motivation: Existing self-improving AI systems rely on fixed, handcrafted meta-level mechanisms that fundamentally limit improvement speed. Darwin Gödel Machine (DGM) shows open-ended self-improvement in coding, but this alignment between task performance and self-modification skill doesn't generally hold beyond coding domains.

Method: Introduce hyperagents - self-referential agents integrating task agent (solves target task) and meta agent (modifies itself and task agent) into a single editable program. The meta-level modification procedure is itself editable, enabling metacognitive self-modification. Extend DGM to create DGM-Hyperagents (DGM-H), eliminating domain-specific alignment assumption.

Result: DGM-H improves performance over time across diverse domains, outperforming baselines without self-improvement/open-ended exploration and prior self-improving systems. It improves the process by which it generates new agents (e.g., persistent memory, performance tracking), and these meta-level improvements transfer across domains and accumulate across runs.
...
Method: Multi-modal language model-based agent trained using multi-turn process-reward reinforcement learning following supervised fine-tuning, enabled by ControlSketch-Part dataset with automatic part-level annotations obtained through a structured multi-stage labeling pipeline.

Result: The approach enables interpretable, controllable, and locally editable text-to-vector sketch generation by incorporating structured part-level data and providing visual feedback through the generation process.

Conclusion: Structured part-level data and process-based visual feedback enable more interpretable and controllable vector sketch generation, allowing for local editing and part-by-part construction.
...
TL;DR: Fine-tuning LLMs for formal counterexample generation with symbolic mutation strategy and multi-reward expert iteration framework, validated on new benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current AI mathematics research focuses primarily on proof construction while neglecting counterexample generation, which is equally important for mathematical reasoning. There's a gap in AI systems that can both propose counterexamples and produce formal proofs verifiable in theorem provers like Lean 4.

Method: 1) Formalize counterexample generation as requiring both candidate proposals and formal proofs verifiable in Lean 4. 2) Introduce symbolic mutation strategy that synthesizes diverse training data by extracting theorems and discarding selected hypotheses to create counterexample instances. 3) Develop multi-reward expert iteration framework combining curated datasets with mutation strategy for efficient training.
...
Conclusion: The proposed approach successfully addresses the gap in AI mathematical reasoning by enabling LLMs to generate formal counterexamples, with the symbolic mutation strategy and multi-reward expert iteration framework proving effective for training.
...
TL;DR: ItinBench is a new benchmark that integrates spatial reasoning (route optimization) with traditional verbal reasoning tasks in trip itinerary planning to evaluate LLMs' performance across multiple cognitive domains simultaneously.


<details>
  <summary>Details</summary>
Motivation: Existing LLM evaluations focus on specific reasoning tasks in controlled environments, but real-world applications require handling multiple cognitive domains concurrently. Travel planning provides a context for integrating various reasoning tasks, but current benchmarks lack comprehensive multi-domain cognitive evaluation.

Method: Developed ItinBench benchmark featuring spatial reasoning (route optimization) integrated with traditional verbal reasoning tasks within trip itinerary planning. Evaluated multiple LLMs including Llama 3.1 8B, Mistral Large, Gemini 1.5 Pro, and GPT family across diverse tasks simultaneously.

Result: LLMs struggle to maintain high and consistent performance when concurrently handling multiple cognitive dimensions. Performance varies across different cognitive domains, revealing limitations in comprehensive reasoning capabilities.

Conclusion: ItinBench provides new insights for building more comprehensive reasoning testbeds that better reflect real-world challenges by incorporating tasks from distinct human-level cognitive domains, highlighting the need for LLMs to improve multi-domain cognitive integration.
...
TL;DR: PA2D-MORL is a novel multi-objective reinforcement learning method that uses Pareto ascent directional decomposition and evolutionary optimization to achieve superior approximation of Pareto policy sets in complex continuous control tasks.


<details>
  <summary>Details</summary>
Motivation: Existing MORL methods struggle to achieve high-quality approximations of Pareto policy sets, especially in complex tasks with continuous or high-dimensional state-action spaces where conflicting objectives must be balanced.

Method: Proposes Pareto Ascent Directional Decomposition based MORL (PA2D-MORL) with three key components: 1) Uses Pareto ascent direction to select scalarization weights and compute multi-objective policy gradient for joint improvement on all objectives, 2) Selectively optimizes multiple policies under evolutionary framework to approximate Pareto frontier from different directions, 3) Applies Pareto adaptive fine-tuning to enhance density and spread of Pareto frontier approximation.
...
Conclusion: PA2D-MORL provides an effective solution for complex multi-objective decision-making problems by combining Pareto ascent directional decomposition with evolutionary optimization, achieving superior approximation of Pareto policy sets compared to existing methods.
...
TL;DR: PowerLens uses LLMs for personalized mobile power management on Android, achieving 38.8% energy savings over stock Android through context-aware policy generation with safety verification and implicit preference learning.


<details>
  <summary>Details</summary>
Motivation: Existing mobile power management systems use static rules or coarse-grained heuristics that ignore user activities and personal preferences, leading to suboptimal battery life management.

Method: PowerLens employs a multi-agent architecture with LLMs for commonsense reasoning to bridge semantic gaps between user activities and system parameters. It uses UI semantics for context recognition, generates holistic power policies across 18 device parameters, verifies actions with a PDL-based constraint framework, and learns individual preferences through a two-tier memory system with confidence-based distillation from implicit user overrides.

Result: Extensive experiments on rooted Android devices show 81.7% action accuracy, 38.8% energy saving over stock Android, outperforming rule-based and LLM-based baselines. The system achieves high user satisfaction, fast preference convergence within 3-5 days, strong safety guarantees, and consumes only 0.5% of daily battery capacity.

Conclusion: PowerLens demonstrates that LLMs can effectively enable safe, personalized mobile power management through zero-shot, context-aware policy generation that adapts to individual preferences without explicit configuration, significantly improving battery life while maintaining user satisfaction and safety.
...
TL;DR: HyEvo is an automated workflow generation framework that combines LLMs for semantic reasoning with deterministic code nodes for rule-based execution, using evolutionary search to optimize workflow topology and reduce inference costs.


<details>
  <summary>Details</summary>
Motivation: Existing automated workflow generation methods are inefficient and underperform due to reliance on predefined operator libraries and homogeneous LLM-only workflows where all computation is done through probabilistic inference.
...
Result: HyEvo consistently outperforms existing methods across diverse reasoning and coding benchmarks, reducing inference cost by up to 19× and execution latency by up to 16× compared to state-of-the-art open-source baselines.

Conclusion: HyEvo addresses limitations of existing workflow generation methods by combining heterogeneous computation models and evolutionary search, achieving superior performance while significantly reducing computational costs.
...
TL;DR: Proposes MiRA framework combining subgoal decomposition for online planning and milestone-based rewards for RL training, achieving state-of-the-art web navigation performance with open models surpassing proprietary systems.


<details>
  <summary>Details</summary>
Motivation: LLM-based agents struggle with long-horizon planning in web navigation due to losing track during online execution and difficulty learning from sparse rewards during RL fine-tuning, requiring better planning mechanisms and training signals.

Method: Two contributions: 1) Agent framework using proprietary models for online planning via subgoal decomposition, 2) MiRA RL training framework with dense milestone-based reward signals to guide learning in extended tasks.

Result: Real-time planning improves proprietary models by ~10% absolute SR on WebArena-Lite. MiRA boosts Gemma3-12B from 6.4% to 43.0% SR, surpassing GPT-4-Turbo (17.6%), GPT-4o (13.9%), and previous SOTA WebRL (38.4%).

Conclusion: Combining explicit inference-time planning with milestone-based rewards significantly enhances long-horizon capabilities, enabling more robust autonomous systems and demonstrating open models can outperform proprietary ones with proper training.
...
TL;DR: Neuro-symbolic framework combining LLMs with interactive theorem provers to automate proof generation for systems-level verification, achieving 77.6% theorem proving rate on seL4 benchmark.


<details>
  <summary>Details</summary>
Motivation: Formal verification via interactive theorem proving is manual and limits scalability, while LLMs show promise in mathematical reasoning but need integration with symbolic verification tools for effective proof automation.

Method: Best-first tree search over proof states with LLM queries for candidate steps, fine-tuning LLMs on proof state-step pairs, and integrating ITP tools for step repair, state filtering/ranking, and subgoal discharge.

Result: Achieves 77.6% theorem proving rate on FVEL seL4 benchmark, surpassing previous LLM-based approaches and standalone Sledgehammer, with strong generalization across additional Isabelle benchmarks.

Conclusion: The neuro-symbolic framework demonstrates viable path toward scalable automated software verification by synergizing neural and symbolic approaches for data-efficient LLM adaptation and semantics-informed search space pruning.
...
TL;DR: The paper argues for "embodied science" - reframing scientific discovery as a closed-loop process where AI agents physically interact with experimental environments, rather than just making isolated predictions.


<details>
  <summary>Details</summary>
Motivation: Current AI approaches to scientific discovery are misaligned with reality - they treat discovery as isolated, task-specific predictions rather than continuous physical interaction with the world through experimental cycles.

Method: Proposes a unified Perception-Language-Action-Discovery (PLAD) framework where embodied agents: perceive experimental environments, reason over scientific knowledge, execute physical interventions, and internalize outcomes to drive subsequent exploration.

Result: The approach bridges the gap between digital prediction and empirical validation by grounding computational reasoning in robust physical feedback.

Conclusion: Embodied science offers a roadmap for autonomous discovery systems in life and chemical sciences by reframing discovery as a closed loop coupling agentic reasoning with physical execution.
...
TL;DR: FormalEvolve: A neuro-symbolic evolutionary framework for autoformalization that optimizes both semantic consistency and prover effectiveness through LLM-driven mutation/crossover with AST rewrites and bounded patch repair.


<details>
  <summary>Details</summary>
Motivation: Current autoformalization approaches focus on semantic consistency but ignore prover effectiveness—semantically consistent formalizations can still differ substantially in proof-search cost and success rates, creating a gap between translation quality and practical usability.

Method: FormalEvolve treats autoformalization as budgeted test-time search for semantically consistent repertoires. It uses LLM-driven mutation and crossover with bounded patch repair, plus symbolic AST rewrite operations to inject structural diversity. The framework is compilation-gated and neuro-symbolic.

Result: On CombiBench and ProofNet with T=100 generator-call budget: achieves semantic hit rates (SH@100) of 58.0% and 84.9%, reduces cross-problem concentration of semantic successes (lower Gini coefficient). Also improves downstream proving performance on CombiBench under fixed prover budget.

Conclusion: FormalEvolve demonstrates that evolutionary search with neuro-symbolic components can effectively bridge the gap between semantic consistency and prover effectiveness in autoformalization, producing diverse formalizations that are both semantically correct and practically provable.
...
TL;DR: Proposes utility-guided orchestration policy for LLM agents to balance answer quality and execution cost, providing controllable framework for studying quality-cost trade-offs.


<details>
  <summary>Details</summary>
Motivation: Tool-using LLM agents face tension between answer quality and execution cost; fixed workflows are inflexible while free-form methods like ReAct improve performance at expense of excessive tool calls, longer trajectories, higher token consumption, and increased latency.

Method: Treats agent orchestration as explicit decision problem with utility-guided policy selecting among actions (respond, retrieve, tool call, verify, stop) by balancing estimated gain, step cost, uncertainty, and redundancy.

Result: Experiments across various approaches (direct answering, threshold control, fixed workflows, ReAct, policy variants) show explicit orchestration signals substantially affect agent behavior; lightweight utility design provides defensible and practical mechanism for agent control.

Conclusion: Provides controllable and analyzable policy framework for studying quality-cost trade-offs in tool-using LLM agents, demonstrating that explicit orchestration can balance performance and cost effectively.
...
TL;DR: Transformers can provably learn to verify long plans in many classical planning domains, with theoretical guarantees for length generalization using C*-RASP framework.
...
Method: Introduce C*-RASP, an extension of C-RASP designed to establish length generalization guarantees for transformers under simultaneous growth in sequence length and vocabulary size. Analyze decoder-only models' ability to verify plan correctness.
...
Conclusion: The paper provides theoretical foundations for understanding transformer generalization in planning tasks, showing transformers can learn to verify plans with provable guarantees for length generalization in many classical domains.
...
<details>
  <summary>Details</summary>
Motivation: Current RL with rubric-based rewards for LLMs suffers from ineffective exploration confined to current policy distribution, limiting performance improvements despite showing progress in enhancing reasoning capabilities.

Method: HeRL treats failed trajectories along with their unmet rubrics as hindsight experience, providing in-context guidance for policy exploration beyond current distribution. It introduces a bonus reward to incentivize responses with greater improvement potential under such guidance.

Result: Extensive experiments across various benchmarks demonstrate superior performance gains over baselines, with the ability to benefit from experience-guided self-improvement at test time.

Conclusion: HeRL facilitates effective learning from desired high-quality samples without repeated trial-and-error, yielding more accurate gradient estimation and enabling better exploration toward reward-maximizing distributions.
...
TL;DR: DIAL-KG is a closed-loop framework for incremental knowledge graph construction that addresses limitations of static methods by using a Meta-Knowledge Base to orchestrate dual-track extraction, governance adjudication, and schema evolution in a continuous cycle.


<details>
  <summary>Details</summary>
Motivation: Conventional KG construction methods are static, requiring complete graph reconstructions for new data and being constrained by predefined schemas, making them suboptimal for dynamic real-world scenarios where data arrives continuously and schemas need to evolve.

Method: DIAL-KG uses a Meta-Knowledge Base to orchestrate a three-stage cycle: (1) Dual-Track Extraction (triple generation for simple knowledge, event extraction for complex knowledge), (2) Governance Adjudication (ensuring fact fidelity and currency to prevent hallucinations/staleness), and (3) Schema Evolution (inducing new schemas from validated knowledge to guide subsequent cycles, with incremental KG updates).

Result: Extensive experiments demonstrate that DIAL-KG achieves state-of-the-art performance in both constructed graph quality and induced schema quality.

Conclusion: The DIAL-KG framework successfully addresses limitations of static KG construction by enabling incremental, adaptive knowledge graph building with evolving schemas, making it suitable for dynamic real-world applications.
...
TL;DR: The paper examines challenges in evaluating automated interpretability systems, particularly for circuit analysis, revealing pitfalls in replication-based evaluation and proposing unsupervised intrinsic evaluation methods.
...
Method: Built an agentic system where a research agent iteratively designs experiments and refines hypotheses for circuit analysis. Evaluated against human expert explanations across six circuit analysis tasks, then proposed unsupervised intrinsic evaluation based on functional interchangeability of model components.

Result: The system appeared competitive with human expert explanations initially, but closer examination revealed pitfalls: human explanations can be subjective/incomplete, outcome-based comparisons obscure research process, and LLM systems may reproduce findings via memorization or guessing.

Conclusion: The work demonstrates fundamental challenges in evaluating complex automated interpretability systems and reveals key limitations of replication-based evaluation, highlighting the need for more robust evaluation frameworks.
...
TL;DR: The paper introduces a structured cognitive trajectory model for Theory of Mind reasoning with LLMs that represents mental states as dynamic belief graphs, improving action prediction and recovering interpretable belief trajectories in high-uncertainty environments.
...
Method: A structured cognitive trajectory model that: 1) projects textual probabilistic statements to consistent probabilistic graphical model updates, 2) uses energy-based factor graphs to represent belief interdependencies, and 3) employs an ELBO-based objective capturing belief accumulation and delayed decisions.

Result: The model significantly improves action prediction across multiple real-world disaster evacuation datasets and recovers interpretable belief trajectories consistent with human reasoning.

Conclusion: The approach provides a principled module for augmenting LLMs with Theory of Mind capabilities in high-uncertainty environments, addressing limitations of prior methods by modeling beliefs as dynamic, interdependent structures.
...
TL;DR: Survey paper organizing uncertainty-aware multi-criteria decision-making (MCDM) into a task-oriented taxonomy covering problem settings, weight elicitation, solution procedures, and guidance for method selection.


<details>
  <summary>Details</summary>
Motivation: Real-world decision-making faces challenges from vagueness, incomplete information, heterogeneous data, and conflicting expert opinions, necessitating a comprehensive review of uncertainty-aware MCDM approaches to help practitioners navigate the field.

Method: The survey organizes the field through a task-oriented taxonomy that categorizes: 1) problem-level settings (discrete, group/consensus, dynamic, multi-stage, multi-level, multiagent, multi-scenario), 2) weight elicitation methods (subjective/objective schemes under fuzzy/linguistic inputs), 3) inter-criteria structure and causality modelling, and 4) solution procedures including compensatory scoring, distance-to-reference, compromise approaches, non-compensatory outranking frameworks, and rule/evidence-based sequential models.

Result: The survey provides a structured overview of uncertainty-aware MCDM, highlighting typical inputs, core computational steps, and primary outputs for different approaches, along with practical guidance on selecting methods based on robustness, interpretability, and data availability requirements.

Conclusion: The paper concludes by identifying open research directions including explainable uncertainty integration, stability analysis, and scalability improvements for large-scale and dynamic decision environments, emphasizing the need for continued development in uncertainty-aware MCDM.
